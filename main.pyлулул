...
# Redis обязателен для работы в многопроцессном режиме
...
redis_db = 1  # используем отдельную БД для FSM

# ==================== СОЗДАНИЕ БОТА ====================
bot = Bot(token=BOT_TOKEN, parse_mode="HTML")

# Используем RedisStorage для FSM
...
def cached_redis(ttl=60):
    """Декоратор для кеширования результата асинхронной функции в Redis."""
    def decorator(func):
        @wraps(func)
        async def wrapper(*args, **kwargs):
            ...
            await redis_set(key, json.dumps(result), ttl)
            return result
        return wrapper
    return decorator
...
DEFAULT_SETTINGS = {
    ...
    "cleanup_days_completed_giveaways": "30",

    # ----- АВТОУДАЛЕНИЕ -----
    "auto_delete_commands_seconds": "30",
    "auto_delete_bot_messages_seconds": "30",
    ...
    # ----- ЗАДАНИЯ (расширено) -----
    "task_subscribe_check_interval": "300",
    "task_unsubscribe_penalty_coins": "100",
    "task_unsubscribe_penalty_reputation": "5",
    ...
    "giveaway_allow_repeat_wins": "1",
    "giveaway_auto_cleanup_days": "7",
}
...
HEIST_TYPES = {
    "incassator": {
        ...
        "phrases_split": [
            "🔪 Дележ добычи! У тебя {minutes} минут, чтобы попытаться кинуть подельников! Жми /betray, если готов рискнуть!",
...
# ==================== ГЛОБАЛЬНЫЕ ПЕРЕМЕННЫЕ ДЛЯ КЕШИРОВАНИЯ В ПАМЯТИ ====================
db_pool = None
settings_cache = {}
...
# Новая функция для отправки сообщения ботом с автоудалением
async def send_temp_message(chat_id: int, text: str, delete_seconds: int = None, **kwargs):
    """Отправляет сообщение, которое будет автоматически удалено через указанное время."""
    if delete_seconds is None:
        delete_seconds = int(await get_setting("auto_delete_bot_messages_seconds"))
...
async def init_db():
    async with db_pool.acquire() as conn:
        # Таблица users (расширена для новых функций)
        await conn.execute('''
        ...
        await conn.execute('''
            CREATE TABLE IF NOT EXISTS giveaways (
                ...
                allow_repeat_wins BOOLEAN DEFAULT TRUE,
        ...
        # Прогресс заданий пользователя (новая таблица)
        await conn.execute('''
        ...
        # Таблица для лога победителей розыгрышей (чтобы отслеживать повторные выигрыши)
        await conn.execute('''
            CREATE TABLE IF NOT EXISTS giveaway_winners_log (
                id SERIAL PRIMARY KEY,
        ...
        await conn.execute("CREATE INDEX IF NOT EXISTS idx_giveaway_winners_log_user ON giveaway_winners_log(user_id, win_date)")
...
# ==================== РАБОТА С НАСТРОЙКАМИ (с кешированием в памяти) ====================
...
async def set_setting(key: str, value: str):
    ...
    last_settings_update = 0
    await redis_delete(f"setting:{key}")

# ==================== ФУНКЦИИ ДЛЯ ЧАТОВ И КАНАЛОВ (с кешированием в памяти) ====================
...
async def notify_chats(message_text: str):
    ...
    for chat_id, data in confirmed.items():
        if not data.get('notify_enabled', True):
            continue
        await send_temp_message(chat_id, message_text)
...
async def add_exp(user_id: int, exp: int, conn=None):
    async def _add(conn):
        ...
        # Если уровень достиг 100, обнуляем опыт, чтобы не копился
        if level == 100 and new_exp > level * level_mult:
            new_exp = level * level_mult  # или 0? Лучше оставить максимальным, но не увеличивать
        await conn.execute(
...
# ==================== ФУНКЦИЯ ДЛЯ ПРОВЕРКИ НЕДЕЕСПОСОБНОСТИ (тюрьма/контрабанда) ====================
async def is_user_incapacitated(user_id: int) -> Tuple[bool, Optional[str]]:
    """Проверяет, находится ли пользователь в тюрьме или на контрабанде. Возвращает (True, описание) если недееспособен."""
    async with db_pool.acquire() as conn:
        # Проверяем тюрьму
        jail = await conn.fetchval(
            "SELECT 1 FROM jail_sentences WHERE user_id=$1 AND status='serving' AND end_time > NOW()",
            user_id
        )
        if jail:
            return True, "тюрьме"
        # Проверяем контрабанду
        smuggle = await conn.fetchval(
            "SELECT 1 FROM smuggle_runs WHERE user_id=$1 AND status='in_progress' AND end_time > NOW()",
            user_id
        )
        if smuggle:
            return True, "контрабандном рейсе"
    return False, None
...
async def create_user_business(user_id: int, business_type_id: int):
    async with db_pool.acquire() as conn:
        await conn.execute(
            ...
            user_id, business_type_id, 1, datetime.now(), 0
        )
...
async def spawn_heist(chat_id: int):
    ...
    if file_id:
        ...
        msg = await send_temp_message(chat_id, text)
    # Сохраняем id сообщения, чтобы потом обновлять список участников? Пока нет.
...
async def finish_heist_joining(heist_id: int, join_until: datetime):
    ...
    async with db_pool.acquire() as conn:
        ...
        if not participants:
            ...
            await send_temp_message(heist['chat_id'], "❌ Никто не присоединился к налёту. Он отменён.")
            return
        ...
        await send_temp_message(heist['chat_id'], text)
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            for p in participants:
                ...
                # Обновляем статистику heists_earned
                await conn.execute("UPDATE users SET heists_earned = heists_earned + $1 WHERE user_id=$2", share, user_id)
                if btc_per_user > 0:
                ...
                # Увеличиваем счётчик участий
                await conn.execute("UPDATE users SET heists_joined = heists_joined + 1 WHERE user_id=$1", user_id)
...
async def process_betray(heist_id: int, attacker_id: int, target_id: int) -> Tuple[bool, str, str]:
    """
    Обрабатывает попытку кинуть конкретного подельника во время распила.
    Возвращает (успех, сообщение для чата, сообщение для жертвы)
    """
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            heist = await conn.fetchrow("SELECT * FROM heists WHERE id=$1 AND status='splitting' FOR UPDATE", heist_id)
            if not heist:
                return False, "❌ Налёт не в стадии распила или уже завершён.", ""
            ...
            if not ok:
                return False, f"⏳ Ты сможешь кинуть снова через {format_time_remaining(remaining)}.", ""

            # Получаем информацию о цели и атакующем
            ...
            target = await conn.fetchrow("SELECT * FROM heist_participants WHERE heist_id=$1 AND user_id=$2 FOR UPDATE", heist_id, target_id)
            if not attacker or not target:
                return False, "❌ Участник не найден.", ""

            # Проверяем, не пытается ли атакующий кинуть сам себя
            if attacker_id == target_id:
                return False, "❌ Нельзя кинуть самого себя.", ""

            # Проверяем, не было ли уже успешного кидка между ними в этом налёте
            prev = await conn.fetchval(
                "SELECT 1 FROM heist_betrayals WHERE heist_id=$1 AND ((attacker_id=$2 AND target_id=$3) OR (attacker_id=$3 AND target_id=$2)) AND success=TRUE",
                heist_id, attacker_id, target_id
            )
            if prev:
                return False, "❌ Между вами уже было кидалово в этом налёте.", ""
            ...
            if success:
                ...
                steal_amount = min(steal_amount, target_share)
                ...
                # Ограничиваем штраф текущей долей
                penalty = min(penalty, attacker_share)
            ...
            await conn.execute(
                ...
                heist_id, attacker_id, target_id, success, amount_changed, datetime.now()
            )
            ...
            if success:
                victim_message += f"У тебя украли {amount_changed:.2f}$."
            else:
                victim_message += f"Но он облажался, и ты получил {amount_changed:.2f}$!"

            return success, chat_message, victim_message
...
async def perform_cleanup(manual=False):
    ...
    days_completed_giveaways = await get_setting_int("cleanup_days_completed_giveaways")
    days_jail = 30
    ...
    cutoff_completed_giveaways = now - timedelta(days=days_completed_giveaways)
    ...
    async with db_pool.acquire() as conn:
        ...
        await conn.execute("DELETE FROM giveaways WHERE status='completed' AND end_date < $1", cutoff_completed_giveaways)
...
async def match_orders(conn):
    while True:
        ...
        # Запрещаем самопокупку
        if buy['user_id'] == sell['user_id']:
            # Проверяем, есть ли другие ордера, чтобы избежать бесконечного цикла
            other_buy = await conn.fetchval("SELECT 1 FROM bitcoin_orders WHERE type='buy' AND status='active' AND id != $1", buy['id'])
            other_sell = await conn.fetchval("SELECT 1 FROM bitcoin_orders WHERE type='sell' AND status='active' AND id != $1", sell['id'])
            if not other_buy and not other_sell:
                # Это единственные ордера – выходим
                break
            else:
                continue
        ...
        # Рассчитываем комиссию (если есть)
        ...
        if commission_percent > 0:
            if commission_side == 'buyer':
                commission = total_cost * commission_percent / 100
                total_cost_buyer = total_cost + commission
                total_cost_seller = total_cost
            elif commission_side == 'seller':
                commission = total_cost * commission_percent / 100
                total_cost_buyer = total_cost
                total_cost_seller = total_cost - commission
            elif commission_side == 'both':
                commission = total_cost * commission_percent / 100
                total_cost_buyer = total_cost + commission / 2
                total_cost_seller = total_cost - commission / 2
            else:
                total_cost_buyer = total_cost
                total_cost_seller = total_cost
            # Округляем
            total_cost_buyer = round(total_cost_buyer, 2)
            total_cost_seller = round(total_cost_seller, 2)
        else:
            total_cost_buyer = total_cost
            total_cost_seller = total_cost

        # Выполняем переводы
        await update_user_balance(seller_id, total_cost_seller, conn=conn)
        await update_user_bitcoin(buyer_id, trade_amount, conn=conn)
        if commission_destination == 'burn':
            # Комиссия сгорает, ничего не делаем
            pass
        elif commission_destination == 'balance':
            # Зачисляем на баланс бота (есть ли у бота кошелёк?) – пока пропустим, можно зачислять на отдельный счёт
            pass
        ...
        await safe_send_message(buyer_id, f"✅ Ваш ордер на покупку {trade_amount:.4f} BTC по цене {trade_price} $ исполнен.")
        await safe_send_message(seller_id, f"✅ Ваш ордер на продажу {trade_amount:.4f} BTC по цене {trade_price} $ исполнен.")
...
async def complete_task(user_id: int, task_id: int):
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            await conn.execute(
                "UPDATE users SET completed_tasks = completed_tasks + 1 WHERE user_id=$1",
                user_id
            )
...
async def create_promocode(code: str, reward: float, reward_type: str, max_uses: int, created_by: int = None):
    async with db_pool.acquire() as conn:
        await conn.execute(
            ...
            code, reward, reward_type, max_uses, datetime.now(), created_by
        )
...
# ----- Управление пользователями (унифицировано) -----
class AdminEditUser(StatesGroup):
    user_id = State()
    resource = State()  # balance, reputation, bitcoin, authority, exp, level
    amount = State()
...
# Для покупки/продажи по конкретной цене из стакана
...
# ----- Кидалово в налёте (выбор цели) -----
class HeistBetrayChoose(StatesGroup):
    heist_id = State()
    candidates = State()  # список словарей {user_id, name}
...
def bitcoin_exchange_keyboard():
    return ReplyKeyboardMarkup(keyboard=[
        ...
        [KeyboardButton(text="📋 Мои заявки"), KeyboardButton(text="📊 Стакан покупок"), KeyboardButton(text="📊 Стакан продаж")],
        [KeyboardButton(text="◀️ Назад")]
    ], resize_keyboard=True)

def order_book_keyboard(orders: List[Dict], order_type: str, page: int = 1, total_pages: int = 1):
    """
    Универсальная клавиатура для отображения стакана (покупок или продаж).
    order_type: 'bids' для покупок, 'asks' для продаж.
    """
    kb = []
    for o in orders:
        kb.append([InlineKeyboardButton(
            text=f"💰 {o['price']} $ | {o['total_amount']:.4f} BTC ({o['count']} заявок)",
            callback_data=f"{order_type}_price_{o['price']}"
        )])
    nav = []
    if page > 1:
...
def active_giveaways_keyboard(giveaways: List[dict], page: int, total_pages: int):
    kb = []
    for gw in giveaways:
        # end_date может быть строкой или datetime – преобразуем для отображения
        ...
        kb.append([InlineKeyboardButton(
            text=f"#{gw['id']} | {gw['prize']} | до {end_date_str}",
...
def completed_giveaways_keyboard(giveaways: List[dict], page: int, total_pages: int):
    kb = []
    for gw in giveaways:
        display = f"#{gw['id']} | {gw['prize']} | {gw['winners_list'][:20] if gw['winners_list'] else ''}"
...
def heist_betray_inline_keyboard(heist_id: int):
    """
    Клавиатура для начала процесса кидалова (кнопка в чате).
    """
    return InlineKeyboardMarkup(inline_keyboard=[
...
def betray_choose_target_keyboard(candidates: List[Dict], heist_id: int):
    """
    Клавиатура для выбора цели из трёх случайных участников.
    candidates: список словарей с ключами 'user_id' и 'name'.
    """
    kb = []
    for cand in candidates:
        kb.append([InlineKeyboardButton(
            text=f"🎯 {cand['name']}",
            callback_data=f"betray_target_{heist_id}_{cand['user_id']}"
        )])
    kb.append([InlineKeyboardButton(text="❌ Отмена", callback_data="betray_cancel")])
    return InlineKeyboardMarkup(inline_keyboard=kb)
...
# ----- Пустая кнопка (noop) -----
def noop_button(text: str = "⚡️"):
    """Используется для неинтерактивных элементов в клавиатуре."""
...
def require_active_user(func):
    """Декоратор для проверки бана и подписки в личных сообщениях."""
    @wraps(func)
...
async def universal_back_handler(message: Message, state: FSMContext):
    ...
    elif current_state.startswith('AdminEditUser') or current_state.startswith('FindUser') or \
         current_state.startswith('BlockUser') or current_state.startswith('UnblockUser'):
        await state.finish()
        await admin_users_menu(message)

    elif current_state.startswith('AddShopItem') or current_state.startswith('RemoveShopItem') or \
         current_state.startswith('EditShopItem') or current_state.startswith('RejectPurchase'):
        await state.finish()
        await admin_shop_menu(message)

    elif current_state.startswith('CreateGiveaway'):
        await state.finish()
    ...
    elif current_state.startswith('HeistBetrayChoose'):
        await state.finish()
        await message.answer("❌ Выбор цели отменён.")
...
async def cmd_start(message: Message):
            ...
            if referrer_id != message.from_user.id:
                async with db_pool.acquire() as conn:
                    ...
                    if referrer_exists and not await is_banned(referrer_id):
                        existing = await conn.fetchval("SELECT 1 FROM referrals WHERE referred_id=$1", message.from_user.id)
                        if not existing:
                            await conn.execute(
                                ...
                                referrer_id, message.from_user.id, datetime.now(), False
                            )
                            await conn.execute("UPDATE referrals SET clicks = clicks + 1 WHERE referred_id=$1", message.from_user.id)
    ...
    created, bonus = await ensure_user_exists(message.from_user.id, message.from_user.username, message.from_user.first_name)
    if created:
    ...
    await send_with_media(message.from_user.id, welcome_text, media_key='welcome')
...
async def profile_handler(message: Message):
    ...
    joined_str = joined.strftime("%Y-%m-%d %H:%M:%S") if joined else 'неизвестно'
    ...
    text = (
        ...
        f"📋 Заданий выполнено: {completed_tasks}, провалено: {failed_tasks}\n\n"
...
async def level_handler(message: Message):
    ...
    exp_needed = level * level_mult
    bar = progress_bar(exp, exp_needed, 10)
    next_coins = await get_level_reward_coins(level+1) if level < 100 else 0
    next_rep = await get_level_reward_rep(level+1) if level < 100 else 0
    text = (
        f"📊 <b>Твой уровень</b>\n\n"
        f"Уровень: {level}\n"
    ...
    if level < 100:
        text += f"Следующая награда: +{next_coins:.2f} баксов, +{next_rep} репутации."
    else:
...
async def casino_menu(message: Message):
    # Проверка на тюрьму/контрабанду
    incapacitated, reason = await is_user_incapacitated(message.from_user.id)
    if incapacitated:
        await message.answer(f"❌ Ты сейчас в {reason} и не можешь играть в казино.")
        return
    ...
    if level < min_level:
        ...
        return
    await send_with_media(message.from_user.id, "Выбери игру:", media_key='casino', reply_markup=casino_menu_keyboard())

# ----- Общая функция для обработки ставок (исправленная с транзакцией) -----
async def process_game_bet(user_id: int, game: str, amount: float, 
                           check_win_func, calc_win_func, 
                           exp_win: int, exp_lose: int) -> Tuple[bool, str]:
    """Универсальная функция для игр. Возвращает (win, сообщение для пользователя)."""
    balance = await get_user_balance(user_id)
    if amount > balance:
        return False, "❌ Недостаточно баксов."

    win = check_win_func()
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            await update_user_balance(user_id, -amount, conn=conn)
            await update_user_game_stats(user_id, game, win, conn=conn)
            if win:
                profit, rep = calc_win_func(amount)
                await update_user_balance(user_id, profit, conn=conn)
                if rep > 0:
                    await update_user_reputation(user_id, rep)
                exp = exp_win
                phrase = f"Победа! +{profit:.2f} баксов!"
                if rep:
                    phrase += f" +{rep} репутации"
            else:
                exp = exp_lose
                phrase = f"Проигрыш. -{amount:.2f} баксов."
            await add_exp(user_id, exp, conn=conn)

    await save_last_bet(user_id, game, amount)
...
async def dice_start(message: Message, state: FSMContext):
    # Проверка incapacitated
    incapacitated, reason = await is_user_incapacitated(message.from_user.id)
    if incapacitated:
        await message.answer(f"❌ Ты сейчас в {reason} и не можешь играть.")
        return
    min_level = await get_setting_int("min_level_casino")
    level = await get_user_level(message.from_user.id)
    if level < min_level:
...
async def dice_bet(message: Message, state: FSMContext):
    ...
    def check_win():
        return random.random() * 100 <= win_chance
    ...
    win, phrase = await process_game_bet(user_id, 'dice', amount, check_win, calc_win,
                                         await get_setting_int("exp_per_dice_win"),
                                         await get_setting_int("exp_per_dice_lose"))

    dice1 = random.randint(1, 6)
    dice2 = random.randint(1, 6)
    total = dice1 + dice2
    full_phrase = f"🎲 {dice1} + {dice2} = {total} — {phrase}"
    await message.answer(full_phrase, reply_markup=repeat_bet_keyboard('dice'))
    await state.finish()
...
async def guess_number_callback(callback: CallbackQuery, state: FSMContext):
    ...
    win, phrase = await process_game_bet(user_id, 'guess', amount, check_win, calc_win,
                                         await get_setting_int("exp_per_guess_win"),
                                         await get_setting_int("exp_per_guess_lose"))

    secret = random.randint(1, 5)
    if win:
        full_phrase = f"🔢 Ты угадал! Было {secret}. {phrase}"
    else:
        full_phrase = f"🔢 Не угадал. Было {secret}. {phrase}"

    bet_data = {'number': guess}
    await save_last_bet(user_id, 'guess', amount, bet_data)

    await callback.message.edit_text(full_phrase, reply_markup=repeat_bet_keyboard('guess'))
    await state.finish()
    await callback.answer()
...
async def shop_page_callback(callback: CallbackQuery):
    ...
    callback.message.text = f"🛒 Магазин {page}"
    ...
    try:
        ...
        async with db_pool.acquire() as conn:
            ...
            if balance < price:
                ...
                return
            async with conn.transaction():
                await update_user_balance(callback.from_user.id, -price, conn=conn)
                await update_user_total_spent(callback.from_user.id, price)
                await conn.execute(
                    "INSERT INTO purchases (user_id, item_id, purchase_date) VALUES ($1, $2, $3)",
                    callback.from_user.id, item_id, datetime.now()
                )
                if stock != -1:
        ...
        await send_with_media(callback.from_user.id, f"✅ Покупка совершена! {phrase}", media_key='purchase')
        await callback.message.delete()
        await callback.answer()
...
async def my_purchases(message: Message):
    ...
    for row in rows:
        ...
        date_str = date.strftime("%Y-%m-%d %H:%M") if date else "неизвестно"
        text += f"{status_emoji} {name} от {date_str}\n"
        if comment:
...
async def promo_handler(message: Message, state: FSMContext):
    await send_with_media(message.from_user.id, "Введи промокод:", media_key='promo', reply_markup=back_keyboard())
...
async def theft_menu(message: Message):
    # Проверка incapacitated для грабителя
    incapacitated, reason = await is_user_incapacitated(message.from_user.id)
    if incapacitated:
        await message.answer(f"❌ Ты сейчас в {reason} и не можешь грабить.")
        return
    phrase = "🔫 Выбери цель:"
    await send_with_media(message.from_user.id, phrase, media_key='theft', reply_markup=theft_choice_keyboard())
...
async def theft_random(message: Message, state: FSMContext):
    user_id = message.from_user.id
    # Проверка incapacitated
    incapacitated, reason = await is_user_incapacitated(user_id)
    if incapacitated:
        await message.answer(f"❌ Ты сейчас в {reason} и не можешь грабить.")
        return
    cooldown_minutes = await get_setting_int("theft_cooldown_minutes")
    async with db_pool.acquire() as conn:
        last_time = await conn.fetchval("SELECT last_theft_time FROM users WHERE user_id=$1", user_id)
        if last_time:
    ...
    if not target_id:
        ...
        return
    # Проверка incapacitated цели
    incapacitated_target, reason_target = await is_user_incapacitated(target_id)
    if incapacitated_target:
        await message.answer(f"❌ Цель сейчас в {reason_target} и её нельзя ограбить.")
        return
...
async def tasks_user_menu(message: Message):
    # Проверка incapacitated – в тюрьме задания выполнять нельзя
    incapacitated, reason = await is_user_incapacitated(message.from_user.id)
    if incapacitated:
        await message.answer(f"❌ Ты сейчас в {reason} и не можешь выполнять задания.")
        return
    user_id = message.from_user.id
    async with db_pool.acquire() as conn:
        # Получаем активные задания
        rows = await conn.fetch("SELECT id, name, description, reward_coins, reward_reputation, max_completions, completed_count, media_file_id, media_type, task_type, target_id FROM tasks WHERE active=TRUE")
        # Получаем прогресс пользователя по заданиям
        user_progress = {}
        if rows:
            task_ids = [r['id'] for r in rows]
            progress_rows = await conn.fetch("SELECT task_id, status, progress, target FROM user_tasks WHERE user_id=$1 AND task_id = ANY($2::int[])", user_id, task_ids)
            for pr in progress_rows:
                user_progress[pr['task_id']] = dict(pr)
    ...
    for row in rows:
        task_id = row['id']
        progress_info = user_progress.get(task_id, {})
        status = progress_info.get('status')
        if status == 'completed':
            continue  # не показываем уже выполненные (если задание одноразовое)
        # Для заданий счётчика показываем прогресс
        progress_str = ""
        if row['task_type'] in ('theft_count', 'smuggle_count', 'casino_wins'):
            current = progress_info.get('progress', 0)
            target = progress_info.get('target', int(row['target_id']))
            progress_str = f" [{current}/{target}]"
        elif row['task_type'] == 'subscribe':
            # Проверим, подписан ли уже
            if await check_user_subscription(user_id, row['target_id']):
                # Если подписан, но задание ещё не выполнено, предлагаем выполнить
                pass
            else:
                # Не подписан, но задание активное – показываем кнопку для подписки
                pass

        text += f"🔹 {row['name']}{progress_str}\n{row['description']}\nНаграда: {float(row['reward_coins']):.2f} баксов, {row['reward_reputation']} репутации\n\n"
...
async def take_task(callback: CallbackQuery):
    # Проверка incapacitated
    incapacitated, reason = await is_user_incapacitated(callback.from_user.id)
    if incapacitated:
        await callback.answer(f"❌ Ты сейчас в {reason} и не можешь выполнять задания.", show_alert=True)
        return
    ...
    async with db_pool.acquire() as conn:
        ...
        if task['task_type'] == 'subscribe':
            ...
            if not subscribed:
                # Отправляем клавиатуру с ссылкой на канал
                await callback.message.answer(
                    f"❌ Ты не подписан на этот канал!\n"
                    f"Подпишись и нажми кнопку ниже для проверки.",
                    reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                        [InlineKeyboardButton(text="📢 Перейти к каналу", url=f"https://t.me/{channel_id.lstrip('@')}")],
                        [InlineKeyboardButton(text="✅ Проверить подписку", callback_data=f"check_sub_task_{task_id}")]
                    ])
                )
                await callback.answer()
                return
            ...
            if success:
                await callback.message.delete()
        elif task['task_type'] in ('theft_count', 'smuggle_count', 'casino_wins'):
            # Создаём запись в user_tasks со статусом in_progress
            target = int(task['target_id'])
            await conn.execute(
                "INSERT INTO user_tasks (user_id, task_id, status, progress, target) VALUES ($1, $2, 'in_progress', 0, $3)",
                user_id, task_id, target
            )
            await callback.answer("✅ Задание активировано! Прогресс будет отслеживаться автоматически.", show_alert=True)
        else:
            await callback.answer("❌ Этот тип заданий пока не поддерживается.", show_alert=True)
    await callback.answer()

@dp.callback_query_handler(lambda c: c.data.startswith("check_sub_task_"))
@require_active_user
async def check_sub_task(callback: CallbackQuery):
    ...
    async with db_pool.acquire() as conn:
        ...
        if not task:
            ...
            return
        if task['task_type'] != 'subscribe':
            await callback.answer("❌ Неверный тип задания.", show_alert=True)
            return
        subscribed = await check_user_subscription(user_id, task['target_id'])
        if not subscribed:
            await callback.answer("❌ Всё ещё не подписан.", show_alert=True)
            return
        success, msg = await complete_task(user_id, task_id)
        await callback.answer(msg, show_alert=True)
        if success:
...
async def my_businesses(message: Message):
    ...
    incapacitated, reason = await is_user_incapacitated(message.from_user.id)
    if incapacitated:
        await message.answer(f"❌ Ты сейчас в {reason} и не можешь управлять бизнесами.")
        return
    user_id = message.from_user.id
...
async def buy_business_menu(callback: CallbackQuery):
    ...
    incapacitated, reason = await is_user_incapacitated(callback.from_user.id)
    if incapacitated:
        await callback.answer(f"❌ Ты сейчас в {reason} и не можешь покупать бизнесы.", show_alert=True)
        return
    user_id = callback.from_user.id
...
async def bitcoin_exchange_menu(message: Message):
    ...
    incapacitated, reason = await is_user_incapacitated(message.from_user.id)
    if incapacitated:
        await message.answer(f"❌ Ты сейчас в {reason} и не можешь пользоваться биржей.")
        return
    await send_with_media(message.from_user.id, "💼 Биткоин-биржа: продавай и покупай BTC за баксы.", media_key='exchange', reply_markup=bitcoin_exchange_keyboard())

@dp.message_handler(lambda message: message.text == "📊 Стакан покупок")
@require_active_user
async def exchange_bid_book(message: Message):
    book = await get_order_book()
    await show_order_book(message, book['bids'], "bids", "📈 Стакан покупок (BID)")

@dp.message_handler(lambda message: message.text == "📊 Стакан продаж")
@require_active_user
async def exchange_ask_book(message: Message):
    book = await get_order_book()
    await show_order_book(message, book['asks'], "asks", "📉 Стакан продаж (ASK)")

async def show_order_book(message: Message, orders: List[Dict], order_type: str, title: str, page: int = 1):
    """Отображает стакан с пагинацией."""
    if not orders:
        await message.answer(f"{title}\n\nНет активных заявок.", reply_markup=bitcoin_exchange_keyboard())
        return

    # Пагинация по страницам (ITEMS_PER_PAGE)
    ...
    kb = order_book_keyboard(page_orders, order_type, page, total_pages)
    await message.answer(text, reply_markup=kb)

@dp.callback_query_handler(lambda c: c.data.startswith("bids_price_") or c.data.startswith("asks_price_"))
@require_active_user
async def price_action_callback(callback: CallbackQuery, state: FSMContext):
    parts = callback.data.split("_")
    order_type = parts[0]  # bids или asks
    price = int(parts[2])

    user_id = callback.from_user.id
    if order_type == 'asks':
        # Это предложения продажи, пользователь хочет купить
        async with db_pool.acquire() as conn:
            orders = await conn.fetch(
                "SELECT * FROM bitcoin_orders WHERE type='sell' AND status='active' AND price=$1 ORDER BY created_at ASC",
                price
            )
        # Исключаем собственные ордера
    ...
    else:  # bids – покупка, пользователь хочет продать
        async with db_pool.acquire() as conn:
            orders = await conn.fetch(
...
@dp.callback_query_handler(lambda c: c.data.startswith("bids_page_") or c.data.startswith("asks_page_"))
@require_active_user
async def order_book_page_callback(callback: CallbackQuery):
    parts = callback.data.split("_")
    order_type = parts[0]  # bids или asks
    page = int(parts[2])
    book = await get_order_book()
    orders = book['bids'] if order_type == 'bids' else book['asks']
    title = "📈 Стакан покупок (BID)" if order_type == 'bids' else "📉 Стакан продаж (ASK)"
    await show_order_book(callback.message, orders, order_type, title, page)
    await callback.answer()
...
async def my_order_detail(callback: CallbackQuery):
    ...
    created_at_str = created_at.strftime("%Y-%m-%d %H:%M") if created_at else "неизвестно"
    text = (
        f"📄 Заявка #{order['id']}\n"
...
async def active_giveaway_detail(callback: CallbackQuery):
    ...
    if gw['max_participants'] > 0:
        text += f"📊 Лимит участников: {gw['max_participants']}\n"
...
async def join_giveaway(callback: CallbackQuery):
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            gw = await conn.fetchrow("SELECT * FROM giveaways WHERE id=$1 AND status='active' FOR UPDATE", gw_id)
            if not gw:
                await callback.answer("Розыгрыш уже завершён.", show_alert=True)
                return
            # Проверяем лимит участников
            if gw['max_participants'] > 0:
                current = await conn.fetchval("SELECT COUNT(*) FROM participants WHERE giveaway_id=$1", gw_id)
                if current >= gw['max_participants']:
...
async def betray_attempt_callback(callback: CallbackQuery, state: FSMContext):
    ...
    # Получаем список других участников (кроме себя)
    async with db_pool.acquire() as conn:
        participants = await conn.fetch("""
            SELECT hp.user_id, u.first_name
    ...
    if len(participants) < 1:
        await callback.answer("❌ Нет других участников для кидалова.", show_alert=True)
        return

    # Выбираем до трёх случайных
    candidates = random.sample(participants, min(3, len(participants)))
    cand_list = [{'user_id': c['user_id'], 'name': c['first_name']} for c in candidates]

    await state.update_data(heist_id=heist_id, candidates=cand_list)
    kb = betray_choose_target_keyboard(cand_list, heist_id)
    await callback.message.answer("🔪 Выбери, кого хочешь кинуть:", reply_markup=kb)
    await HeistBetrayChoose.set()
    await callback.answer()

@dp.callback_query_handler(lambda c: c.data.startswith("betray_target_"), state=HeistBetrayChoose)
@require_active_user
async def betray_target_chosen(callback: CallbackQuery, state: FSMContext):
    parts = callback.data.split("_")
    ...
    data = await state.get_data()
    if data.get('heist_id') != heist_id:
        await callback.answer("❌ Данные устарели, начните заново.", show_alert=True)
        await state.finish()
        return

    # Выполняем попытку кидалова
    success, chat_message, victim_message = await process_betray(heist_id, attacker_id, target_id)
    ...
    async with db_pool.acquire() as conn:
        ...
        if heist:
            ...
            if chat_message:
                await send_temp_message(chat_id, chat_message)

    # Если попытка была успешной и есть жертва, отправляем ей личное сообщение
    if success and victim_message:
...
@dp.callback_query_handler(lambda c: c.data == "betray_cancel", state=HeistBetrayChoose)
@require_active_user
async def betray_cancel(callback: CallbackQuery, state: FSMContext):
    await state.finish()
    await callback.answer("❌ Выбор цели отменён.")
    await callback.message.delete()
...
async def add_active_chat_user(chat_id: int, user_id: int, ttl: int = 3600):
    """Добавляет пользователя в множество активных участников чата в Redis."""
    key = f"chat_active_users:{chat_id}"
    await redis_client.sadd(key, str(user_id))
    await redis_client.expire(key, ttl)

async def get_random_active_chat_user(chat_id: int, exclude_id: int) -> Optional[int]:
    """Возвращает случайного активного участника чата (исключая exclude_id) или None."""
    key = f"chat_active_users:{chat_id}"
    members = await redis_client.smembers(key)
    if not members:
        return None
    # Исключаем exclude_id и ботов (ботов там быть не должно, но на всякий случай проверим)
    candidates = [int(uid) for uid in members if int(uid) != exclude_id]
    if not candidates:
        return None
...
async def heist_keyword_handler(message: Message):
    ...
    incapacitated, reason = await is_user_incapacitated(user_id)
    if incapacitated:
        await auto_delete_command(message, f"❌ Ты сейчас в {reason} и не можешь участвовать в налётах.")
        return
    ...
    async with db_pool.acquire() as conn:
        ...
        # Добавляем пользователя в активные участники чата (для подгона) – после успешного участия
        await add_active_chat_user(chat_id, user_id)
        ...
        if heist['btc_pot'] > 0:
            await auto_delete_reply(message, f"₿ В банке есть BTC! Твоя доля BTC будет распределена после распила.")
...
async def cmd_smuggle(message: Message):
    ...
    incapacitated, reason = await is_user_incapacitated(user_id)
    if incapacitated:
        await auto_delete_command(message, f"❌ Ты сейчас в {reason} и не можешь отправиться в контрабанду.")
        return
    ...
    # Проверка, не выполняется ли уже рейс (защита от дублирования)
    async with db_pool.acquire() as conn:
        active = await conn.fetchval(
            "SELECT id FROM smuggle_runs WHERE user_id=$1 AND status='in_progress'",
            user_id
        )
        if active:
    ...
    await add_active_chat_user(message.chat.id, user_id)
    ...
    if file_id:
        ...
        await send_temp_message(message.chat.id, phrase)

    await auto_delete_command(message)  # удаляем команду
...
async def cmd_jail(message: Message):
    ...
    incapacitated, reason = await is_user_incapacitated(user_id)
    if incapacitated:
        await auto_delete_command(message, f"❌ Ты сейчас в {reason} и не можешь отправиться в тюрьму.")
        return
    ...
    # Проверка, не отбывает ли уже срок
    async with db_pool.acquire() as conn:
        active = await conn.fetchval(
            "SELECT id FROM jail_sentences WHERE user_id=$1 AND status='serving'",
            user_id
        )
        if active:
            await auto_delete_command(message, "❌ Ты уже отбываешь срок! Не пытайся снова.")
            return
    ...
    await auto_delete_command(message)
...
async def cmd_chat_top(message: Message):
    ...
    await auto_delete_command(message)  # удаляем команду, ответ останется
...
async def cmd_chat_heist_status(message: Message):
    ...
    async with db_pool.acquire() as conn:
        ...
        participants = await conn.fetch("SELECT u.first_name FROM heist_participants hp JOIN users u ON hp.user_id = u.user_id WHERE hp.heist_id=$1 LIMIT 10", heist['id'])
        participants_list = "\n".join([f"• {p['first_name']}" for p in participants])
        if count > 10:
            participants_list += f"\n... и ещё {count - 10}"

        status_emoji = "🟡" if heist['status'] == 'joining' else "🔴"
        if heist['status'] == 'joining':
            time_remaining = (heist['join_until'] - datetime.now()).total_seconds()
            time_str = format_time_remaining(int(time_remaining)) if time_remaining > 0 else "завершается"
        else:
            time_remaining = (heist['split_until'] - datetime.now()).total_seconds()
            time_str = format_time_remaining(int(time_remaining)) if time_remaining > 0 else "завершается"
...
async def cmd_my_heist(message: Message):
    ...
    async with db_pool.acquire() as conn:
        ...
        if not row:
            # Ничего не выводим в чат, чтобы не засорять
            return

        # Отправляем информацию в личку
        text = (
            f"🔫 Твой текущий налёт ({row['event_type']}):\n"
            f"Статус: {'сбор' if row['status'] == 'joining' else 'распил'}\n"
            f"Твоя текущая доля: {float(row['current_share']):.2f} $"
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            chat_info = await conn.fetchrow(
                "SELECT * FROM confirmed_chats WHERE chat_id=$1 FOR UPDATE",
                message.chat.id
            )
            if not chat_info:
                return
            ...
            gift_count_today = chat_info['gift_count_today'] if last_gift_date and last_gift_date == today else 0

            if gift_count_today >= gift_limit_per_chat:
                await auto_delete_reply(
                    message,
                    f"❌ Сегодня в этом чате уже использовано {gift_count_today} из {gift_limit_per_chat} подгонов."
                )
                return

            user = await conn.fetchrow(
                "SELECT last_gift_time, gift_count_today FROM users WHERE user_id=$1 FOR UPDATE",
                user_id
            )
            if not user:
                user = {'last_gift_time': None, 'gift_count_today': 0}

            user_gift_count = user['gift_count_today'] if user['last_gift_time'] and user['last_gift_time'].date() == today else 0

            if user_gift_count >= gift_global_limit:
                await auto_delete_reply(
                    message,
                    f"❌ Сегодня ты уже получил {user_gift_count} из {gift_global_limit} подгонов во всех чатах."
                )
                return

            if user['last_gift_time']:
                diff = (now - user['last_gift_time']).total_seconds() / 60
                if diff < gift_cooldown:
                    remaining = int(gift_cooldown - diff)
                    await auto_delete_reply(
                        message,
                        f"⏳ Подгон можно будет использовать через {remaining} мин."
                    )
                    return

            # Выбираем случайного участника из активных в Redis
            recipient_id = await get_random_active_chat_user(message.chat.id, user_id)
            if not recipient_id:
                # Если Redis пуст, пробуем получить через get_chat_members (ограничено)
                try:
                    members = []
                    async for member in bot.get_chat_members(chat_id=message.chat.id):
                        if not member.user.is_bot and member.user.id != user_id and not await is_banned(member.user.id):
                            members.append(member.user)
                            # Добавляем в Redis для будущих раздач
                            await add_active_chat_user(message.chat.id, member.user.id)
                    if not members:
                        await auto_delete_reply(message, "❌ Нет подходящих получателей для подарка.")
                        return
                    recipient = random.choice(members)
                except Exception as e:
                    logging.error(f"Gift error (fallback): {e}")
                    await auto_delete_reply(message, "❌ Не удалось выбрать получателя.")
                    return
            else:
                # Получаем объект пользователя из БД (нам нужно только имя)
                recipient_data = await conn.fetchrow("SELECT user_id, first_name FROM users WHERE user_id=$1", recipient_id)
                if not recipient_data:
                    # Пользователь есть в Redis, но не в БД? Такое может быть, если бот не знает о нём. Создаём минимальную запись.
                    await conn.execute(
                        "INSERT INTO users (user_id, first_name, joined_date) VALUES ($1, $2, $3) ON CONFLICT (user_id) DO NOTHING",
                        recipient_id, f"ID{recipient_id}", datetime.now()
                    )
                    recipient = types.User(id=recipient_id, first_name=f"ID{recipient_id}", is_bot=False)
                else:
                    recipient = types.User(id=recipient_data['user_id'], first_name=recipient_data['first_name'], is_bot=False)

            # Убеждаемся, что получатель есть в БД (уже проверили или создали)
            await update_user_balance(recipient.id, gift_amount, conn=conn)

            # Обновляем статистику чата
            if last_gift_date and last_gift_date == today:
                await conn.execute(
                    "UPDATE confirmed_chats SET gift_count_today = gift_count_today + 1 WHERE chat_id=$1",
                    message.chat.id
                )
            else:
                await conn.execute(
                    "UPDATE confirmed_chats SET last_gift_date=$1, gift_count_today=1 WHERE chat_id=$2",
...
SETTINGS_CATEGORIES = {
    "⚙️ Казино": [
        ("casino_win_chance", "Шанс выигрыша (%)"),
        ("casino_min_bet", "Мин. ставка"),
        ("casino_max_bet", "Макс. ставка"),
        ("min_level_casino", "Мин. уровень"),
        ("slots_win_probability", "Шанс выигрыша в слотах (%)"),
        ("slots_multiplier_three", "Множитель трёх одинаковых"),
        ("slots_multiplier_diamond", "Множитель 💎"),
        ("slots_multiplier_seven", "Множитель 7️⃣"),
        ("roulette_number_multiplier", "Множитель числа"),
        ("roulette_green_multiplier", "Множитель зелёного"),
        ("roulette_color_multiplier", "Множитель цвета"),
    ...
    "⚙️ Кража": [
        ...
        ("theft_success_chance", "Базовый шанс успеха (%)"),
        ("theft_defense_chance", "Базовый шанс защиты (%)"),
        ("theft_defense_penalty", "Штраф при защите"),
        ("min_theft_amount", "Мин. сумма кражи"),
        ("max_theft_amount", "Макс. сумма кражи"),
    ],
    "⚙️ Кидалово (PVP)": [
        ("betray_base_chance", "Базовый шанс кидка (%)"),
        ("betray_steal_percent", "Процент кражи при успехе"),
        ("betray_fail_penalty_percent", "Штраф при провале (%)"),
        ("betray_cooldown_minutes", "Кулдаун кидка (мин)"),
        ("betray_max_chance", "Макс. шанс кидка (%)"),
    ],
    "⚙️ Налёты": [
        ("heist_min_interval_minutes", "Мин. интервал между налётами"),
        ("heist_max_interval_minutes", "Макс. интервал"),
        ("heist_join_minutes", "Время на сбор (мин)"),
        ("heist_split_minutes", "Время на распил (мин)"),
        ("heist_min_pot", "Мин. банк"),
        ("heist_max_pot", "Макс. банк"),
        ("heist_btc_chance", "Шанс BTC в налёте (%)"),
        ("heist_min_btc", "Мин. BTC"),
        ("heist_max_btc", "Макс. BTC"),
        ("heist_cooldown_minutes", "Кулдаун налёта в чате"),
        ("heist_participant_cooldown_hours", "Кулдаун участия (ч)"),
        ("heist_share_min", "Мин. доля"),
        ("heist_share_max", "Макс. доля"),
    ],
    "⚙️ Бизнесы": [
        ("business_upgrade_cost_per_level", "Стоимость апгрейда за уровень (BTC)"),
        ("business_collect_interval_minutes", "Интервал сбора (мин)"),
        ("business_max_storage_hours", "Макс. хранение (ч)"),
        ("business_max_businesses", "Макс. бизнесов у игрока"),
    ],
    "⚙️ Опыт и уровни": [
        ("level_multiplier", "Множитель опыта для уровня"),
        ("level_reward_coins", "Базовая награда баксами"),
        ("level_reward_reputation", "Базовая награда репутацией"),
        ...
        ("exp_per_slots_win", "Опыт за победу в слоты"),
        ("exp_per_slots_lose", "Опыт за проигрыш"),
        ("exp_per_roulette_win", "Опыт за победу в рулетку"),
        ...
        ("exp_per_betray_success", "Опыт за успешный кидок"),
        ("exp_per_betray_fail", "Опыт за провал кидка"),
        ("exp_per_smuggle", "Опыт за контрабанду"),
        ("exp_per_jail", "Опыт за тюрьму"),
    ],
    "⚙️ Рефералы": [
        ("referral_bonus", "Бонус рефереру (баксы)"),
        ("referral_reputation", "Бонус рефереру (репутация)"),
        ("referral_required_thefts", "Требуемое число краж"),
    ],
    "⚙️ Подгон": [
    ...
    "⚙️ Биткоин-биржа": [
        ("exchange_min_price", "Мин. цена за BTC"),
        ("exchange_max_price", "Макс. цена"),
        ("exchange_commission_percent", "Комиссия (%)"),
        ...
        ("exchange_min_amount_btc", "Мин. сумма заявки (BTC)"),
    ],
    "⚙️ Автоудаление": [
        ("auto_delete_commands_seconds", "Автоудаление команд (сек)"),
        ("auto_delete_bot_messages_seconds", "Автоудаление сообщений бота (сек)"),
    ],
    "⚙️ Прокачка навыков": [
        ("skill_share_cost_per_level", "Стоимость прокачки доли (авторитет)"),
        ("skill_luck_cost_per_level", "Стоимость прокачки удачи"),
        ("skill_betray_cost_per_level", "Стоимость прокачки кидалова"),
        ...
        ("skill_max_level", "Макс. уровень навыка"),
    ],
    "⚙️ Контрабанда": [
        ("smuggle_base_amount", "Базовая сумма BTC"),
        ("smuggle_cooldown_minutes", "Кулдаун (мин)"),
        ("smuggle_fail_penalty_minutes", "Штраф при провале (мин)"),
        ("smuggle_success_chance", "Шанс успеха (%)"),
        ("smuggle_caught_chance", "Шанс быть пойманным"),
        ("smuggle_lost_chance", "Шанс потерять груз"),
        ("smuggle_min_duration", "Мин. длительность (мин)"),
        ("smuggle_max_duration", "Макс. длительность"),
    ],
    "⚙️ Тюрьма": [
        ("jail_min_duration", "Мин. срок (мин)"),
        ("jail_max_duration", "Макс. срок"),
        ("jail_success_chance", "Шанс получить авторитет"),
        ("jail_auth_min", "Мин. авторитет при успехе"),
        ("jail_auth_max", "Макс. авторитет"),
        ("jail_cooldown_hours", "Кулдаун тюрьмы (ч)"),
    ],
    "⚙️ Задания": [
        ("task_subscribe_check_interval", "Интервал проверки подписки (сек)"),
        ("task_unsubscribe_penalty_coins", "Штраф за отписку (баксы)"),
        ("task_unsubscribe_penalty_reputation", "Штраф за отписку (репутация)"),
    ],
    "⚙️ Промокоды": [
...
# ----- Унифицированный обработчик для изменения ресурсов пользователя -----
@dp.message_handler(lambda message: message.text in [
    "💰 Начислить баксы", "💸 Списать баксы",
    "⭐️ Начислить репутацию", "🔻 Снять репутацию",
    "📈 Начислить опыт", "🔝 Установить уровень",
    "₿ Начислить биткоины", "₿ Списать биткоины",
    "⚔️ Начислить авторитет", "⚔️ Списать авторитет"
])
async def admin_edit_user_start(message: Message, state: FSMContext):
    if not await check_admin_permissions(message.from_user.id, "manage_users"):
        await message.answer("❌ Недостаточно прав.")
        return

    # Определяем ресурс и операцию по тексту кнопки
    text = message.text
    resource_map = {
        "💰 Начислить баксы": ("balance", "add"),
        "💸 Списать баксы": ("balance", "remove"),
        "⭐️ Начислить репутацию": ("reputation", "add"),
        "🔻 Снять репутацию": ("reputation", "remove"),
        "📈 Начислить опыт": ("exp", "add"),
        "🔝 Установить уровень": ("level", "set"),
        "₿ Начислить биткоины": ("bitcoin", "add"),
        "₿ Списать биткоины": ("bitcoin", "remove"),
        "⚔️ Начислить авторитет": ("authority", "add"),
        "⚔️ Списать авторитет": ("authority", "remove"),
    }
    resource, operation = resource_map.get(text, (None, None))
    if not resource:
        await message.answer("❌ Неизвестная операция.")
        return

    await state.update_data(resource=resource, operation=operation)
    await message.answer("Введи ID или @username пользователя:", reply_markup=back_keyboard())
    await AdminEditUser.user_id.set()

@dp.message_handler(state=AdminEditUser.user_id)
async def admin_edit_user_id(message: Message, state: FSMContext):
    if message.text == "◀️ Назад":
        await state.finish()
        await admin_users_menu(message)
        return
    ...
    if resource == "level" and operation == "set":
        await message.answer("Введи новый уровень (целое число ≥ 1):")
        await AdminEditUser.amount.set()
    elif resource == "exp" and operation == "add":
        await message.answer("Введи количество опыта для начисления (целое число):")
        await AdminEditUser.amount.set()
    elif resource in ["balance", "bitcoin"]:
        if operation == "add":
            await message.answer("Введи сумму начисления (можно дробную):")
        else:
            await message.answer("Введи сумму списания (можно дробную):")
        await AdminEditUser.amount.set()
    elif resource in ["reputation", "authority"]:
        if operation == "add":
            await message.answer("Введи количество для начисления (целое число):")
        else:
            await message.answer("Введи количество для снятия (целое число):")
        await AdminEditUser.amount.set()
    else:
        await message.answer("❌ Неподдерживаемая операция.")
        await state.finish()

@dp.message_handler(state=AdminEditUser.amount)
async def admin_edit_user_amount(message: Message, state: FSMContext):
    if message.text == "◀️ Назад":
        await state.finish()
        await admin_users_menu(message)
        return
    ...
    try:
        if resource == "level":
            ...
            if amount < 1:
                raise ValueError
            async with db_pool.acquire() as conn:
                await conn.execute("UPDATE users SET level=$1 WHERE user_id=$2", amount, uid)
            await message.answer(f"✅ Пользователю {uid} установлен уровень {amount}.")
            await safe_send_message(uid, f"🔝 Ваш уровень изменён на {amount} администратором.")
            ...
            await update_user_balance(uid, delta)
            verb = "начислено" if operation == "add" else "списано"
            await message.answer(f"✅ Пользователю {uid} {verb} {amount:.2f} баксов.")
            await safe_send_message(uid, f"💰 Вам {verb} {amount:.2f} баксов администратором.")

        elif resource == "bitcoin":
            amount = float(message.text)
            if amount <= 0:
                raise ValueError
            amount = round(amount, 4)
            ...
            if amount > max_input:
                ...
                return
            delta = amount if operation == "add" else -amount
            await update_user_bitcoin(uid, delta)
            verb = "начислено" if operation == "add" else "списано"
            await message.answer(f"✅ Пользователю {uid} {verb} {amount:.4f} BTC.")
            await safe_send_message(uid, f"₿ Вам {verb} {amount:.4f} BTC администратором.")
            ...
            if amount < 0:
                raise ValueError
            delta = amount if operation == "add" else -amount
            await update_user_reputation(uid, delta)
            verb = "начислено" if operation == "add" else "снято"
            await message.answer(f"✅ Пользователю {uid} {verb} {amount} репутации.")
            await safe_send_message(uid, f"⭐️ Вам {verb} {amount} репутации администратором.")
            ...
            if amount < 0:
                raise ValueError
            delta = amount if operation == "add" else -amount
            await update_user_authority(uid, delta)
            verb = "начислено" if operation == "add" else "снято"
            await message.answer(f"✅ Пользователю {uid} {verb} {amount} авторитета.")
            await safe_send_message(uid, f"⚔️ Вам {verb} {amount} авторитета администратором.")

    except ValueError:
        await message.answer("❌ Введи корректное число.")
        return
    except Exception as e:
        logging.error(f"Admin edit user error: {e}")
    ...
    try:
        async with db_pool.acquire() as conn:
            await conn.execute(
                ...
                uid, message.from_user.id, datetime.now(), reason
            )
...
async def edit_shop_item_value(message: Message, state: FSMContext):
    ...
    # Безопасное обновление с проверкой поля
    async with db_pool.acquire() as conn:
        if field == 'price':
...
async def admin_purchases(message: Message):
    ...
    try:
        ...
        for row in rows:
            pid, uid, username, item_name, date, status = row['id'], row['user_id'], row['username'] or "нет username", row['name'], row['purchase_date'], row['status']
            date_str = date.strftime("%Y-%m-%d %H:%M") if date else "неизвестно"
        ...
        if nav_buttons:
            kb.append(nav_buttons)
        # Кнопки действий для каждой покупки можно добавить, но для краткости выводим только текст
...
@dp.callback_query_handler(lambda c: c.data.startswith("admin_purchases_page_"))
async def admin_purchases_page_callback(callback: CallbackQuery):
    page = int(callback.data.split("_")[3])
    callback.message.text = f"🛍️ Список покупок {page}"
    await admin_purchases(callback.message)
    await callback.answer()
...
async def purchase_reject(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("Введи причину отказа:", reply_markup=back_keyboard())
    await RejectPurchase.reason.set()
    await callback.answer()

@dp.message_handler(state=RejectPurchase.reason)
async def purchase_reject_reason(message: Message, state: FSMContext):
    if message.text == "◀️ Назад":
        await state.finish()
        await admin_shop_menu(message)
        return
    ...
    try:
        async with db_pool.acquire() as conn:
            await conn.execute("UPDATE purchases SET status='rejected', admin_comment=$1 WHERE id=$2", reason, purchase_id)
            user_id = await conn.fetchval("SELECT user_id FROM purchases WHERE id=$1", purchase_id)
            if user_id:
                await safe_send_message(user_id, f"❌ К сожалению, твоя покупка не может быть выполнена.\nПричина: {reason}")
        await message.answer("✅ Покупка отклонена, пользователь уведомлён.")
    except Exception as e:
...
async def create_promo_max_uses(message: Message, state: FSMContext):
    ...
    try:
        async with db_pool.acquire() as conn:
            await conn.execute(
                "INSERT INTO promocodes (code, reward, reward_type, max_uses, created_at, created_by) VALUES ($1, $2, $3, $4, $5, $6)",
                data['code'], data['reward'], data['reward_type'], max_uses, datetime.now(), message.from_user.id
            )
...
async def list_pending_requests(message: Message):
    ...
    for req in requests:
        ...
        date_str = request_date.strftime("%Y-%m-%d %H:%M:%S") if request_date else "неизвестно"
        text += f"• {req['title']} (ID: {req['chat_id']})\n  Запросил: {req['requested_by']} ({date_str})\n"
    parts = safe_split_text(text)
    for part in parts:
...
async def list_confirmed_chats(message: Message):
    ...
    for chat_id, data in confirmed.items():
        ...
        date_str = confirmed_date.strftime("%Y-%m-%d %H:%M:%S") if confirmed_date else "неизвестно"
        text += f"• {data['title']} (ID: {chat_id})\n  Подтверждён: {date_str}\n"
    parts = safe_split_text(text)
    for part in parts:
        await message.answer(part)
...
async def edit_business_value(message: Message, state: FSMContext):
    ...
    # Безопасное обновление через словарь соответствия полей
    column_map = {
        'name': 'name',
        'emoji': 'emoji',
...
async def admin_trade_history(message: Message):
    ...
    for r in rows:
        ...
        date_str = traded_at.strftime("%Y-%m-%d %H:%M") if traded_at else "неизвестно"
        text += f"ID {r['id']}: {float(r['amount']):.4f} BTC @ {r['price']} $ (покупатель {r['buyer_id']}, продавец {r['seller_id']}) в {date_str}\n"
    parts = safe_split_text(text)
    for part in parts:
        await message.answer(part, reply_markup=admin_exchange_keyboard())
...
async def create_task_description(message: Message, state: FSMContext):
    if message.text == "◀️ Назад":
        ...
        return
    await state.update_data(description=message.text)
    await message.answer("Введи тип задания (subscribe / theft_count / smuggle_count / casino_wins):")
...
async def create_task_type(message: Message, state: FSMContext):
    ...
    allowed_types = ['subscribe', 'theft_count', 'smuggle_count', 'casino_wins']
    if task_type not in allowed_types:
        await message.answer(f"❌ Поддерживаются только: {', '.join(allowed_types)}")
        return
    await state.update_data(task_type=task_type)
    if task_type == 'subscribe':
        await message.answer("Введи ID канала или @username для подписки:")
    else:
        await message.answer("Введи целевое количество (целое число):")
...
async def add_admin_user(message: Message, state: FSMContext):
    ...
    if await is_admin(uid):
        ...
        await state.finish()
        return
    # Проверяем, не супер-админ (хотя суперадмины в таблице admins не хранятся, но проверка is_admin вернёт True для них)
    # Эта проверка уже покрыта is_admin
    ...
    try:
        async with db_pool.acquire() as conn:
            await conn.execute(
                ...
                uid, message.from_user.id, datetime.now(), json.dumps(selected)
            )
...
async def list_admins(message: Message):
    ...
    for row in rows:
        ...
        date_str = added_date.strftime("%Y-%m-%d %H:%M:%S") if added_date else "неизвестно"
        text += f"ID: {row['user_id']}\n"
        text += f"Добавлен: {row['added_by']} ({date_str})\n"
...
async def broadcast_media(message: Message, state: FSMContext):
    ...
    await message.answer(
        "📨 Превью сообщения. Подтвердите отправку всем пользователям? (да/нет)",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
@dp.callback_query_handler(lambda c: c.data == "broadcast_confirm", state=Broadcast.media)
async def broadcast_confirm(callback: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    content = data['content']
    await state.finish()

    status_msg = await callback.message.answer("⏳ Рассылка начата... Это может занять некоторое время.")
...
@dp.callback_query_handler(lambda c: c.data == "broadcast_cancel", state=Broadcast.media)
async def broadcast_cancel(callback: CallbackQuery, state: FSMContext):
    await state.finish()
    await callback.message.edit_text("❌ Рассылка отменена.")
    permissions = await get_admin_permissions(callback.from_user.id)
    await callback.message.answer("Панель администратора:", reply_markup=admin_main_keyboard(permissions))
    await callback.answer()
...
# ==================== ФОНОВАЯ ЗАДАЧА: СПАВН НАЛЁТОВ (ВЫЗЫВАЕТСЯ ПЛАНИРОВЩИКОМ) ====================
async def heist_spawner_job():
    """Периодически создаёт налёты во всех подтверждённых чатах (с учётом кулдауна)."""
    try:
        min_int = await get_setting_int("heist_min_interval_minutes")
        max_int = await get_setting_int("heist_max_interval_minutes")
        # Эта функция вызывается планировщиком, поэтому интервал уже задан при добавлении задачи.
        # Здесь мы просто проверяем, нужно ли создавать налёт в каждом чате.
        confirmed = await get_confirmed_chats()
        if not confirmed:
            return
        ...
        for chat_id, chat_data in confirmed.items():
            try:
                ...
                async with db_pool.acquire() as conn:
                    ...
                    if last_heist:
                        if datetime.now() - last_heist < timedelta(minutes=min_int):
                            continue
                ...
                # Небольшая задержка между чатами, чтобы не флудить
        ...
        logging.error(f"Ошибка в heist_spawner_job: {e}")

# ==================== ФОНОВАЯ ЗАДАЧА: ОБРАБОТКА КОНТРАБАНДНЫХ РЕЙСОВ ====================
async def process_smuggle_runs_job():
    ...
    try:
        ...
        logging.error(f"Ошибка в process_smuggle_runs_job: {e}")

# ==================== ФОНОВАЯ ЗАДАЧА: ОБРАБОТКА ТЮРЕМНЫХ СРОКОВ ====================
async def process_jail_sentences_job():
    ...
    try:
        ...
        logging.error(f"Ошибка в process_jail_sentences_job: {e}")
...
async def periodic_cleanup_job():
    ...
    try:
        ...
        logging.error(f"Ошибка в periodic_cleanup_job: {e}")

# ==================== ФОНОВАЯ ЗАДАЧА: ЗАВЕРШЕНИЕ РОЗЫГРЫШЕЙ ====================
async def process_giveaways_job():
    """Проверяет истекшие розыгрыши, выбирает победителей и уведомляет."""
    try:
        now = datetime.now()
        async with db_pool.acquire() as conn:
            # Выбираем активные розыгрыши, у которых end_date <= now
            ...
            for gw in giveaways:
                ...
                allow_repeat = gw['allow_repeat_wins']
                max_participants = gw['max_participants']
                ...
                if not participant_ids:
                    ...
                    await conn.execute("UPDATE giveaways SET status='completed', winners_list='[]' WHERE id=$1", gw_id)
                    continue

                # Если запрещены повторные выигрыши, исключаем тех, кто уже выигрывал сегодня
                if not allow_repeat:
                    today = date.today()
                    # Получаем список ID, которые уже выигрывали сегодня
                    recent_winners = await conn.fetch("""
                        SELECT user_id FROM giveaway_winners_log
                        WHERE win_date = $1
                    """, today)
                    exclude_ids = [r['user_id'] for r in recent_winners]
                    eligible = [uid for uid in participant_ids if uid not in exclude_ids]
                    if not eligible:
                        # Если все уже выигрывали, то выбираем из всех (или можно завершить без победителей? Лучше выбрать из всех)
                        eligible = participant_ids
                else:
                    eligible = participant_ids

                # Выбираем победителей (без повторений)
                ...
                await conn.execute(
                    "UPDATE giveaways SET status='completed', winners_list=$1, notified=FALSE WHERE id=$2",
                    winners_list, gw_id
                )

                # Записываем в лог победителей, если запрещены повторы
                if not allow_repeat:
                    for winner_id in winners:
                        await conn.execute(
                            "INSERT INTO giveaway_winners_log (giveaway_id, user_id, win_date) VALUES ($1, $2, $3)",
                            gw_id, winner_id, today
                        )
                ...
                for winner_id in winners:
                    await safe_send_message(
                        winner_id,
                        f"🎉 Поздравляем! Вы выиграли в розыгрыше «{prize}»! Админ свяжется с вами."
                    )

                # Уведомление в чаты, если розыгрыш был создан в чате (по желанию)
                # Можно добавить уведомление в чат, но пока пропустим

    except Exception as e:
        logging.error(f"Ошибка в process_giveaways_job: {e}")

# ==================== ФОНОВАЯ ЗАДАЧА: ПРОВЕРКА ПОДПИСОК ДЛЯ ЗАДАНИЙ ====================
async def check_subscription_tasks_job():
    """Периодически проверяет, не отписались ли пользователи от каналов, и применяет штрафы."""
    try:
        interval = await get_setting_int("task_subscribe_check_interval")
        # Эта функция вызывается планировщиком с интервалом, поэтому interval не используется напрямую.
        # Просто выполняем проверку.

        async with db_pool.acquire() as conn:
            # Находим всех пользователей, у которых есть выполненные задания типа 'subscribe'
            rows = await conn.fetch("""
                SELECT ut.user_id, ut.task_id, t.target_id, t.reward_coins, t.reward_reputation
                FROM user_tasks ut
                JOIN tasks t ON ut.task_id = t.id
                WHERE t.task_type = 'subscribe' AND ut.status = 'completed'
            """)

            for row in rows:
                user_id = row['user_id']
                task_id = row['task_id']
                channel_id = row['target_id']
                reward_coins = float(row['reward_coins'])
                reward_rep = row['reward_reputation']

                # Проверяем подписку
                subscribed = await check_user_subscription(user_id, channel_id)
                if not subscribed:
                    # Пользователь отписался – применяем штраф
                    # Штраф: списываем награду (если есть) и репутацию, помечаем задание как проваленное
                    penalty_coins = await get_setting_int("task_unsubscribe_penalty_coins")
                    penalty_rep = await get_setting_int("task_unsubscribe_penalty_reputation")

                    # Списание баксов (если есть)
                    if penalty_coins > 0:
                        try:
                            await update_user_balance(user_id, -penalty_coins, conn=conn)
                        except:
                            # Если недостаточно, списываем сколько есть
                            balance = await get_user_balance(user_id)
                            if balance > 0:
                                await update_user_balance(user_id, -balance, conn=conn)

                    if penalty_rep > 0:
                        await update_user_reputation(user_id, -penalty_rep)

                    # Обновляем статус задания на 'failed'
                    await conn.execute(
                        "UPDATE user_tasks SET status='failed' WHERE user_id=$1 AND task_id=$2",
                        user_id, task_id
                    )

                    await safe_send_message(
                        user_id,
                        f"❌ Вы отписались от канала, за который получили награду. Штраф: {penalty_coins} баксов, {penalty_rep} репутации."
                    )

    except Exception as e:
        logging.error(f"Ошибка в check_subscription_tasks_job: {e}")
...
async def on_startup(dp):
    ...
    # Запускаем планировщик (он уже создан в Части 1)
    # Добавляем задачи в планировщик
    # Интервалы задаём в секундах

    # Спавн налётов – проверяем каждую минуту (можно настроить)
    scheduler.add_job(heist_spawner_job, 'interval', seconds=60, id='heist_spawner')

    # Контрабандные рейсы – каждые 30 секунд
    scheduler.add_job(process_smuggle_runs_job, 'interval', seconds=30, id='smuggle_runs')

    # Тюремные сроки – каждые 30 секунд
    scheduler.add_job(process_jail_sentences_job, 'interval', seconds=30, id='jail_sentences')

    # Очистка – раз в сутки (86400 секунд)
    scheduler.add_job(periodic_cleanup_job, 'interval', seconds=86400, id='cleanup')

    # Розыгрыши – каждую минуту
    scheduler.add_job(process_giveaways_job, 'interval', seconds=60, id='giveaways')

    # Проверка подписок для заданий – интервал из настроек, но не чаще 5 минут
    # Получаем интервал из настроек, но если он слишком мал, установим минимум 300 секунд
    task_check_interval = await get_setting_int("task_subscribe_check_interval")
    if task_check_interval < 60:
        task_check_interval = 300  # минимум 5 минут
    scheduler.add_job(check_subscription_tasks_job, 'interval', seconds=task_check_interval, id='subscription_tasks')

    scheduler.start()
    logging.info("✅ Планировщик запущен, добавлены фоновые задачи")
