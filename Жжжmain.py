# ==================== ЧАСТЬ 1: ИМПОРТЫ, НАСТРОЙКИ, ГЛОБАЛЬНЫЕ ПЕРЕМЕННЫЕ, ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ ====================
...
# ==================== ТИПЫ СОБЫТИЙ (НАЛЁТОВ) - без изменений, но можно добавить новые фразы позже ====================
HEIST_TYPES = {
    "incassator": {
        "name": "🚐 Инкассатор",
        "keyword": "ФАРТ",
...
# ==================== ФРАЗЫ ДЛЯ КОНТРАБАНДЫ (с небольшими правками) ====================
SMUGGLE_START_PHRASES = [
    ...
    "🛩️ {name}, ты летишь на параплане с {cargo}. Главное не чихай в воздухе. Вернёшься через {duration} мин.",
]
...
SMUGGLE_SUCCESS_PHRASES = [
    ...
    "✅ {name} (@{username}) засунул {cargo} в банку из-под шпрот. Никто не догадался! +{amount} BTC."
]
...
SMUGGLE_FAIL_PHRASES = [
    ...
    "❌ {name} (@{username}) так обрадовался, что начал танцевать и уронил {cargo} за борт. Убыток."
]
...
# ==================== ФУНКЦИИ ДЛЯ БИЗНЕСОВ (без изменений) ====================
@db_retry()
...
# ==================== ФУНКЦИИ ДЛЯ НАЛЁТОВ (без изменений) ====================
@db_retry()
...
async def get_chat_stats(chat_id: int = None) -> Dict:
    ...
    async with db_pool.acquire() as conn:
        if chat_id:
            ...
            total = await conn.fetchval("SELECT COUNT(*) FROM confirmed_chats")
            total_users = await conn.fetchval("SELECT COUNT(*) FROM users")
            # и т.д. можно расширить
            return {"total_chats": total, "total_users": total_users}
...
# ==================== КОНЕЦ ЧАСТИ 1 ====================
# ==================== ЧАСТЬ 2: СОСТОЯНИЯ FSM И КЛАВИАТУРЫ (ПОЛНАЯ) ====================
...
# ==================== СОСТОЯНИЯ FSM (добавлены новые для подпольных боёв) ====================
...
def admin_main_keyboard(permissions: List[str]) -> ReplyKeyboardMarkup:
    """Главное меню админ-панели (обновлённое с кнопкой управления боями)."""
    builder = ReplyKeyboardBuilder()
    row1 = []
    if "manage_users" in permissions:
        row1.append(KeyboardButton(text="👥 Пользователи"))
    if "manage_shop" in permissions:
        row1.append(KeyboardButton(text="🛒 Управление магазином"))
    if "manage_giveaways" in permissions:
        row1.append(KeyboardButton(text="+Розыгрыш"))
        row1.append(KeyboardButton(text="📋 Список розыгрышей"))
    if row1:
        builder.row(*row1)
    row2 = []
    ...
    # Новая кнопка управления боями (используем право manage_businesses или своё)
    if "manage_businesses" in permissions:
        row3.append(KeyboardButton(text="🥊 Управление боями"))
    if row3:
        builder.row(*row3)
    row4 = []
    if "manage_users" in permissions:  # используем manage_users для заданий
        row4.append(KeyboardButton(text="+Задание"))
        row4.append(KeyboardButton(text="📋 Список заданий"))
...
# ==================== КЛАВИАТУРЫ ДЛЯ ВЫБОРА ЧИСЕЛ (Inline) с цветами ====================

def guess_number_keyboard() -> InlineKeyboardMarkup:
    kb = []
    row = []
    for i in range(1, 6):
        row.append(InlineKeyboardButton(text=str(i), callback_data=f"guess_num_{i}", color="primary"))
        if len(row) == 3:
            kb.append(row)
            row = []
    if row:
        kb.append(row)
    kb.append([InlineKeyboardButton(text="❌ Отмена", callback_data="guess_cancel", color="negative")])
    return InlineKeyboardMarkup(inline_keyboard=kb)

def roulette_type_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔴 Красное", callback_data="roulette_type_red", color="primary"),
         InlineKeyboardButton(text="⚫️ Чёрное", callback_data="roulette_type_black", color="primary")],
        [InlineKeyboardButton(text="🟢 Зелёное", callback_data="roulette_type_green", color="primary"),
         InlineKeyboardButton(text="🔢 Число", callback_data="roulette_type_number", color="primary")],
        [InlineKeyboardButton(text="❌ Отмена", callback_data="roulette_cancel", color="negative")]
    ])
...
def roulette_number_keyboard() -> InlineKeyboardMarkup:
    ...
    row = []
    for i in range(0, 37):
        row.append(InlineKeyboardButton(text=str(i), callback_data=f"roulette_num_{i}", color="primary"))
        if len(row) == 5:
            kb.append(row)
            row = []
    if row:
        kb.append(row)
    kb.append([InlineKeyboardButton(text="❌ Отмена", callback_data="roulette_cancel", color="negative")])
    return InlineKeyboardMarkup(inline_keyboard=kb)

def repeat_bet_keyboard(game: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔁 Повторить", callback_data=f"repeat_{game}", color="primary")]
    ])

# ==================== КЛАВИАТУРЫ ДЛЯ ПОДТВЕРЖДЕНИЯ И Т.П. С ЦВЕТАМИ ====================

def confirm_chat_inline(chat_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Подтвердить", callback_data=f"confirm_chat_{chat_id}", color="positive"),
         InlineKeyboardButton(text="❌ Отклонить", callback_data=f"reject_chat_{chat_id}", color="negative")]
    ])
...
def subscription_inline(not_subscribed: List[Tuple[str, str]]) -> InlineKeyboardMarkup:
    ...
    for title, link in not_subscribed:
        if link:
            kb.append([InlineKeyboardButton(text=f"📢 {title}", url=link, color="primary")])
        else:
            kb.append([InlineKeyboardButton(text=f"📢 {title}", callback_data="no_link", color="secondary")])
    kb.append([InlineKeyboardButton(text="✅ Я подписался", callback_data="check_sub", color="positive")])
    return InlineKeyboardMarkup(inline_keyboard=kb)

def betray_choice_keyboard(heist_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔪 Украсть у подельников", callback_data=f"betray_choice_yes_{heist_id}", color="negative")],
        [InlineKeyboardButton(text="❌ Отказаться", callback_data=f"betray_choice_no_{heist_id}", color="secondary")]
    ])
...
def jail_cell_keyboard() -> InlineKeyboardMarkup:
    ...
    row = []
    for i in range(1, 16):
        row.append(InlineKeyboardButton(text=str(i), callback_data=f"jail_cell_{i}", color="primary"))
        if len(row) == 5:
            kb.append(row)
            row = []
    if row:
...
def task_detail_keyboard(task_id: int, button_link: str = None) -> InlineKeyboardMarkup:
    kb = []
    if button_link:
        kb.append([InlineKeyboardButton(text="📢 Перейти в канал", url=button_link, color="primary")])
    kb.append([InlineKeyboardButton(text="✅ Проверить подписку", callback_data=f"check_task_{task_id}", color="positive")])
    kb.append([InlineKeyboardButton(text="◀️ Назад", callback_data="tasks_back", color="secondary")])
    return InlineKeyboardMarkup(inline_keyboard=kb)

def giveaway_condition_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⏰ По времени", callback_data="giveaway_cond_time", color="primary")],
        [InlineKeyboardButton(text="👥 По количеству участников", callback_data="giveaway_cond_participants", color="primary")]
    ])

def promo_type_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="💰 MLB", callback_data="promo_type_coins", color="primary"),
         InlineKeyboardButton(text="₿ Биткоины", callback_data="promo_type_bitcoin", color="primary")]
    ])

def reset_stats_confirm_keyboard(user_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Подтвердить сброс", callback_data=f"reset_stats_confirm_{user_id}", color="positive")],
        [InlineKeyboardButton(text="❌ Отмена", callback_data="reset_stats_cancel", color="negative")]
    ])

def purchase_action_keyboard(purchase_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Выполнено", callback_data=f"purchase_done_{purchase_id}", color="positive"),
         InlineKeyboardButton(text="❌ Отказ", callback_data=f"purchase_reject_{purchase_id}", color="negative")]
    ])

def settings_param_keyboard(params: List[Tuple[str, str]], category: str) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    for key, desc in params:
        builder.button(text=desc, callback_data=f"edit_{key}", color="primary")
    builder.button(text="◀️ Назад", callback_data=f"settings_back_{category}", color="secondary")
    builder.adjust(1)
    return builder.as_markup()

def completed_giveaway_detail_keyboard(giveaway_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="👀 Посмотреть победителей", callback_data=f"view_completed_{giveaway_id}", color="primary")]
    ])
...
def fighters_keyboard(fighters: List[Dict]) -> InlineKeyboardMarkup:
    ...
    builder.button(text="❌ Отмена", callback_data="fight_cancel", color="negative")
    builder.adjust(2)
    return builder.as_markup()
...
def fight_bet_amount_keyboard() -> InlineKeyboardMarkup:
    ...
    for amount in [10, 50, 100, 500, 1000]:
        builder.button(text=f"{amount} MLB", callback_data=f"fight_amount_{amount}", color="primary")
    builder.button(text="✏️ Другая сумма", callback_data="fight_amount_custom", color="secondary")
    builder.button(text="❌ Отмена", callback_data="fight_cancel", color="negative")
    builder.adjust(3)
    return builder.as_markup()
...
def fight_confirm_keyboard() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Да", callback_data="fight_confirm_yes", color="positive")],
        [InlineKeyboardButton(text="❌ Нет", callback_data="fight_confirm_no", color="negative")]
    ])
...
def fighter_edit_fields_keyboard(fighter_id: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Имя", callback_data=f"fighter_edit_name_{fighter_id}")],
        [InlineKeyboardButton(text="Эмодзи", callback_data=f"fighter_edit_emoji_{fighter_id}")],
        [InlineKeyboardButton(text="Описание", callback_data=f"fighter_edit_desc_{fighter_id}")],
        [InlineKeyboardButton(text="Ключ картинки", callback_data=f"fighter_edit_image_{fighter_id}")],
        [InlineKeyboardButton(text="❌ Удалить бойца", callback_data=f"fighter_delete_{fighter_id}")],
        [InlineKeyboardButton(text="◀️ Назад", callback_data="admin_fight_back")]
    ])

def fight_result_keyboard(fight_id: int) -> InlineKeyboardMarkup:
    """Клавиатура для выбора результата боя."""
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🥇 Победа бойца 1", callback_data=f"fight_result_win1_{fight_id}")],
        [InlineKeyboardButton(text="🥈 Победа бойца 2", callback_data=f"fight_result_win2_{fight_id}")],
        [InlineKeyboardButton(text="🤝 Ничья", callback_data=f"fight_result_draw_{fight_id}")],
        [InlineKeyboardButton(text="◀️ Назад", callback_data="admin_fight_back")]
    ])

# ==================== КОНЕЦ ЧАСТИ 2 ====================
# ==================== ЧАСТЬ 3: ПОЛЬЗОВАТЕЛЬСКИЕ ХЕНДЛЕРЫ (ПОЛНАЯ, С НОВЫМИ БОЯМИ) ====================

# Внимание: этот код объединяет все пользовательские хендлеры из исходного рабочего кода
# (профиль, бонус, топ, казино, магазин, промокоды, рефералка, задания, розыгрыши, фармилка, биржа, ограбления)
# и добавляет новые хендлеры для подпольных боёв.
# Все функции и переменные из Частей 1 и 2 предполагаются доступными.
...
# ==================== ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ ДЛЯ УРОВНЕЙ (из исходного кода) ====================
...
# ==================== ПОВТОР СТАВКИ (из исходного кода) ====================
...
# ==================== УНИВЕРСИТЕТ (ПРОКАЧКА НАВЫКОВ) (из исходного кода) ====================
...
# ==================== МАГАЗИН ПОДАРКОВ (из исходного кода) ====================
...
# ==================== РЕФЕРАЛЬНАЯ ССЫЛКА (из исходного кода) ====================
...
# ==================== ЗАДАНИЯ (ПОЛЬЗОВАТЕЛЬСКАЯ ЧАСТЬ) (из исходного кода) ====================
...
# ==================== ПОЛЬЗОВАТЕЛЬСКИЕ РОЗЫГРЫШИ (из исходного кода) ====================
...
# ==================== ФАРМИЛКА (ПОЛЬЗОВАТЕЛЬСКАЯ ЧАСТЬ) (из исходного кода) ====================
...
# ==================== БИТКОИН-БИРЖА (ПОЛЬЗОВАТЕЛЬСКАЯ ЧАСТЬ) (из исходного кода) ====================
...
# ==================== НОВЫЕ ХЕНДЛЕРЫ ДЛЯ ПОДПОЛЬНЫХ БОЁВ ====================
...
async def fight_menu(message: Message, state: FSMContext):
    ...
    text = (
        ...
        f"⏳ До начала боя: {minutes_left} мин {seconds_left} сек.\n"
...
# ==================== КОНЕЦ ЧАСТИ 3 ====================
# ==================== ЧАСТЬ 4: АДМИНИСТРИРОВАНИЕ ПОДПОЛЬНЫХ БОЁВ, РЕФЕРАЛЬНЫЙ ТОП, СТАТИСТИКА ЧАТОВ ====================

# Внимание: этот код предполагает, что все импорты из Части 1 уже выполнены,
# а также что в Части 2 добавлены новые состояния и клавиатуры (AddFighter, CreateFight и т.д.).
# Здесь определяются хендлеры для администраторов и новые фоновые задачи.
...
# ==================== ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ ДЛЯ БОЁВ (дублируются из Части 3 для целостности) ====================
...
# ==================== АДМИНИСТРИРОВАНИЕ БОЙЦОВ ====================

@dp.message(F.text == "🥊 Управление боями")
async def admin_fights_main(message: Message):
    """Главное меню управления подпольными боями для админов."""
    if not await check_admin_permissions(message.from_user.id, "manage_businesses"):  # используем существующее право
        await message.answer("❌ Недостаточно прав.")
        return
    kb = ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="🥊 Бойцы"), KeyboardButton(text="🥊 Бои")],
            [KeyboardButton(text="◀️ Назад в админку")]
        ],
        resize_keyboard=True
    )
    await message.answer("Управление подпольными боями:", reply_markup=kb)

@dp.message(F.text == "🥊 Бойцы")
async def admin_fighters_list(message: Message):
    """Список бойцов с возможностью редактирования."""
    if not await check_admin_permissions(message.from_user.id, "manage_businesses"):
        return
    fighters = await get_fighters()
    if not fighters:
        await message.answer("Нет бойцов. Добавь первого!", reply_markup=admin_fighters_keyboard([]))
        return
    await message.answer("Список бойцов:", reply_markup=admin_fighters_keyboard(fighters))

@dp.callback_query(F.data == "add_fighter")
async def add_fighter_start(callback: CallbackQuery, state: FSMContext):
    """Начало добавления нового бойца."""
    await callback.answer()
    await callback.message.answer("Введи имя бойца (например, 'Мясник Боб'):", reply_markup=back_keyboard())
    await state.set_state(AddFighter.name)

@dp.message(AddFighter.name, F.text)
async def add_fighter_name(message: Message, state: FSMContext):
    if message.text == "◀️ Назад":
        await state.clear()
        await admin_fighters_list(message)
        return
    await state.update_data(name=message.text)
    await message.answer("Введи эмодзи для бойца (один символ, например 🥩):")
    await state.set_state(AddFighter.emoji)

@dp.message(AddFighter.emoji, F.text)
async def add_fighter_emoji(message: Message, state: FSMContext):
    if message.text == "◀️ Назад":
        await state.clear()
        await admin_fighters_list(message)
        return
    await state.update_data(emoji=message.text)
    await message.answer("Введи описание бойца (например, 'Бывший мясник, рубит с плеча'):")
    await state.set_state(AddFighter.description)

@dp.message(AddFighter.description, F.text)
async def add_fighter_description(message: Message, state: FSMContext):
    if message.text == "◀️ Назад":
        await state.clear()
        await admin_fighters_list(message)
        return
    await state.update_data(description=message.text)
    await message.answer("Введи ключ картинки для бойца (например, 'fighter_butcher') или '-' если без картинки:")
    await state.set_state(AddFighter.image_key)

@dp.message(AddFighter.image_key, F.text)
async def add_fighter_image_key(message: Message, state: FSMContext):
    if message.text == "◀️ Назад":
        await state.clear()
        await admin_fighters_list(message)
        return
    image_key = None if message.text == '-' else message.text.strip()
    await state.update_data(image_key=image_key)
    data = await state.get_data()
    text = (
        f"Проверь данные бойца:\n"
        f"Имя: {data['name']}\n"
        f"Эмодзи: {data['emoji']}\n"
        f"Описание: {data['description']}\n"
        f"Ключ картинки: {image_key}\n\n"
        f"Подтвердить добавление?"
    )
    await message.answer(text, reply_markup=fight_confirm_keyboard())
    await state.set_state(AddFighter.confirm)

@dp.callback_query(AddFighter.confirm, F.data.startswith("fight_confirm_"))
async def add_fighter_confirm(callback: CallbackQuery, state: FSMContext):
    action = callback.data.split("_")[2]
    if action == "no":
    ...
    async with db_pool.acquire() as conn:
        await conn.execute(
            "INSERT INTO fighters (name, emoji, description, image_key) VALUES ($1, $2, $3, $4)",
            data['name'], data['emoji'], data['description'], data['image_key']
        )
    await callback.message.edit_text(f"✅ Боец {data['emoji']} {data['name']} добавлен!")
    await state.clear()
    await admin_fighters_list(callback.message)

@dp.callback_query(F.data.startswith("edit_fighter_"))
async def edit_fighter_start(callback: CallbackQuery, state: FSMContext):
    fighter_id = int(callback.data.split("_")[2])
    fighter = await get_fighter(fighter_id)
    if not fighter:
        await callback.answer("❌ Боец не найден.", show_alert=True)
        return
    await state.update_data(fighter_id=fighter_id)
    text = (
        f"{fighter['emoji']} {fighter['name']}\n"
        f"Описание: {fighter['description']}\n"
        f"Статистика: {fighter['wins']} побед, {fighter['losses']} поражений, {fighter['draws']} ничьих\n\n"
        f"Что хочешь изменить?"
    )
    await callback.message.edit_text(text, reply_markup=fighter_edit_fields_keyboard(fighter_id))
    await callback.answer()

@dp.callback_query(F.data.startswith("fighter_edit_"))
async def edit_fighter_field(callback: CallbackQuery, state: FSMContext):
    parts = callback.data.split("_")
    field = parts[2]  # name, emoji, desc, image
    fighter_id = int(parts[3])
    await state.update_data(fighter_id=fighter_id, field=field)
    prompts = {
        "name": "Введи новое имя:",
        "emoji": "Введи новый эмодзи:",
        "desc": "Введи новое описание:",
        "image": "Введи новый ключ картинки (или '-'):"
    }
    await callback.message.edit_text(prompts[field])
    await state.set_state(EditFighter.value)
    await callback.answer()

@dp.message(EditFighter.value, F.text)
async def edit_fighter_value(message: Message, state: FSMContext):
    ...
    async with db_pool.acquire() as conn:
        await conn.execute(f"UPDATE fighters SET {db_column}=$1 WHERE id=$2", value, fighter_id)

    await message.answer("✅ Обновлено.")
    await state.clear()
    await admin_fighters_list(message)

@dp.callback_query(F.data.startswith("fighter_delete_"))
async def delete_fighter(callback: CallbackQuery, state: FSMContext):
    fighter_id = int(callback.data.split("_")[2])
    # Проверим, не участвует ли боец в активных боях
    async with db_pool.acquire() as conn:
        active = await conn.fetchval(
            "SELECT 1 FROM fights WHERE (fighter1_id=$1 OR fighter2_id=$1) AND status='scheduled'",
            fighter_id
        )
        if active:
            await callback.answer("❌ Нельзя удалить бойца, участвующего в активном бою.", show_alert=True)
            return
        await conn.execute("DELETE FROM fighters WHERE id=$1", fighter_id)
    await callback.answer("✅ Боец удалён.")
    await admin_fighters_list(callback.message)

# ==================== АДМИНИСТРИРОВАНИЕ БОЁВ ====================

@dp.message(F.text == "🥊 Бои")
async def admin_fights_list(message: Message):
    """Список боёв (активные и запланированные)."""
    if not await check_admin_permissions(message.from_user.id, "manage_businesses"):
        return
    async with db_pool.acquire() as conn:
        rows = await conn.fetch("""
            SELECT f.*, f1.name as fighter1_name, f1.emoji as fighter1_emoji,
                   f2.name as fighter2_name, f2.emoji as fighter2_emoji
            FROM fights f
        ...
        """)
    if not rows:
        await message.answer("Нет запланированных боёв.", reply_markup=admin_fights_list_keyboard([]))
        return
    fights = [dict(r) for r in rows]
    await message.answer("Запланированные бои:", reply_markup=admin_fights_list_keyboard(fights))

@dp.callback_query(F.data == "create_fight")
async def create_fight_start(callback: CallbackQuery, state: FSMContext):
    """Начало создания боя."""
    fighters = await get_fighters()
    if len(fighters) < 2:
        await callback.answer("❌ Нужно хотя бы два бойца.", show_alert=True)
        return
    # Покажем список для выбора первого бойца
    kb = fighters_keyboard(fighters)  # используем ту же клавиатуру, что для ставок
    await callback.message.edit_text("Выбери первого бойца:", reply_markup=kb)
    await state.set_state(CreateFight.fighter1_id)
    await callback.answer()

@dp.callback_query(CreateFight.fighter1_id, F.data.startswith("fight_fighter_"))
async def create_fight_fighter1(callback: CallbackQuery, state: FSMContext):
    fighter1_id = int(callback.data.split("_")[2])
    await state.update_data(fighter1_id=fighter1_id)
    fighters = await get_fighters()
    # Убираем выбранного из списка
    fighters = [f for f in fighters if f['id'] != fighter1_id]
    if not fighters:
        await callback.answer("❌ Нет других бойцов.", show_alert=True)
        return
    kb = fighters_keyboard(fighters)
    await callback.message.edit_text("Выбери второго бойца:", reply_markup=kb)
    await state.set_state(CreateFight.fighter2_id)
    await callback.answer()

@dp.callback_query(CreateFight.fighter2_id, F.data.startswith("fight_fighter_"))
async def create_fight_fighter2(callback: CallbackQuery, state: FSMContext):
    fighter2_id = int(callback.data.split("_")[2])
    await state.update_data(fighter2_id=fighter2_id)
    await callback.message.edit_text("Через сколько минут начнётся бой? (введи число):")
    await state.set_state(CreateFight.start_delay_minutes)
    await callback.answer()

@dp.message(CreateFight.start_delay_minutes, F.text)
async def create_fight_delay(message: Message, state: FSMContext):
    try:
        ...
        return
    data = await state.get_data()
    fighter1 = await get_fighter(data['fighter1_id'])
    fighter2 = await get_fighter(data['fighter2_id'])
    start_time = datetime.now() + timedelta(minutes=delay)
    # Время окончания боя = время начала + длительность боя (берём из настроек)
    fight_duration = await get_setting_int("fight_result_delay_seconds")  # в секундах
    end_time = start_time + timedelta(seconds=fight_duration)

    text = (
        f"📅 Создание боя:\n"
        f"{fighter1['emoji']} {fighter1['name']} VS {fighter2['emoji']} {fighter2['name']}\n"
        f"Начало через {delay} мин.\n\n"
        f"Подтверждаешь?"
    )
    await message.answer(text, reply_markup=fight_confirm_keyboard())
...
@dp.callback_query(CreateFight.confirm, F.data.startswith("fight_confirm_"))
async def create_fight_confirm(callback: CallbackQuery, state: FSMContext):
    action = callback.data.split("_")[2]
    if action == "no":
        await callback.message.edit_text("❌ Создание боя отменено.")
        await state.clear()
        await callback.answer()
        return
    data = await state.get_data()
    async with db_pool.acquire() as conn:
        await conn.execute(
            "INSERT INTO fights (fighter1_id, fighter2_id, start_time, end_time, status) VALUES ($1, $2, $3, $4, $5)",
            data['fighter1_id'], data['fighter2_id'], data['start_time'], data['end_time'], 'scheduled'
        )
    await callback.message.edit_text("✅ Бой создан!")
    await state.clear()
    await admin_fights_list(callback.message)

@dp.callback_query(F.data.startswith("admin_fight_"))
async def admin_fight_detail(callback: CallbackQuery):
    ...
    f = dict(fight)
    text = (
        f"Бой #{f['id']}: {f['fighter1_emoji']} {f['fighter1_name']} vs {f['fighter2_emoji']} {f['fighter2_name']}\n"
        f"Статус: {f['status']}\n"
        f"Начало: {f['start_time'].strftime('%d.%m.%Y %H:%M')}\n"
        f"Окончание: {f['end_time'].strftime('%d.%m.%Y %H:%M')}\n"
        f"Всего ставок: {float(f['total_bets_mlb']):.2f} MLB\n"
    )
    if f['status'] == 'scheduled':
        # Кнопка для принудительного завершения
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🏁 Завершить бой", callback_data=f"end_fight_{fight_id}")]
        ])
    else:
        kb = None
...
@dp.callback_query(F.data.startswith("end_fight_"))
async def end_fight_start(callback: CallbackQuery, state: FSMContext):
    fight_id = int(callback.data.split("_")[2])
    async with db_pool.acquire() as conn:
        fight = await conn.fetchrow("SELECT * FROM fights WHERE id=$1 AND status='scheduled'", fight_id)
        if not fight:
            await callback.answer("❌ Бой не найден или уже завершён.", show_alert=True)
            return
    await state.update_data(fight_id=fight_id)
    await callback.message.edit_text("Выбери результат:", reply_markup=fight_result_keyboard(fight_id))
    await callback.answer()

@dp.callback_query(F.data.startswith("fight_result_"))
async def end_fight_result(callback: CallbackQuery, state: FSMContext):
    parts = callback.data.split("_")
    result = parts[2]  # win1, win2, draw
    fight_id = int(parts[3])
    await state.update_data(result=result)
    # Запрашиваем подтверждение
    await callback.message.edit_text(
        f"Ты выбрал результат: {result}. Подтверди завершение боя?",
        reply_markup=fight_confirm_keyboard()
    )
    await state.set_state(EndFight.confirm)
    await callback.answer()

@dp.callback_query(EndFight.confirm, F.data.startswith("fight_confirm_"))
async def end_fight_confirm(callback: CallbackQuery, state: FSMContext):
    action = callback.data.split("_")[2]
    if action == "no":
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            if result == 'win1':
                winner_id = await conn.fetchval("SELECT fighter1_id FROM fights WHERE id=$1", fight_id)
                await conn.execute(
                    "UPDATE fights SET status='finished', result='win1', winner_id=$1 WHERE id=$2",
                    winner_id, fight_id
                )
                await conn.execute("UPDATE fighters SET wins = wins + 1 WHERE id=$1", winner_id)
                loser_id = await conn.fetchval("SELECT fighter2_id FROM fights WHERE id=$1", fight_id)
                await conn.execute("UPDATE fighters SET losses = losses + 1 WHERE id=$1", loser_id)
            elif result == 'win2':
                winner_id = await conn.fetchval("SELECT fighter2_id FROM fights WHERE id=$1", fight_id)
                await conn.execute(
                    "UPDATE fights SET status='finished', result='win2', winner_id=$1 WHERE id=$2",
                    winner_id, fight_id
                )
                await conn.execute("UPDATE fighters SET wins = wins + 1 WHERE id=$1", winner_id)
                loser_id = await conn.fetchval("SELECT fighter1_id FROM fights WHERE id=$1", fight_id)
                await conn.execute("UPDATE fighters SET losses = losses + 1 WHERE id=$1", loser_id)
            else:  # draw
                await conn.execute(
                    "UPDATE fights SET status='finished', result='draw' WHERE id=$1",
                    fight_id
                )
                await conn.execute(
                    "UPDATE fighters SET draws = draws + 1 WHERE id IN (SELECT fighter1_id FROM fights WHERE id=$1 UNION SELECT fighter2_id FROM fights WHERE id=$1)",
                    fight_id
                )
                winner_id = None

            # Обрабатываем ставки
            bets = await conn.fetch("SELECT * FROM fight_bets WHERE fight_id=$1", fight_id)
            for bet in bets:
                user_id = bet['user_id']
                amount = float(bet['amount_mlb'])
                if winner_id is None:  # ничья
                    refund_percent = await get_setting_float("fight_draw_refund_percent") / 100
                    refund = amount * refund_percent
                    await update_user_balance(user_id, refund, conn=conn, allow_negative=False)
                    await conn.execute(
                        "UPDATE fight_bets SET settled=TRUE, won=FALSE WHERE id=$1",
                        bet['id']
                    )
                elif bet['fighter_id'] == winner_id:
                    win_amount = float(bet['potential_win'])
                    await update_user_balance(user_id, win_amount, conn=conn, allow_negative=False)
                    await conn.execute(
                        "UPDATE fight_bets SET settled=TRUE, won=TRUE WHERE id=$1",
                        bet['id']
                    )
                else:
                    await conn.execute(
    ...
    await callback.message.edit_text("✅ Бой завершён, результаты обработаны.")
    await state.clear()

@dp.callback_query(F.data == "admin_fight_back")
async def admin_fight_back(callback: CallbackQuery):
    await callback.answer()
    await admin_fights_main(callback.message)
...
# ==================== СТАТИСТИКА ПО ЧАТАМ (ДЛЯ АДМИНА) ====================

async def get_chat_stats() -> Dict:
    """Возвращает общую статистику по чатам."""
    async with db_pool.acquire() as conn:
        total_chats = await conn.fetchval("SELECT COUNT(*) FROM confirmed_chats")
        total_users = await conn.fetchval("SELECT COUNT(*) FROM users")
        return {"total_chats": total_chats, "total_users": total_users}

@dp.message(F.text == "📊 Статистика чатов")
async def chat_stats(message: Message):
    """Показывает общую статистику по чатам."""
    if not await check_admin_permissions(message.from_user.id, "view_stats"):
        return
    stats = await get_chat_stats()
    text = (
        f"📊 Статистика чатов:\n"
        f"Всего подтверждённых чатов: {stats.get('total_chats', 0)}\n"
        f"Всего пользователей: {stats.get('total_users', 0)}\n"
    )
    await message.answer(text)

# ==================== ФОНОВАЯ ЗАДАЧА ДЛЯ АВТОМАТИЧЕСКОГО ЗАВЕРШЕНИЯ БОЁВ ====================

async def fight_auto_resolver():
    """Периодически проверяет бои, у которых наступило время окончания, и завершает их."""
    while True:
        try:
            await asyncio.sleep(60)  # проверка раз в минуту
            now = datetime.now()
            async with db_pool.acquire() as conn:
                # Находим бои, которые должны были закончиться, но ещё не завершены
                overdue_fights = await conn.fetch("""
                    SELECT * FROM fights WHERE status='scheduled' AND end_time <= $1
                """, now)

                for fight in overdue_fights:
                    # Автоматически определяем результат (случайно, с небольшим перевесом в сторону статистики)
                    fighter1 = await conn.fetchrow("SELECT wins, losses FROM fighters WHERE id=$1", fight['fighter1_id'])
                    fighter2 = await conn.fetchrow("SELECT wins, losses FROM fighters WHERE id=$1", fight['fighter2_id'])
                    # Простой алгоритм: шанс на победу пропорционален отношению побед к поражениям, но не менее 30%
                    total1 = fighter1['wins'] + fighter1['losses'] + 1
                    total2 = fighter2['wins'] + fighter2['losses'] + 1
                    chance1 = max(30, min(70, fighter1['wins'] / total1 * 100))
                    chance2 = max(30, min(70, fighter2['wins'] / total2 * 100))
                    # Нормализуем
                    total = chance1 + chance2
                    chance1 = chance1 / total * 100
                    chance2 = chance2 / total * 100
                    rand = random.uniform(0, 100)
                    if rand < chance1:
                        result = 'win1'
                    elif rand < chance1 + chance2:
                        result = 'win2'
                    else:
                        result = 'draw'

                    # Применяем результат (аналогично ручному завершению)
                    async with conn.transaction():
                        if result == 'win1':
                            ...
                            await conn.execute("UPDATE fighters SET wins = wins + 1 WHERE id=$1", winner_id)
                            await conn.execute("UPDATE fighters SET losses = losses + 1 WHERE id=$1", fight['fighter2_id'])
                        elif result == 'win2':
                            ...
                            await conn.execute("UPDATE fighters SET wins = wins + 1 WHERE id=$1", winner_id)
                            await conn.execute("UPDATE fighters SET losses = losses + 1 WHERE id=$1", fight['fighter1_id'])
                        else:
                            await conn.execute(
                                "UPDATE fights SET status='finished', result='draw' WHERE id=$1",
                                fight['id']
                            ...
                            await conn.execute(
                                "UPDATE fighters SET draws = draws + 1 WHERE id IN ($1, $2)",
                                fight['fighter1_id'], fight['fighter2_id']
                            )
                            winner_id = None

                        # Обрабатываем ставки
                        bets = await conn.fetch("SELECT * FROM fight_bets WHERE fight_id=$1", fight['id'])
                        for bet in bets:
                            user_id = bet['user_id']
                            amount = float(bet['amount_mlb'])
                            if winner_id is None:  # ничья
            ...
            logging.error(f"Ошибка в fight_auto_resolver: {e}")
            await asyncio.sleep(60)

# ==================== ФОНОВАЯ ЗАДАЧА: НАГРАЖДЕНИЕ ТОПА РЕФЕРАЛОВ ====================

async def referral_top_reward():
    """Раз в неделю награждает топ рефералов BTC."""
    while True:
        interval_days = await get_setting_int("referral_top_interval_days")
        await asyncio.sleep(interval_days * 86400)
        top_count = await get_setting_int("referral_top_count")
        reward_btc = await get_setting_float("referral_top_reward_btc")
        top = await get_referral_top(limit=top_count)
        for user in top:
            user_id = user['user_id']
            await update_user_bitcoin(user_id, reward_btc)
            await safe_send_message(
                user_id,
                f"🏆 Ты попал в топ рефералов и получаешь {reward_btc:.4f} BTC!"
            )

# ==================== КОНЕЦ ЧАСТИ 4 ====================
# ==================== ЧАСТЬ 5: АДМИНИСТРАТИВНЫЕ ХЕНДЛЕРЫ (ПОЛЬЗОВАТЕЛИ, МАГАЗИН, КАНАЛЫ, ПРОМОКОДЫ, ЧАТЫ, БИЗНЕСЫ, БИРЖА, МЕДИА, ЗАДАНИЯ, РОЗЫГРЫШИ, АДМИНИСТРАТОРЫ, СТАТИСТИКА, РАССЫЛКА, НАСТРОЙКИ, ОЧИСТКА) ====================

# Внимание: этот код предполагает, что все импорты из Части 1 уже выполнены,
# а также что в Части 2 добавлены все необходимые клавиатуры.
# Здесь определяются все административные хендлеры, которые были в исходном рабочем коде,
# и добавляются новые для подпольных боёв (уже в Части 4), поэтому здесь они не повторяются.
...
SETTINGS_CATEGORIES = {
    ...
    "⚙️ Подпольные бои": [
        ("fight_min_bet", "💰 Мин. ставка (MLB)"),
        ("fight_max_bet", "💰 Макс. ставка (MLB)"),
        ("fight_commission_percent", "💸 Комиссия (%)"),
        ("fight_draw_refund_percent", "🤝 Возврат при ничьей (%)"),
        ("fight_result_delay_seconds", "⏱ Длительность боя (сек)"),
        ("fight_cooldown_hours", "⏳ Кулдаун между боями (часы)"),
    ],
}
...
async def create_task_name(message: Message, state: FSMContext):
    if message.text == "◀️ Назад":
        await state.clear()
        await admin_panel(message)  # вернуться в админку
        return
    await state.update_data(name=message.text)
...
async def create_task_media(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(f"✅ Задание создано! ID: {task_id}", reply_markup=admin_main_keyboard(await get_admin_permissions(message.from_user.id)))
    except Exception as e:
    ...
    try:
        async with db_pool.acquire() as conn:
            ...
            rows = await conn.fetch(
                ...
                "completed_count, task_type, required_days, penalty_days FROM tasks WHERE active=TRUE "
        ...
        for row in rows:
            text += (
                ...
                f"  Требуется дней: {row['required_days']}, штраф дней: {row['penalty_days']}\n\n"
            )
        total_pages = (total + ITEMS_PER_PAGE - 1) // ITEMS_PER_PAGE
        if page > 1 or page < total_pages:
            builder = InlineKeyboardBuilder()
            ...
            await message.answer(text, reply_markup=admin_main_keyboard(await get_admin_permissions(message.from_user.id)))
    except Exception as e:
...
async def delete_task_finish(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer("✅ Задание удалено.", reply_markup=admin_main_keyboard(await get_admin_permissions(message.from_user.id)))
    except Exception as e:
...
async def create_giveaway_media(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer("✅ Розыгрыш создан!", reply_markup=admin_main_keyboard(await get_admin_permissions(message.from_user.id)))
    except Exception as e:
        logging.error(f"Create giveaway error: {e}")
        await message.answer("❌ Ошибка при создании розыгрыша.")
    await state.clear()

# ----- Активные розыгрыши (админ) с пагинацией -----
...
async def admin_active_giveaways(message: Message, page: int = 1):
    ...
    async with db_pool.acquire() as conn:
        ...
        rows = await conn.fetch(
            "SELECT id, prize, description, end_date, media_file_id, media_type, min_participants, condition_type "
            "FROM giveaways WHERE status='active' ORDER BY end_date LIMIT $1 OFFSET $2",
            ITEMS_PER_PAGE, offset
        )
    if not rows:
        await message.answer("Нет активных розыгрышей.")
    ...
    if page > 1 or page < total_pages:
        nav = []
        if page > 1:
            nav.append(InlineKeyboardButton(text="⬅️", callback_data=f"admin_gw_page_{page-1}"))
        if page < total_pages:
            nav.append(InlineKeyboardButton(text="➡️", callback_data=f"admin_gw_page_{page+1}"))
...
# ----- Завершение розыгрыша (через инлайн-кнопку в списке) -----
...
async def complete_giveaway_winners(message: Message, state: FSMContext):
    ...
    await message.answer("✅ Розыгрыш завершён.", reply_markup=admin_main_keyboard(await get_admin_permissions(message.from_user.id)))
    await state.clear()

# ----- Завершённые розыгрыши (админ) -----
@dp.message(F.text == "📋 Завершённые розыгрыши")
async def admin_completed_giveaways(message: Message, page: int = 1):
    if not await check_admin_permissions(message.from_user.id, "manage_giveaways"):
        return
    ...
    if page > 1 or page < total_pages:
        nav = []
        if page > 1:
            nav.append(InlineKeyboardButton(text="⬅️", callback_data=f"admin_completed_gw_page_{page-1}"))
        if page < total_pages:
            nav.append(InlineKeyboardButton(text="➡️", callback_data=f"admin_completed_gw_page_{page+1}"))
        kb = InlineKeyboardMarkup(inline_keyboard=[nav])
        await message.answer(text, reply_markup=kb)
...
@dp.callback_query(F.data.startswith("admin_completed_gw_page_"))
async def admin_completed_gw_page_callback(callback: CallbackQuery):
    await callback.answer()
    page = int(callback.data.split("_")[4])
    await admin_completed_giveaways(callback.message, page=page)
...
# ==================== НАСТРОЙКИ ====================
# Категории настроек (определены ранее, но здесь они уже есть)
...
# Предполагается, что все необходимые функции и переменные уже определены в предыдущих частях
...
# ==================== ФОНОВАЯ ЗАДАЧА: ОЧИСТКА ЛОГОВ КАЖДЫЕ 3 ЧАСА ====================
async def log_cleaner():
    """Очищает старые записи из таблиц с логами."""
    while True:
        await asyncio.sleep(10800)  # 3 часа
        try:
            async with db_pool.acquire() as conn:
                cutoff = datetime.now() - timedelta(hours=6)
                await conn.execute("DELETE FROM heists WHERE status='finished' AND split_until < $1", cutoff)
                await conn.execute("DELETE FROM smuggle_runs WHERE status IN ('completed','failed') AND end_time < $1", cutoff)
                await conn.execute("DELETE FROM jail_sentences WHERE status='completed' AND end_time < $1", cutoff)
                await conn.execute("DELETE FROM heist_betrayals WHERE created_at < $1", cutoff)
                await conn.execute("DELETE FROM bitcoin_trades WHERE traded_at < $1", cutoff)
            logging.info("🧹 Логи очищены за последние 6 часов")
        except Exception as e:
            logging.error(f"Log cleaner error: {e}")
...
async def on_startup():
    ...
    await bot.set_my_commands([
        ...
        types.BotCommand(command="topref", description="🔗 Топ рефералов"),
    ])
    ...
    asyncio.create_task(fight_auto_resolver())
    asyncio.create_task(referral_top_reward())
