...
# Безопасное добавление sslmode=require, если его нет
def ensure_sslmode_in_url(url: str) -> str:
    parsed = urlparse(url)
...
DATABASE_URL = ensure_sslmode_in_url(DATABASE_URL)
...
# ==================== ВСПОМОГАТЕЛЬНАЯ ФУНКЦИЯ ДЛЯ ЧАСОВЫХ ПОЯСОВ ====================
def ensure_aware(dt: Optional[datetime]) -> Optional[datetime]:
    """Если datetime наивный, добавляет UTC timezone. Если None, возвращает None."""
    if dt is None:
        return None
...
async def redis_get(key: str) -> Optional[str]:
    ...
    try:
        ...
        if value is None:
            return None
        # aioredis может возвращать bytes или str в зависимости от decode_responses
...
async def acquire_lock(lock_name: str, timeout: int = 10) -> bool:
    ...
    if redis_client is not None:
        try:
            ...
            # Используем setnx для атомарности
    ...
    try:
        async with db_pool.acquire() as conn:
            # Используем SHA256 для стабильного 64-битного хэша
...
async def create_db_pool(retries: int = 10, delay: int = 5) -> bool:
    global db_pool
    database_url = DATABASE_URL  # уже с sslmode благодаря ensure_sslmode_in_url
...
async def keep_db_alive():
    ...
    while True:
        try:
            ...
            await ensure_db_connection()
            consecutive_failures = 0
            base_delay = 30  # сброс задержки при успехе
        except Exception as e:
            ...
            base_delay = min(300, base_delay * 2)
            await asyncio.sleep(base_delay)

# ==================== ВСПОМОГАТЕЛЬНАЯ ФУНКЦИЯ ДЛЯ ДОБАВЛЕНИЯ КОЛОНОК ====================
async def add_column_if_not_exists(conn, table: str, column: str, definition: str):
    ...
    """, table, column)
    if not exists:
        logging.info(f"Добавление колонки {table}.{column} ({definition})")
        await conn.execute(f"ALTER TABLE {table} ADD COLUMN {column} {definition}")

# ==================== ИНИЦИАЛИЗАЦИЯ ТАБЛИЦ ====================
async def migrate_date_columns(conn):
    """Безопасное преобразование текстовых колонок в TIMESTAMP с обработкой ошибок."""
    ...
    for table, column in migrations:
        try:
            ...
            if col_type == 'text':
                ...
                # Сначала обновим пустые строки в NULL
                await conn.execute(f"UPDATE {table} SET {column}=NULL WHERE {column}=''")
                # Затем преобразуем тип с проверкой формата
                await conn.execute(f"""
                    ALTER TABLE {table} 
                    ALTER COLUMN {column} TYPE TIMESTAMP 
                    USING CASE 
                        WHEN {column} IS NULL THEN NULL
                        WHEN {column} ~ '^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$' THEN {column}::timestamp
                        ELSE NULL
                    END
                """)
        except Exception as e:
            logging.warning(f"Миграция {table}.{column} не удалась: {e} (пропускаем)")
...
async def init_db():
    async with db_pool.acquire() as conn:
        ...
        ''')
        # Добавляем недостающие колонки (на случай, если таблица уже существовала)
        await add_column_if_not_exists(conn, 'users', 'skill_share', 'INTEGER DEFAULT 0')
        await add_column_if_not_exists(conn, 'users', 'skill_luck', 'INTEGER DEFAULT 0')
        await add_column_if_not_exists(conn, 'users', 'skill_betray', 'INTEGER DEFAULT 0')
        await add_column_if_not_exists(conn, 'users', 'heists_joined', 'INTEGER DEFAULT 0')
        await add_column_if_not_exists(conn, 'users', 'heists_betray_attempts', 'INTEGER DEFAULT 0')
        await add_column_if_not_exists(conn, 'users', 'heists_betray_success', 'INTEGER DEFAULT 0')
        await add_column_if_not_exists(conn, 'users', 'heists_betrayed_count', 'INTEGER DEFAULT 0')
        await add_column_if_not_exists(conn, 'users', 'heists_earned', 'NUMERIC(12,2) DEFAULT 0')
        await add_column_if_not_exists(conn, 'users', 'smuggle_success', 'INTEGER DEFAULT 0')
        await add_column_if_not_exists(conn, 'users', 'smuggle_fail', 'INTEGER DEFAULT 0')
        await add_column_if_not_exists(conn, 'users', 'strength', 'INTEGER DEFAULT 1')
        await add_column_if_not_exists(conn, 'users', 'agility', 'INTEGER DEFAULT 1')
        await add_column_if_not_exists(conn, 'users', 'defense', 'INTEGER DEFAULT 1')
        await add_column_if_not_exists(conn, 'users', 'authority_balance', 'INTEGER DEFAULT 0')
        await add_column_if_not_exists(conn, 'users', 'global_authority', 'INTEGER DEFAULT 0')  # возможно дубль, но оставим
...
# Кеш настроек с TTL 60 секунд и защитой от одновременного обновления
...
async def get_setting(key: str) -> str:
    ...
    async with settings_cache_lock:
        # Обновляем кеш, если он устарел или пуст
        if now - last_settings_update > settings_cache_ttl or not settings_cache:
            try:
                async with db_pool.acquire() as conn:
                ...
                logging.error(f"Ошибка при обновлении кеша настроек: {e}")
                # Если кеш пуст, пробуем взять из DEFAULT_SETTINGS
                if not settings_cache:
                    # Если и DEFAULT нет, вернём пустую строку
                    pass
        value = settings_cache.get(key)
        if value is None:
            value = DEFAULT_SETTINGS.get(key, "")
            if value:
                # Асинхронно вставляем в БД, но не ждём
                asyncio.create_task(_insert_default_setting(key, value))
                settings_cache[key] = value
    return value

async def _insert_default_setting(key: str, value: str):
    """Вставляет настройку по умолчанию в БД (фоново)."""
    try:
        async with db_pool.acquire() as conn:
            await conn.execute(
                "INSERT INTO settings (key, value) VALUES ($1, $2) ON CONFLICT (key) DO NOTHING",
        ...
        logging.error(f"Не удалось вставить настройку по умолчанию {key}: {e}")
...
async def set_setting(key: str, value: str):
    ...
    async with settings_cache_lock:
        ...
        last_settings_update = 0  # сброс, чтобы при следующем get обновилось
...
@db_retry()
async def add_confirmed_chat(chat_id: int, title: str, chat_type: str, confirmed_by: int, conn=None):
    async def _add(conn):
        await conn.execute(
            ...
            chat_id, title, chat_type, datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S"), confirmed_by, datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S")
        )
    if conn:
        await _add(conn)
    else:
...
@db_retry()
async def remove_confirmed_chat(chat_id: int, conn=None):
...
async def create_chat_confirmation_request(chat_id: int, title: str, chat_type: str, requested_by: int):
    async with db_pool.acquire() as conn:
        await conn.execute(
            ...
            chat_id, title, chat_type, requested_by, datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S"), 'pending'
        )
...
@db_retry()
async def update_chat_request_status(chat_id: int, status: str, conn=None):
    ...
    async with db_pool.acquire() as conn:
        ...
        if not exists:
            ...
            await conn.execute(
                ...
                user_id, username, first_name, datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S"),
...
async def add_exp(user_id: int, exp: int, conn=None) -> Optional[str]:
    ...
    async def _add(conn):
        ...
        if not user:
            # Если пользователя нет, создаём его (на всякий случай)
            await ensure_user_exists(user_id, conn=conn)
            user = await conn.fetchrow("SELECT exp, level, balance, reputation FROM users WHERE user_id=$1 FOR UPDATE", user_id)
            if not user:
                logging.error(f"Не удалось создать пользователя {user_id} в add_exp")
                return None
        new_exp = user['exp'] + exp
        level = user['level']
        level_mult = await get_setting_float("level_multiplier")  # используем float
        levels_gained = 0
...
async def get_random_user(exclude_id: int):
    # Используем более эффективный метод с OFFSET
    async with db_pool.acquire() as conn:
...
async def check_global_cooldown(user_id: int, command: str, cooldown_seconds: int = None) -> Tuple[bool, int]:
    ...
    async with db_pool.acquire() as conn:
        ...
        if row and row['last_used']:
            last_used = ensure_aware(row['last_used'])
            diff = datetime.now(timezone.utc) - last_used
...
async def set_global_cooldown(user_id: int, command: str, cooldown_seconds: int = None):
    ...
    async with db_pool.acquire() as conn:
        ...
        ''', user_id, command, datetime.now(timezone.utc))
...
async def collect_business_income(user_id: int, business_id: int, conn=None) -> Tuple[bool, str, float]:
    ...
    async def _update(conn):
        ...
        if last_col:
            last_date = ensure_aware(last_col)
        else:
...
async def finish_heist_joining(heist_id: int, join_until: datetime):
    delay = max(0, (ensure_aware(join_until) - datetime.now(timezone.utc)).total_seconds())
    await asyncio.sleep(delay)
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            split_until = ensure_aware(heist['split_until'])
...
async def process_betray_results(heist_id: int, split_until: datetime):
    delay = max(0, (ensure_aware(split_until) - datetime.now(timezone.utc)).total_seconds())
    await asyncio.sleep(delay)
        ...
        async with conn.transaction():
            ...
            for attacker_id, target_id in assigned.items():
                ...
                await conn.execute(
                    ...
                    heist_id, attacker_id, target_id, success, abs(attacker_share - new_attacker_share), datetime.now(timezone.utc)
                )
                ...
                config = HEIST_TYPES[heist['event_type']]
                # Получаем username внутри транзакции, используя conn
                attacker_username = await get_user_username(attacker_id, conn=conn)
                target_username = await get_user_username(target_id, conn=conn)
                if success:
            ...
            # Начисляем доли на баланс
            for p in final_participants:
                success, _, _ = await update_user_balance(p['user_id'], float(p['current_share']), conn=conn, allow_negative=False)
                if not success:
                    logging.error(f"Не удалось начислить долю {p['current_share']} пользователю {p['user_id']} в налёте {heist_id}")
...
async def check_smuggle_cooldown(user_id: int) -> Tuple[bool, int]:
    async with db_pool.acquire() as conn:
        ...
        if row and row['cooldown_until']:
            cooldown_until = ensure_aware(row['cooldown_until'])
...
async def set_smuggle_cooldown(user_id: int, penalty: int = 0):
    ...
    cooldown_until = datetime.now(timezone.utc) + timedelta(minutes=base + penalty)
    async with db_pool.acquire() as conn:
        await conn.execute('''
...
async def match_orders(conn):
    while True:
        ...
        # Списание комиссии (проверяем успешность)
        if commission_percent > 0:
            if commission_side == 'buyer' or commission_side == 'both':
                ...
                if not success:
                    # Если не удалось списать комиссию, откатываем сделку
                    logging.error(f"Не удалось списать комиссию с покупателя {buyer_id} в сделке buy {buy['id']} и sell {sell['id']}")
                    # Откат не требуется, так как ордера ещё не изменены
                    # Просто пропускаем эту пару и продолжаем
                    # Выйдем из цикла, чтобы не зависнуть
                    break
            if commission_side == 'seller' or commission_side == 'both':
                ...
                if not success:
                    logging.error(f"Не удалось списать комиссию с продавца {seller_id} в сделке buy {buy['id']} и sell {sell['id']}")
                    # Откатываем уже списанную комиссию с покупателя, если была
                    if commission_side == 'buyer' or commission_side == 'both':
...
@db_retry()
async def reset_user_stats(user_id: int, conn=None):
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            await conn.execute(
                ...
                user_id, task_id, datetime.now(timezone.utc)
            )
            await conn.execute(
...
async def create_promocode(code: str, reward: float, reward_type: str, max_uses: int, created_by: int = None):
    async with db_pool.acquire() as conn:
        await conn.execute(
            ...
            code, reward, reward_type, max_uses, datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S"), created_by
        )
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            await conn.execute(
                ...
                user_id, code, datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S")
            )
...
async def recover_heists():
    async with db_pool.acquire() as conn:
        ...
        for h in active_joining:
            join_until = ensure_aware(h['join_until'])
        ...
        for h in active_splitting:
            split_until = ensure_aware(h['split_until'])
...
async def buy_callback(callback: CallbackQuery):
    ...
    logging.info(f"🛒 buy_callback вызван пользователем {user_id} с данными: {callback.data}")

    # Проверка на бан
    try:
        if await is_banned(user_id) and not await is_admin(user_id):
            logging.warning(f"Пользователь {user_id} забанен, покупка отклонена")
            await callback.answer("⛔ Вы заблокированы.", show_alert=True)
            return
    except Exception as e:
        logging.exception(f"Ошибка при проверке бана для {user_id}: {e}")
    ...
    try:
        ...
        logging.exception(f"Ошибка ensure_user_exists для {user_id}: {e}")
    ...
    try:
        ...
        if not ok:
            logging.info(f"Пользователь {user_id} не подписан на каналы")
        ...
        logging.exception(f"Ошибка check_subscription для {user_id}: {e}")
    ...
    try:
        ...
        logging.info(f"ID товара: {item_id}")
    ...
    async with db_pool.acquire() as conn:
        try:
            ...
            logging.info(f"Товар: {name}, цена: {price}, остаток: {stock}")

            if stock != -1 and stock <= 0:
                logging.info(f"Товар {name} отсутствует в наличии")
            ...
            if balance < price:
                logging.info(f"Недостаточно средств: нужно {price}, есть {balance}")
                await callback.answer(
            ...
            # Списание средств и создание покупки
            async with conn.transaction():
                success, new_balance, _ = await update_user_balance(
                    user_id, -price, conn=conn, allow_negative=False
                )
                if not success:
                    logging.error(f"Не удалось списать {price} у пользователя {user_id}")
                ...
                logging.info(f"Покупка успешно завершена для пользователя {user_id}, товар {name}")
    ...
    try:
        ...
        logging.info(f"Сообщение с покупкой удалено для чата {chat_id}")
    except Exception as e:
    ...
    logging.info(f"Покупка для пользователя {user_id} полностью обработана")
...
# ВНИМАНИЕ: хендлеры noop и cancel_action были удалены из этой части
# и перенесены в конец Части 5.2 согласно инструкции.
...
# ==================== КОНЕЦ ЧАСТИ 2 ====================
# ==================== ЧАСТЬ 3.1: ПОЛЬЗОВАТЕЛЬСКИЕ ХЕНДЛЕРЫ (ЛИЧНЫЕ СООБЩЕНИЯ) – ПРОФИЛЬ, БОНУС, ТОП, КАЗИНО (исправленная) ====================
                        ...
                        if referrer_exists and not await is_banned(referrer_id):
                            ...
                            if not existing:
                                await conn.execute(
                                    ...
                                    referrer_id, user_id, datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S"), False
                                )
...
async def profile_handler(message: Message):
    ...
    level_mult = await get_setting_float("level_multiplier")
    ...
    text = (
        f"👤 <b>Твой профиль</b>\n"
        f"📊 <b>Уровень:</b> {level}\n"
        f"📈 <b>Опыт:</b> {exp:.0f}/{exp_needed:.0f}\n{bar}\n"
...
async def bonus_handler(message: Message):
    ...
    async with db_pool.acquire() as conn:
        ...
        last_bonus = ensure_aware(last_bonus)  # приводим к aware
...
async def guess_number_callback(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await callback.message.answer(phrase, reply_markup=repeat_bet_keyboard('guess'))
    await state.clear()
...
async def slots_bet(message: Message, state: FSMContext):
    ...
    try:
        ...
        # Если сообщение удалено – прерываем анимацию, но продолжаем игру
        pass
...
# ==================== КОНЕЦ ЧАСТИ 3.1 ====================
# ==================== ЧАСТЬ 3.2: ПОЛЬЗОВАТЕЛЬСКИЕ ХЕНДЛЕРЫ (ЛИЧНЫЕ СООБЩЕНИЯ) – РУЛЕТКА, ПОВТОР СТАВОК, МАГАЗИН, ПРОМОКОДЫ, ОГРАБЛЕНИЕ, РЕФЕРАЛКА, ЗАДАНИЯ, БИЗНЕСЫ, БИРЖА, РОЗЫГРЫШИ, УНИВЕРСИТЕТ (исправленная) ====================
...
async def process_roulette_bet(message: Message, state: FSMContext, user_id: int):
    ...
    try:
        ...
        # Если сообщение удалено – продолжаем без анимации
        pass
...
async def process_dice_repeat(user_id: int, amount: float, chat_id: int):
    ...
    async with db_pool.acquire() as conn:
        success, new_balance, _ = await update_user_balance(user_id, -amount, conn=conn, allow_negative=False)
        if not success:
            await safe_send_message(chat_id, "❌ Ошибка при списании ставки.")
            return
        await update_user_game_stats(user_id, 'dice', win, conn=conn)
        if win:
            multiplier = 2.0
    ...
    await save_last_bet(user_id, 'dice', amount)
    await safe_send_message(chat_id, phrase, reply_markup=repeat_bet_keyboard('dice'))
...
async def process_guess_repeat(user_id: int, amount: float, number: int, chat_id: int):
    ...
    await save_last_bet(user_id, 'guess', amount, bet_data)
    await safe_send_message(chat_id, phrase, reply_markup=repeat_bet_keyboard('guess'))
...
async def process_slots_repeat(user_id: int, amount: float, chat_id: int):
    ...
    await save_last_bet(user_id, 'slots', amount)
    await safe_send_message(chat_id, phrase, reply_markup=repeat_bet_keyboard('slots'))
...
async def process_roulette_repeat(user_id: int, amount: float, bet_type: str, number: int, chat_id: int):
    ...
    await save_last_bet(user_id, 'roulette', amount, bet_data)
    await safe_send_message(chat_id, phrase, reply_markup=repeat_bet_keyboard('roulette'))

# ==================== УНИВЕРСИТЕТ (ПРОКАЧКА НАВЫКОВ) С ИСПРАВЛЕНИЯМИ ====================
...
async def upgrade_skill_callback(callback: CallbackQuery, state: FSMContext):
    ...
    confirm_kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Да", callback_data=f"upgrade_confirm_{skill}")],
        [InlineKeyboardButton(text="❌ Нет", callback_data="upgrade_cancel")]
    ])
    await callback.message.answer(f"Прокачать {skill} до уровня {current_level+1} за {cost} авторитета?", reply_markup=confirm_kb)
    await state.set_state(UpgradeSkill.confirming)

@dp.callback_query(UpgradeSkill.confirming, F.data.startswith("upgrade_confirm_"))
async def upgrade_confirm_yes(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    skill = callback.data.split("_")[2]
    data = await state.get_data()
    saved_skill = data.get('skill')
    if saved_skill != skill:
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            authority = await get_user_authority(user_id)
            if authority < cost:
                await callback.answer("❌ Недостаточно авторитета.", show_alert=True)
                await state.clear()
                return
            await update_user_authority(user_id, -cost, conn=conn)
            await update_user_skill(user_id, f'skill_{skill}', delta=1, conn=conn)
    try:
        await callback.message.edit_text(f"✅ Навык {skill} успешно прокачан!")
    except TelegramBadRequest:
        await callback.message.answer(f"✅ Навык {skill} успешно прокачан!")
    await state.clear()
    await university_menu(callback.message)

@dp.callback_query(UpgradeSkill.confirming, F.data == "upgrade_cancel")
async def upgrade_confirm_no(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    await state.clear()
    try:
        await callback.message.delete()
...
# Оставляем старый хендлер на случай, если кто-то введёт "да" вручную (но он может быть не нужен)
...
# ==================== МАГАЗИН ПОДАРКОВ (хендлер покупки перенесён в Часть 2) ====================
@dp.message(F.text == "🛒 Магазин подарков")
async def shop_handler(message: Message, page: int = 1):
    ...
    if not ok:
        await message.answer("❗️ Сначала подпишись на каналы.", reply_markup=subscription_inline(not_subscribed))
        return
    # page больше не парсим из текста, используем переданный параметр
    offset = (page - 1) * ITEMS_PER_PAGE
    async with db_pool.acquire() as conn:
...
async def shop_page_callback(callback: CallbackQuery):
    ...
    await shop_handler(callback.message, page=page)
...
async def my_purchases(message: Message, page: int = 1):
    if message.chat.type != 'private':
        return
    user_id = message.from_user.id
    if await is_banned(user_id) and not await is_admin(user_id):
    ...
    for row in rows:
        pid, name, date, status, comment = row['id'], row['name'], ensure_aware(row['purchase_date']).strftime("%Y-%m-%d %H:%M:%S"), row['status'], row['admin_comment']
...
async def mypurchases_page_callback(callback: CallbackQuery):
    await callback.answer()
    page = int(callback.data.split("_")[2])
    await my_purchases(callback.message, page=page)
...
# ==================== ОГРАБЛЕНИЕ ====================
# Функция perform_theft уже определена в части 1.2, здесь только хендлеры
...
async def theft_random(message: Message, state: FSMContext):
    ...
    async with db_pool.acquire() as conn:
        ...
        if last_time_str:
            last_time = ensure_aware(last_time_str)
...
async def task_detail_callback(callback: CallbackQuery):
    ...
    if task['media_file_id'] and task['media_type'] == 'photo':
        ...
        try:
            ...
            await callback.message.answer(text, reply_markup=task_detail_keyboard(task_id, task['button_link']))
...
async def business_view(callback: CallbackQuery):
    ...
    last_col = ensure_aware(biz['last_collection']) if biz['last_collection'] else datetime.now(timezone.utc) - timedelta(days=365)
    now = datetime.now(timezone.utc)
    minutes_passed = int((now - last_col).total_seconds() / 60)
    max_storage_minutes = max_storage_hours * 60
    collectable_minutes = min(minutes_passed, max_storage_minutes)
    ...
    next_collect = last_col + timedelta(minutes=collect_interval)
    if next_collect > now:
        wait_minutes = int((next_collect - now).total_seconds() / 60)
...
# ==================== БИТКОИН-БИРЖА (ПОЛЬЗОВАТЕЛЬСКАЯ ЧАСТЬ) С ИСПРАВЛЕНИЕМ ГОНОК ====================
...
async def buy_from_price(callback: CallbackQuery, state: FSMContext):
    ...
    user_id = callback.from_user.id
    # Вся логика будет внутри транзакции в следующем хендлере
    async with db_pool.acquire() as conn:
...
async def buy_from_price_amount(message: Message, state: FSMContext):
    ...
    data = await state.get_data()
    price = data['price']
    # Входим в транзакцию и повторно выбираем ордера с FOR UPDATE
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            await conn.execute("SET LOCAL statement_timeout = '5s'")
            orders = await conn.fetch(
            ...
            for order in orders_list:
                ...
                # Проверяем, что ордер всё ещё активен (может быть изменён параллельно)
                ...
                if not current or float(current['amount']) < take - 0.0001:
                    continue
                success, _, _ = await update_user_balance(user_id, -take * price, conn=conn, allow_negative=False)
                if not success:
                    logging.error(f"Не удалось списать {take * price} у покупателя {user_id} при покупке BTC")
                    continue
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            for order in orders_list:
                ...
                if not current or float(current['amount']) < take - 0.0001:
                    continue
                success, _ = await update_user_bitcoin(user_id, -take, conn=conn)
                if not success:
                    logging.error(f"Не удалось списать {take} BTC у продавца {user_id} при продаже BTC")
                    continue
...
async def sell_bitcoin_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"Максимальная сумма заявки: {max_input:.4f} BTC.\n"
...
async def my_orders(message: Message, page: int = 1):
    if message.chat.type != 'private':
        return
    user_id = message.from_user.id
    async with db_pool.acquire() as conn:
...
async def my_order_detail(callback: CallbackQuery):
    ...
    text = (
        ...
        f"Создана: {ensure_aware(order['created_at']).strftime('%Y-%m-%d %H:%M')}"
    )
    kb = InlineKeyboardMarkup(inline_keyboard=[
...
async def myorders_page_callback(callback: CallbackQuery):
    await callback.answer()
    page = int(callback.data.split("_")[2])
    await my_orders(callback.message, page=page)
...
async def active_giveaways_user(message: Message, page: int = 1):
    if message.chat.type != 'private':
        return
    user_id = message.from_user.id
    if await is_banned(user_id) and not await is_admin(user_id):
    ...
    for row in rows:
        end_str = ensure_aware(row['end_date']).strftime("%Y-%m-%d %H:%M") if row['end_date'] else "не указано"
...
async def active_giveaway_detail(callback: CallbackQuery):
    ...
    end_str = ensure_aware(gw['end_date']).strftime("%Y-%m-%d %H:%M") if gw['end_date'] else "не указано"
    text = (
        f"🎁 <b>{gw['prize']}</b>\n"
        f"📝 {gw['description']}\n"
        f"⏳ Окончание: {end_str}\n"
...
@dp.callback_query(F.data.startswith("active_gw_page_"))
async def active_gw_page_callback(callback: CallbackQuery):
    await callback.answer()
    page = int(callback.data.split("_")[3])
    await active_giveaways_user(callback.message, page=page)

@dp.message(F.text == "🏁 Завершённые розыгрыши")
async def completed_giveaways_user(message: Message, page: int = 1):
    if message.chat.type != 'private':
        return
    user_id = message.from_user.id
    if await is_banned(user_id) and not await is_admin(user_id):
    ...
    for row in rows:
        end_str = ensure_aware(row['end_date']).strftime("%Y-%m-%d %H:%M") if row['end_date'] else "неизвестно"
        text += f"🎁 #{row['id']} - {row['prize']}\n"
...
async def completed_giveaway_detail(callback: CallbackQuery):
    ...
    end_str = ensure_aware(gw['end_date']).strftime("%Y-%m-%d %H:%M") if gw['end_date'] else "неизвестно"
    text = (
        f"🏁 Розыгрыш #{gw['id']}\n"
        f"🎁 Приз: {gw['prize']}\n"
...
async def completed_gw_page_callback(callback: CallbackQuery):
    await callback.answer()
    page = int(callback.data.split("_")[3])
    await completed_giveaways_user(callback.message, page=page)
...
async def heist_keyword_handler(message: Message):
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            # Пытаемся удалить сообщение с ключевым словом
            if await can_delete_message(message.chat.id, message):
                try:
                    await message.delete()
                except TelegramBadRequest:
            ...
            await conn.execute(
                ...
                "VALUES ($1, $2, $3, $3, 0, $4)",
                heist['id'], user_id, share, datetime.now(timezone.utc)
            )
...
async def cmd_chat_heist_status(message: Message):
    ...
    async with db_pool.acquire() as conn:
        ...
        join_until = ensure_aware(heist['join_until']) if heist['status'] == 'joining' else None
        split_until = ensure_aware(heist['split_until']) if heist['status'] == 'splitting' else None
        now = datetime.now(timezone.utc)
        if heist['status'] == 'joining':
            time_remaining = (join_until - now).total_seconds()
            time_str = format_time_remaining(int(time_remaining)) if time_remaining > 0 else "завершается"
        else:
            time_remaining = (split_until - now).total_seconds()
            time_str = format_time_remaining(int(time_remaining)) if time_remaining > 0 else "завершается"
...
async def cmd_smuggle(message: Message):
    ...
    async with db_pool.acquire() as conn:
        run_id = await conn.fetchval(
            ...
            user_id, datetime.now(timezone.utc), end_time, message.chat.id
        )
    ...
    if file_id:
        try:
            ...
            logging.error(f"Ошибка отправки фото smuggle_start: {e}")
...
async def cmd_jail(message: Message, state: FSMContext):
    ...
    try:
        ...
        await state.update_data(cell_message_id=sent.message_id)
    except Exception as e:
...
async def cmd_my_heist(message: Message):
    ...
    async with db_pool.acquire() as conn:
        ...
        if row['status'] == 'splitting':
            remaining = (ensure_aware(row['split_until']) - now).total_seconds()
            if remaining > 0:
...
async def chat_gift(message: Message):
    ...
    try:
        ...
        await auto_delete_reply(message, "❌ Не удалось получить список администраторов.")
        return

    admin_ids = [a.user.id for a in admins if a.user.id != user_id and a.user.id != bot.id]
    banned_ids = []
    if admin_ids:
        async with db_pool.acquire() as conn:
            rows = await conn.fetch("SELECT user_id FROM banned_users WHERE user_id = ANY($1)", admin_ids)
            banned_ids = [r['user_id'] for r in rows]
    eligible_ids = [uid for uid in admin_ids if uid not in banned_ids]

    if not eligible_ids:
        await auto_delete_reply(message, "❌ Нет подходящих получателей для подарка.")
        return

    recipient_id = random.choice(eligible_ids)
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            last_gift = ensure_aware(user['last_gift_time']) if user['last_gift_time'] else None
            if last_gift and last_gift.date() == today_date:
            ...
            await conn.execute(
                ...
                recipient_id, None, None, datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S")
            )
    ...
    recipient_name = (await get_user_name(recipient_id)) or str(recipient_id)
    await auto_delete_reply(message,
        f"🎁 {message.from_user.first_name} активировал подгон!\n"
        f"Счастливчик: {recipient_name} получает {gift_amount:.2f} баксов! 🎉\n"
...
async def reset_stats_confirm(callback: CallbackQuery, state: FSMContext):
    ...
    if await verify_reset_key(key, uid):
        async with db_pool.acquire() as conn:
            async with conn.transaction():
                await reset_user_stats(uid, conn=conn)
        try:
            await callback.message.edit_text(f"✅ Статистика пользователя {uid} успешно сброшена.")
        except TelegramBadRequest:
            await callback.message.answer(f"✅ Статистика пользователя {uid} успешно сброшена.")
        ...
        try:
            ...
            await callback.message.answer("❌ Ошибка при сбросе (ключ недействителен). Попробуйте снова.")
    await state.clear()
    ...
    try:
        async with db_pool.acquire() as conn:
            await conn.execute(
                ...
                uid, message.from_user.id, datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S"), reason
            )
...
async def admin_purchases(message: Message, page: int = 1):
    ...
    try:
        ...
        for row in rows:
            pid, uid, username, item_name, date, status = row['id'], row['user_id'], row['username'] or "нет username", row['name'], ensure_aware(row['purchase_date']).strftime("%Y-%m-%d %H:%M:%S"), row['status']
...
async def create_promo_reward_type(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await callback.message.answer("Введи максимальное количество использований (целое число):")
...
async def create_promo_max_uses(message: Message, state: FSMContext):
    ...
    try:
        async with db_pool.acquire() as conn:
            await conn.execute(
                "INSERT INTO promocodes (code, reward, reward_type, max_uses, created_at, created_by) VALUES ($1, $2, $3, $4, $5, $6)",
                data['code'], data['reward'], data['reward_type'], max_uses, datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S"), message.from_user.id
            )
...
async def process_chat_id(message: Message, state: FSMContext):
    ...
    async with db_pool.acquire() as conn:
        if action == "confirm":
            ...
            if request:
                await add_confirmed_chat(chat_id, request['title'], request['type'], message.from_user.id, conn=conn)
                await update_chat_request_status(chat_id, 'approved', conn=conn)
                ...
                try:
                    ...
                    await add_confirmed_chat(chat_id, chat.title, chat.type, message.from_user.id, conn=conn)
            ...
            if not request:
                ...
                await state.clear()
                return
            await update_chat_request_status(chat_id, 'rejected', conn=conn)
            ...
            await remove_confirmed_chat(chat_id, conn=conn)
...
async def confirm_chat_callback(callback: CallbackQuery):
    ...
    async with db_pool.acquire() as conn:
        ...
        if not request:
            ...
            return
        await add_confirmed_chat(chat_id, request['title'], request['type'], callback.from_user.id, conn=conn)
        await update_chat_request_status(chat_id, 'approved', conn=conn)
        try:
            await callback.message.edit_text(f"✅ Чат {request['title']} подтверждён.")
        except TelegramBadRequest:
            await callback.message.answer(f"✅ Чат {request['title']} подтверждён.")
        await safe_send_message(request['requested_by'], f"✅ Ваш чат «{request['title']}» активирован!")
    await callback.answer()
...
async def reject_chat_callback(callback: CallbackQuery):
    ...
    async with db_pool.acquire() as conn:
        ...
        try:
            ...
            await callback.message.answer(f"❌ Запрос для чата {request['title']} отклонён.")
        await safe_send_message(request['requested_by'], f"❌ Запрос на активацию чата «{request['title']}» отклонён.")
    await callback.answer()

# ==================== КОНЕЦ ЧАСТИ 5.1 ====================
                   # ==================== ЧАСТЬ 5.2: АДМИНИСТРАТИВНАЯ ПАНЕЛЬ (ПРОДОЛЖЕНИЕ) + ПЕРЕМЕЩЁННЫЕ ХЕНДЛЕРЫ ====================
...
# - Настройки
# - Очистка
# - noop и cancel_action (перенесены из Части 2)
#
...
async def admin_list_businesses(message: Message, page: int = 1):
    ...
    offset = (page - 1) * ITEMS_PER_PAGE
    page_items = types_list[offset:offset + ITEMS_PER_PAGE]

    text = f"🏪 Типы бизнесов (страница {page}):\n\n"
    for bt in page_items:
    ...
    nav_buttons = []
    if page > 1:
        nav_buttons.append(InlineKeyboardButton(text="⬅️", callback_data=f"admin_business_page_{page-1}"))
    if offset + ITEMS_PER_PAGE < total:
        nav_buttons.append(InlineKeyboardButton(text="➡️", callback_data=f"admin_business_page_{page+1}"))
    if nav_buttons:
        kb.append(nav_buttons)
...
@dp.callback_query(F.data.startswith("admin_business_page_"))
async def admin_business_page_callback(callback: CallbackQuery):
    await callback.answer()
    page = int(callback.data.split("_")[3])
    await admin_list_businesses(callback.message, page=page)
...
async def admin_list_orders(message: Message, page: int = 1):
    ...
    for o in rows:
        d = dict(o)
        created = ensure_aware(d['created_at'])
...
async def admin_trade_history(message: Message):
    ...
    for r in rows:
        traded = ensure_aware(r['traded_at'])
        text += f"ID {r['id']}: {float(r['amount']):.4f} BTC @ {r['price']} $ (покупатель {r['buyer_id']}, продавец {r['seller_id']}) в {traded.strftime('%Y-%m-%d %H:%M')}\n"
    parts = safe_split_text(text)
    for part in parts:
...
async def create_giveaway_condition(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if cond_type == 'time':
            await callback.message.answer("Введи дату и время окончания в формате ДД.ММ.ГГГГ ЧЧ:ММ (например, 25.12.2025 18:00):")
            await state.set_state(CreateGiveaway.end_date)
        else:
            await callback.message.answer("Введи минимальное количество участников (целое число):")
            await state.set_state(CreateGiveaway.min_participants)
...
async def admin_end_giveaway(callback: CallbackQuery):
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            participants = await conn.fetch("SELECT user_id FROM participants WHERE giveaway_id=$1", gw_id)
            if not participants:
                await conn.execute("UPDATE giveaways SET status='completed' WHERE id=$1", gw_id)
                await callback.answer("❌ Нет участников, розыгрыш завершён без победителя.", show_alert=True)
                return
    ...
    try:
        async with db_pool.acquire() as conn:
            await conn.execute(
                ...
                uid, message.from_user.id, datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S"), json.dumps(selected)
            )
...
# ==================== ИСПРАВЛЕННЫЙ ХЕНДЛЕР edit_setting_start С StateFilter(None) ====================
@dp.callback_query(F.data.startswith("edit_"), StateFilter(None))
...
# ==================== ПЕРЕМЕЩЁННЫЕ ХЕНДЛЕРЫ ИЗ ЧАСТИ 2 ====================
            ...
            for chat_id, chat_data in confirmed.items():
                try:
                    async with db_pool.acquire() as conn:
                        ...
                        if last_heist:
                            last_heist = ensure_aware(last_heist)
                            if datetime.now(timezone.utc) - last_heist < timedelta(minutes=interval_minutes):
                                continue
