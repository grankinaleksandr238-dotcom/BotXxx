...
# --- Объединённые состояния для заданий ---
class TaskManagement(StatesGroup):
    action = State()          # 'create', 'list', 'delete'
    # для создания:
    name = State()
    description = State()
    task_type = State()
...
def admin_main_keyboard(permissions: List[str]) -> ReplyKeyboardMarkup:
    """Главное меню админ-панели (компактное, с подменю)."""
    builder = ReplyKeyboardBuilder()
    row1 = []
...
# Подменю для розыгрышей (админка)
...
# Остальные админские клавиатуры (можно оставить как есть или тоже оптимизировать)
...
# Обратите внимание: в админ-меню кнопка "📋 Задания" ведёт в подменю, но сами операции вынесены сюда.
...
async def create_task_start(message: Message, state: FSMContext):
    ...
    await state.set_state(TaskManagement.name)  # используем объединённое состояние
...
# ==================== КАТЕГОРИИ НАСТРОЕК ====================
# Определяем здесь, так как они используются только в этом разделе
SETTINGS_CATEGORIES = {
    "⚙️ Казино": [
...
async def stats_handler(message: Message):
    ...
    try:
        ...
        # Возвращаемся в главное меню админки (оно в части 6, но там есть кнопка "Назад в админку", а здесь просто покажем статистику и предложим вернуться)
        permissions = await get_admin_permissions(message.from_user.id)
