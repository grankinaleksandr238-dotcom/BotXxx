...
# 3. В match_orders добавлена проверка успешности списания комиссии и сама функция.
...
# ИСПРАВЛЕНО: добавлена функция match_orders
...
# ИСПРАВЛЕНО: в complete_task добавлен conn в update_user_reputation
...
# ИСПРАВЛЕНО: 
# - в heist_keyword_handler добавлена проверка на администратора чата
...
async def heist_keyword_handler(message: Message):
    ...
    # ИСПРАВЛЕНО: проверка, что пользователь не администратор чата
    try:
    ...
    try:
        async with db_pool.acquire() as conn:
            async with conn.transaction():
                ...
                # ИСПРАВЛЕНО: обработка ошибки удаления
                try:
                    if await can_delete_message(message.chat.id, message):
                        await message.delete()
                except Exception as e:
...
# ИСПРАВЛЕНО: добавлены пользовательские хендлеры для фармилок.
...
# (Весь код из предыдущей версии, включая исправленные состояния EditGiveaway)
