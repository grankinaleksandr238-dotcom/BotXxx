...
import json
import html
from datetime import datetime, timedelta, date  # ИСПРАВЛЕНО: добавлен date
...
DEFAULT_SETTINGS = {
    ...
    # ----- СТАТЫ ЗА УРОВЕНЬ (не используются) -----
...
# ==================== ТИПЫ СОБЫТИЙ (НАЛЁТОВ) С ДОБАВЛЕННЫМИ ФРАЗАМИ ====================
HEIST_TYPES = {
    "incassator": {
        "name": "🚐 Инкассатор",
        "keyword": "ФАРТ",
...
async def collect_business_income(user_id: int, business_id: int) -> Tuple[bool, str]:
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            now = datetime.now()
            delta = now - last_date
            # ИСПРАВЛЕНО: проверка на отрицательную разницу
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            if success:
                ...
                # ИСПРАВЛЕНО: ограничение defense_bonus до 30
...
# ----- НОВЫЕ СОСТОЯНИЯ ДЛЯ КИДАЛОВА В НАЛЁТЕ (ИСПРАВЛЕНО НАЗВАНИЕ) -----
class HeistBetray(StatesGroup):      # ИСПРАВЛЕНО: раньше было HeistBetrat
    heist_id = State()
    target_id = State()
    confirming = State()
...
# УДАЛЕНО: university_keyboard (не используется, чтобы избежать await в синхронной функции)

# ==================== КОНЕЦ ЧАСТИ 2 ====================
# ==================== ЧАСТЬ 3: ПОЛЬЗОВАТЕЛЬСКИЕ ХЕНДЛЕРЫ ====================
...
async def universal_back_handler(message: types.Message, state: FSMContext):
    ...
    elif current_state.startswith('HeistBetray'):   # ИСПРАВЛЕНО: было HeistBetrat
        await state.finish()
    ...
    try:
        async with db_pool.acquire() as conn:
            async with conn.transaction():
                ...
                # ИСПРАВЛЕНО: убрано дублирование last_theft_time (оно уже установлено в блоке защиты и в блоке успеха)
                # Если ни защита, ни успех не сработали (например, victim_balance=0), то нужно установить время
                await conn.execute("UPDATE users SET last_theft_time = $1 WHERE user_id=$2", datetime.now().strftime("%Y-%m-%d %H:%M:%S"), robber_id)
...
async def theft_target_entered(message: types.Message, state: FSMContext):
    ...
    if not target_data:
        ...
        await state.finish()  # ИСПРАВЛЕНО: добавлен finish
        return
...
async def buy_business_confirm(message: types.Message, state: FSMContext):
    ...
    if message.text.lower() == 'да':
        ...
        try:
            ...
            # ИСПРАВЛЕНО: можно сразу показать список бизнесов
            await my_businesses(message)
        except Exception as e:
...
async def betray_target_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    # Если пользователь захочет выйти, он нажмёт Отмена
...
async def cmd_smuggle(message: types.Message):
    ...
    # ИСПРАВЛЕНО: сначала вставляем рейс, потом устанавливаем кулдаун
    async with db_pool.acquire() as conn:
