...
async def keep_db_alive():
    """Периодически пингует БД, чтобы соединение не закрывалось, и переподключается при сбоях."""
    while True:
        try:
...
async def update_user_balance(user_id: int, delta: float, conn=None):
    ...
    async def _update(conn):
        # Устанавливаем таймаут на транзакцию
...
async def find_user_result(message: Message, state: FSMContext):
    ...
    await message.answer(text)
    await state.clear()  # ← ИСПРАВЛЕНО: сбрасываем состояние
...
async def export_users(message: Message):
    ...
    try:
        ...
        await message.answer_document(
            InputFile(io.BytesIO(csv_data), filename="users.csv"),  # ← ИСПРАВЛЕНО: использован InputFile
...
async def shopitems_page_callback(callback: CallbackQuery):
    page = int(callback.data.split("_")[2])
    # ← ИСПРАВЛЕНО: передаём page как аргумент (функция list_shop_items поддерживает page через параметр message)
...
async def admin_purchases(message: Message, page: int = 1):
    if not await check_admin_permissions(message.from_user.id, "manage_shop"):
        return
    # page может быть передан из callback
    try:
        parts = message.text.split()
        if len(parts) > 1 and not page:
            page = int(parts[1])
    except:
        pass
    offset = (page - 1) * ITEMS_PER_PAGE
...
async def admin_purchases_page_callback(callback: CallbackQuery):
    page = int(callback.data.split("_")[3])
    # ← ИСПРАВЛЕНО: вызываем функцию с параметром page
