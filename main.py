...
def convert_value(value, column_name, table_name):
    ...
    # Список полей, которые должны быть STRING (TEXT), даже если похожи на дату
    text_fields = {
        'users': ['joined_date'],
        'admins': ['added_date'],
        'banned_users': ['banned_date'],
        'chat_confirmation_requests': ['request_date'],
        'confirmed_chats': ['joined_date', 'confirmed_date'],
        'giveaways': ['description', 'prize', 'media_file_id', 'media_type', 'winners_list'],
        'promo_activations': ['activated_at'],
        'promocodes': ['created_at'],
        'referrals': ['referred_date'],
        'tasks': ['created_at', 'description', 'name', 'target_id', 'button_link', 'media_file_id', 'media_type']
    }
    
    # Если поле должно быть текстовым - возвращаем как есть
    if table_name in text_fields and column_name in text_fields[table_name]:
        return str(value) if value else ''
    
    # Словарь соответствия для числовых полей
    type_map = {
        'users': {
            'user_id': int,
            'balance': float,
        ...
        'business_types': {
            ...
            'max_level': int,
            'available': lambda x: str(x).lower() in ('true', 't', 'yes', '1', 'true'),
            'lifetime_hours': int
        },
        'channels': {
            'id': int
        ...
        'confirmed_chats': {
            ...
            'confirmed_by': int,
            'gift_count_today': int,
            'auto_delete_enabled': lambda x: str(x).lower() in ('true', 't', 'yes', '1', 'true'),
        ...
        'giveaways': {
            ...
            'notified': lambda x: str(x).lower() in ('true', 't', 'yes', '1', 'true'),
            'status': str
        },
        'global_cooldowns': {
            'user_id': int
        ...
        'media': {
            'file_id': str,  # обязательно строка, не NULL
            'description': str
        },
        'participants': {
            'user_id': int,
        ...
        'referrals': {
            ...
            'referred_id': int,
            'clicks': int,
            'reward_given': lambda x: str(x).lower() in ('true', 't', 'yes', '1', 'true'),
            'active': lambda x: str(x).lower() in ('true', 't', 'yes', '1', 'true')
        },
        'reset_keys': {
            'user_id': int
        },
...
def parse_date(date_str):
    """Парсит дату, но только для timestamp полей"""
    ...
    return date_str  # возвращаем как строку, если не удалось распарсить
...
async def process_restore(callback: types.CallbackQuery):
    ...
    try:
        ...
        order = ['business_types', 'settings', 'level_rewards', 'users', 'admins', 'bitcoin_orders', 
                 'channels', 'chat_confirmation_requests', 'confirmed_chats', 'giveaways',
                 'global_cooldowns', 'heists', 'jail_sentences', 
            ...
            for row in rows:
                try:
                    ...
                    for key, value in row.items():
                        ...
                        # Пропускаем NULL file_id для media
                        if table_name == 'media' and key == 'file_id' and (not value or value == ''):
                            continue
                        
                        # Определяем, нужно ли парсить дату
                        is_timestamp_field = key in ['last_bonus', 'last_theft_time', 'last_gift_time', 
                                                    'created_at', 'traded_at', 'start_time', 'end_time',
                                                    'last_heist_time', 'updated_at', 'purchase_date',
                                                    'warned_at', 'joined_at', 'expires_at', 'purchased_at',
                                                    'last_collection']
                        
                        if is_timestamp_field and value:
                            parsed = parse_date(value)
                            if isinstance(parsed, datetime):
                                clean_row[key] = parsed
                                continue
