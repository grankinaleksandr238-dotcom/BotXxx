...
class GlobalCooldownMiddleware(BaseMiddleware):
    async def __call__(self, handler, event: Message, data: dict):
        ...
        if event.text and event.text.startswith('/'):
            ...
            whitelist = ['/mlb_profile', '/mlb_top', '/help', '/start', '/mlb_heist', '/myheist', '/mlb_fights']  # добавлена команда для боёв
    ...
    async with db_pool.acquire() as conn:
        ...
        await conn.execute('''
            CREATE TABLE IF NOT EXISTS heists (
                ...
                message_id BIGINT   -- НОВОЕ: ID сообщения с кнопкой участия
            )
        ''')
...
async def finish_fight(fight_id: int):
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            f1 = await conn.fetchrow("SELECT strength, agility, stamina FROM fighters WHERE id = $1", fight['fighter1_id'])
            f2 = await conn.fetchrow("SELECT strength, agility, stamina FROM fighters WHERE id = $1", fight['fighter2_id'])
            if not f1 or not f2:
            ...
            if fight['chat_id'] and fight['message_id']:
                winner_name = await conn.fetchval("SELECT name FROM fighters WHERE id = $1", winner_id)
                text = f"🥊 Бой завершён! Победитель: {winner_name}!\n"
                text += f"Всего ставок: {len(bets)}. Спасибо за участие!"
                try:
                    await bot.edit_message_text(text, fight['chat_id'], fight['message_id'])
                except Exception as e:
                    logging.error(f"Failed to edit fight message: {e}")

            # Логируем комиссию (можно сохранять в отдельную таблицу)
...
async def get_chat_stats(chat_id: int = None) -> dict:
    ...
    async with db_pool.acquire() as conn:
        if chat_id:
            ...
            return {'chat_id': chat_id, 'users_count': 0, 'activity': 0}
        else:
            # Общая статистика по всем чатам
            confirmed = await get_confirmed_chats()
            total_chats = len(confirmed)
            total_users_in_chats = 0  # пока не реализовано
            # Можно добавить агрегацию по активности из логов
            return {
                'total_chats': total_chats,
                'total_users': total_users_in_chats,
                'active_today': 0,
            }
...
def admin_main_keyboard(permissions: List[str]) -> ReplyKeyboardMarkup:
    """Главное меню админ-панели с категориями (без длинных списков)."""
    builder = ReplyKeyboardBuilder()
    # Категории: Пользователи, Магазин, Розыгрыши
    row1 = []
    ...
    # Категории: Задания, Статистика, Рассылка
    row4 = []
...
def settings_categories_keyboard() -> ReplyKeyboardMarkup:
    ...
    return ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text="⚙️ Подпольные бои")],  # новая категория
            [KeyboardButton(text="◀️ Назад в админку")]
        ],
        resize_keyboard=True
    )
...
def admin_fights_keyboard() -> ReplyKeyboardMarkup:
    """Меню управления подпольными боями."""
    return ReplyKeyboardMarkup(
        keyboard=[
...
async def universal_back_handler(message: Message, state: FSMContext):
    ...
    # Новые состояния для боёв
         ...
         current_state.startswith('CreateFight') or current_state.startswith('PlaceBet'):
        await state.clear()
        await message.answer("Главное меню:", reply_markup=main_menu_keyboard(is_admin_user))
...
# ==================== НОВЫЙ ХЕНДЛЕР ДЛЯ ПОДПОЛЬНЫХ БОЁВ (начальный) ====================
...
# Обработчик для просмотра деталей боя (вызывается из инлайн-кнопки)
...
async def fights_back_callback(callback: CallbackQuery):
    ...
    # Для этого нужно создать объект Message из callback
    # Проще вызвать хендлер заново, но нужно передать сообщение. Создадим фиктивное сообщение.
    # Или просто отредактируем текущее сообщение, показав список.
    # Вызовем underground_fights_menu с callback.message
    # Но underground_fights_menu ожидает Message, а не CallbackQuery. Можно создать фиктивный message.
    # Проще скопировать логику.
    user_id = callback.from_user.id
    fights = await get_active_fights()
    if not fights:
...
# Обработчик выбора бойца для ставки (переход к вводу суммы)
...
async def bet_amount_handler(message: Message, state: FSMContext):
    ...
    # Дополнительная проверка, что бой ещё активен
    fight = await get_fight(fight_id)
    if not fight or fight['status'] != 'active':
    ...
    await state.update_data(amount=amount)
    # Рассчитаем коэффициент для отображения
    total1 = float(fight['total_bets_fighter1'])
    total2 = float(fight['total_bets_fighter2'])
    commission = fight['commission_percent']
    odds = calculate_odds(total1, total2, commission, fighter_id == fight['fighter1_id'])
...
async def bet_confirm_callback(callback: CallbackQuery, state: FSMContext):
    ...
    if success:
        ...
        # Отправляем обновлённую информацию о бое в чат? Нет, это ЛС.
        # Можно обновить сообщение в группе, но это будет сделано фоновой задачей.
    else:
        await callback.message.edit_text(msg)
    await state.clear()
    await callback.answer()
...
async def heist_join_callback(callback: CallbackQuery):
    ...
    ok, remaining = await check_global_cooldown(user_id, "chat_activity", cooldown_hours * 3600)
    if not ok:
        await callback.answer(f"⏳ Глобальный кулдаун! Осталось {format_time_remaining(remaining)}.", show_alert=True)
        return
        ...
        async with conn.transaction():
            ...
            if heist['message_id']:
                try:
                    ...
                    # Для простоты просто добавляем строку в конец (если её ещё нет)
...
async def admin_tasks_menu(message: Message):
    if not await check_admin_permissions(message.from_user.id, "manage_users"):  # используем manage_users для заданий
        await message.answer("❌ Недостаточно прав.")
        return
    # Здесь можно было бы сделать отдельное меню, но для простоты используем reply-клавиатуру admin_main_keyboard уже содержит кнопки +Задание и Список заданий
    # Поэтому просто вызовем нужные хендлеры по тексту кнопок. Они уже есть.
...
async def create_fight_fighter2(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"Выбраны бойцы: {data['fighter1_name']} и {fighter['name']}.\n"
        f"Теперь введи дату и время начала боя в формате ГГГГ-ММ-ДД ЧЧ:ММ (например, 2025-04-15 20:00):"
    )
    await state.set_state(CreateFight.start_date)

@dp.message(CreateFight.start_date, F.text)
async def create_fight_start_time(message: Message, state: FSMContext):
    if message.text == "◀️ Назад":
        await state.clear()
    ...
    await message.answer("Введи длительность боя в минутах (по умолчанию 60):", reply_markup=back_keyboard())
    await state.set_state(CreateFight.duration)

@dp.message(CreateFight.duration, F.text)
async def create_fight_duration(message: Message, state: FSMContext):
    if message.text == "◀️ Назад":
        await state.clear()
        await admin_fight_management(message)
        return
    ...
    text = (
        ...
        f"Начало: {data['start_time'].strftime('%Y-%m-%d %H:%M')}\n"
...
async def create_fight_confirm(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await callback.message.edit_text(f"✅ Бой успешно создан! ID: {fight_id}")
    except Exception as e:
...
# ==================== НОВАЯ ФОНОВАЯ ЗАДАЧА: ОБРАБОТКА ПОДПОЛЬНЫХ БОЁВ ====================
async def fight_scheduler():
    """
    - Переводит запланированные бои в активные в указанное время.
    ...
    while True:
        try:
            ...
            async with db_pool.acquire() as conn:
                # 1. Запланированные бои, которые должны начаться
                scheduled = await conn.fetch("""
                    SELECT * FROM fights
                    WHERE status = 'scheduled' AND start_time <= $1
                """, now)

                for fight in scheduled:
                    fight_id = fight['id']
                    # Переводим в активные
                    await conn.execute("UPDATE fights SET status = 'active' WHERE id = $1", fight_id)
                    # Получаем информацию о бойцах
                    f1 = await get_fighter(fight['fighter1_id'])
                    f2 = await get_fighter(fight['fighter2_id'])
                    if not f1 or not f2:
                        logging.error(f"Fight {fight_id}: fighters not found")
                        continue

                    # Отправляем анонс во все подтверждённые чаты (или в выбранный чат, если указан)
                    # По умолчанию отправляем во все
                    confirmed = await get_confirmed_chats()
                    text = (
                        f"🥊 <b>БОЙ НАЧАЛСЯ!</b>\n\n"
                        f"{f1['name']} vs {f2['name']}\n"
                        f"⏳ Длительность: {fight['duration_minutes']} минут\n"
                        f"💰 Делайте ставки!\n\n"
                        f"Коэффициенты обновляются автоматически."
                    )
                    for chat_id in confirmed.keys():
                        try:
                            msg = await bot.send_message(chat_id, text, reply_markup=active_fights_keyboard([fight]))
                            # Сохраняем chat_id и message_id для последующего обновления
                ...
                # 2. Активные бои, которые должны закончиться
...
async def on_startup():
    ...
    asyncio.create_task(fight_scheduler())  # Новая задача
