...
# Список супер-админов (имеют абсолютные права)
SUPER_ADMINS = []
...
# ГАРАНТИРОВАННО добавляем ваш ID (8127013147) в супер-админы
YOUR_ID = 8127013147
...
async def redis_get(key: str) -> Optional[str]:
    ...
    try:
        ...
        if value is None:
            return None
        # Если значение получено в байтах, декодируем
...
# *** ИСПРАВЛЕНО: добавлены ключи 'purchase', 'promo', 'business' ***
MEDIA_KEYS = [
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt == max_retries - 1:
                        # Вместо проброса исключения возвращаем None (или можно пробросить, но тогда выше надо ловить)
                        # Лучше пробросить, чтобы выше знали об ошибке
                        raise
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            for attacker_id, target_id in assigned.items():
                ...
                # Отправляем сообщение о кидалове в чат
                config = HEIST_TYPES[heist['event_type']]
                if success:
