...
class GlobalCooldownMiddleware:
    async def __call__(self, handler, event: Message, data: dict):
        if not isinstance(event, Message) or event.chat.type == 'private':
            return await handler(event, data)
        user_id = event.from_user.id
...
async def process_betray_results(heist_id: int, split_until: datetime):
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            assigned = {}
            # Индексы участников для быстрого доступа
            participants_indices = {p['user_id']: i for i, p in enumerate(participants)}
            # Множество доступных жертв (индексы)
...
# ==================== ФУНКЦИЯ ДЛЯ БЛОКИРОВОК ====================
async def acquire_lock(lock_name: str, timeout: int = 10) -> bool:
    """Пытается захватить блокировку в Redis. Если Redis недоступен, возвращает True (отсутствие блокировки)."""
    if not redis_client:
        return True
...
@dp.callback_query(F.data.startswith("promo_type_"), state=CreatePromocode.reward_type)
...
async def add_business_max_level(message: Message, state: FSMContext):
    ...
    await AddBusiness.lifetime_hours.set()
...
@dp.callback_query(F.data.startswith("giveaway_cond_"), state=CreateGiveaway.condition_type)
...
async def create_giveaway_min_participants(message: Message, state: FSMContext):
    ...
    await CreateGiveaway.winners_count.set()
...
@dp.message(CreateGiveaway.winners_count, F.text)  # этот хендлер уже есть выше, но он перекрывает? Нет, он для другого пути.
# Чтобы избежать дублирования, сделаем единый хендлер для winners_count, который вызывается из обоих путей.
# У нас уже есть хендлер для winners_count, который обрабатывает любое сообщение в этом состоянии.
# Он будет работать и после end_date, так как мы устанавливаем состояние winners_count.
# Поэтому дополнительный хендлер не нужен. Оставим как есть.
...
# ----- Редактирование розыгрышей (дописаны недостающие хендлеры) -----
...
async def edit_giveaway_end_date(message: Message, state: FSMContext):
    ...
    gw_id = data['giveaway_id']
    async with db_pool.acquire() as conn:
        await conn.execute("UPDATE giveaways SET end_date=$1 WHERE id=$2", end_date.strftime("%Y-%m-%d %H:%M:%S"), gw_id)
...
# Все функции и переменные из частей 1-2 и 5 предполагаются доступными
...
# (Уже реализовано выше в JailProcess)
...
# Все функции и переменные из частей 1-3 и 5 предполагаются доступными
...
# ==================== НОВАЯ ФОНОВАЯ ЗАДАЧА: СПИСАНИЕ ПРОСРОЧЕННЫХ БИЗНЕСОВ ====================
