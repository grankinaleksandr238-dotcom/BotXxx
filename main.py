...
# ВОССТАНОВЛЕНЫ ПОЛНЫЕ ОРИГИНАЛЬНЫЕ ФРАЗЫ (без изменений)
HEIST_TYPES = {
    "incassator": {
        "name": "🚐 Инкассатор",
        "keyword": "ФАРТ",   # оставляем для совместимости, но теперь используем кнопки
        "phrases_start": [
    ...
    async with db_pool.acquire() as conn:
        ...
        await conn.execute('''
            CREATE TABLE IF NOT EXISTS warnings (
                ...
                UNIQUE(user_id, chat_id, warned_at)  -- чтобы можно было несколько, но избежать дубликатов в одну секунду
            )
        ''')
        ...
        await conn.execute("CREATE INDEX IF NOT EXISTS idx_warnings_user_chat ON warnings(user_id, chat_id)")  # новый индекс
...
# ==================== ФУНКЦИИ ДЛЯ ПРЕДУПРЕЖДЕНИЙ (НОВЫЕ) ====================
@db_retry()
...
async def spawn_heist(chat_id: int):
    ...
    try:
        if file_id:
            msg = await bot.send_photo(chat_id, file_id, caption=text, reply_markup=kb)
        else:
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            if not participants:
                ...
                # Редактируем сообщение, чтобы показать отмену
                try:
                    await bot.edit_message_text(
                        "❌ Никто не присоединился к налёту. Он отменён.",
...
async def create_bitcoin_order(user_id: int, order_type: str, amount: float, price: int) -> int:
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            # Убрал SET LOCAL statement_timeout, чтобы избежать ошибок прав
            if order_type == 'sell':
...
# ИСПРАВЛЕНО: функция match_orders с корректной логикой
async def match_orders(conn):
    """Сопоставляет заявки на бирже"""
    while True:
        # Лучшая заявка на покупку (макс цена, потом по времени)
        ...
        """)
        # Лучшая заявка на продажу (мин цена, потом по времени)
        ...
        price = sell['price']  # Цена продавца определяет сделку
        total = amount * price
...
class JailProcess(StatesGroup):
    cell = State()        # теперь пользователь вводит число
    article = State()
...
# НОВЫЕ СОСТОЯНИЯ ДЛЯ ПРЕДУПРЕЖДЕНИЙ
...
def business_main_keyboard(businesses: List[dict]) -> ReplyKeyboardMarkup:
    """Клавиатура со списком фармов пользователя."""
    builder = ReplyKeyboardBuilder()
...
def business_actions_keyboard() -> ReplyKeyboardMarkup:
    """Клавиатура действий с выбранным фармом."""
    return ReplyKeyboardMarkup(
        keyboard=[
...
def business_buy_keyboard(business_types: List[dict]) -> ReplyKeyboardMarkup:
    """Клавиатура для выбора фарма при покупке."""
    builder = ReplyKeyboardBuilder()
...
def admin_main_keyboard(permissions: List[str]) -> ReplyKeyboardMarkup:
    ...
    if "manage_users" in permissions:
        ...
        row4.append(KeyboardButton(text="❌ Удалить задание"))
...
def admin_users_keyboard() -> ReplyKeyboardMarkup:
    ...
    return ReplyKeyboardMarkup(
        keyboard=[
            ...
            # НОВЫЕ КНОПКИ ДЛЯ ПРЕДУПРЕЖДЕНИЙ
...
def settings_categories_keyboard() -> ReplyKeyboardMarkup:
    ...
    return ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text="⚙️ Предупреждения")],   # новая категория
            [KeyboardButton(text="◀️ Назад в админку")]
        ],
        resize_keyboard=True
    )
...
# Клавиатура для тюрьмы больше не нужна (ввод числа), оставляем только для отмены
    ...
    if current_state.startswith('DiceBet') or current_state.startswith('GuessBet') or \
       current_state.startswith('SlotsBet') or current_state.startswith('RouletteBet'):
        ...
        await casino_menu(message)
         ...
         current_state.startswith('WarnUser') or current_state.startswith('ClearWarnings'):  # добавлено
        await state.clear()
...
# ==================== КОНЕЦ ЧАСТИ 3 ====================
# ==================== ЧАСТЬ 4: ПОЛЬЗОВАТЕЛЬСКИЕ ХЕНДЛЕРЫ – РУЛЕТКА, ПОВТОР СТАВОК, УНИВЕРСИТЕТ, МАГАЗИН, ПРОМОКОДЫ, РЕФЕРАЛКА, ЗАДАНИЯ, РОЗЫГРЫШИ, БИРЖА, ОГРАБЛЕНИЯ ====================
...
# ==================== НАЛЁТЫ (НОВАЯ РЕАЛИЗАЦИЯ С КНОПКОЙ) ====================
...
async def heist_join_callback(callback: CallbackQuery):
    ...
    # Проверка глобального кулдауна чата
    ...
    try:
        async with db_pool.acquire() as conn:
            async with conn.transaction():
                ...
                # Отправляем сообщение о присоединении (можно в чат или в ответ на нажатие)
                config = HEIST_TYPES[heist['event_type']]
                phrase = get_random_phrase(config.get('phrases_join', ["✅ {name} присоединился к налёту!"]), name=name)

                # Отправляем в чат (не удаляем, т.к. это часть игры)
                await bot.send_message(chat_id, phrase)

                # Обновляем сообщение с налётом (количество участников)
                ...
                try:
                    await bot.edit_message_text(
                        ...
                        chat_id=chat_id,
                        message_id=heist['message_id'],
                        reply_markup=callback.message.reply_markup  # сохраняем кнопку
                    )
                except:
                    pass
...
async def cmd_jail(message: Message, state: FSMContext):
    ...
    try:
        await bot.send_message(
            ...
            reply_markup=back_keyboard()  # обычная клавиатура с кнопкой "Назад"
        )
...
async def jail_article_input(message: Message, state: FSMContext):
    ...
    # Получаем настройки длительности
    ...
    # Проверка на золотой билет (очень редкий)
    golden_ticket_chance = await get_setting_float("golden_ticket_chance")  # в процентах, например 0.1
...
async def chat_gift(message: Message):
    """Команда для администраторов чата: выдача подарка случайному участнику."""
    if not await check_chat(message):
        return
    user_id = message.from_user.id
    if await is_banned(user_id) and not await is_admin(user_id):
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            # Получаем список администраторов чата, чтобы исключить их из получателей
            try:
            ...
            # Выбираем случайного получателя, не являющегося администратором чата и не забаненного
            ...
            # Убеждаемся, что получатель существует в users
            ...
            # Отправляем результат (системное сообщение не удаляем)
...
# Эти команды доступны администраторам чата (не обязательно администраторам бота)
...
async def warn_user(message: Message):
    """Выдать предупреждение пользователю. Использование: /warn @username [причина]"""
    if not await check_chat(message):
        return
    ...
    if warnings_count >= warnings_per_ban:
        # Блокируем пользователя в чате (кикаем или баним)
        try:
            await bot.ban_chat_member(message.chat.id, target_id)
            text += f"\n⛔ Пользователь заблокирован в чате за превышение лимита предупреждений."
            await clear_warnings(target_id, message.chat.id)  # очищаем предупреждения после бана
        except Exception as e:
...
async def show_warnings(message: Message):
    """Показать предупреждения пользователя. Использование: /warnings [@username]"""
    if not await check_chat(message):
        return
    if not await is_chat_admin(message.chat.id, message.from_user.id):
        await auto_delete_command(message, "❌ Только администраторы чата могут использовать эту команду.")
    ...
    if len(args) > 1:
        ...
        target_id = message.from_user.id  # можно смотреть свои, если админ

    warnings = await get_user_warnings(target_id, message.chat.id)
    if not warnings:
        text = f"У пользователя {target['first_name'] if target else 'тебя'} нет предупреждений."
    else:
        text = f"📋 Предупреждения для {target['first_name'] if target else 'тебя'}:\n"
        for w in warnings:
...
async def clear_warnings_cmd(message: Message):
    """Очистить предупреждения пользователя. Использование: /clearwarnings @username"""
    if not await check_chat(message):
        return
    if not await is_chat_admin(message.chat.id, message.from_user.id):
        await auto_delete_command(message, "❌ Только администраторы чата могут использовать эту команду.")
...
# ==================== КОНЕЦ ЧАСТИ 5 ====================
# ==================== ЧАСТЬ 6: АДМИНИСТРАТИВНАЯ ПАНЕЛЬ (ПОЛНАЯ) ====================
...
# Эти хендлеры добавляют возможность управлять предупреждениями через админ-панель (глобально)
...
async def admin_warn_reason(message: Message, state: FSMContext):
    ...
    # Глобальное предупреждение — добавляем запись с chat_id = 0 (означает глобальное)
    try:
        async with db_pool.acquire() as conn:
            await conn.execute(
                "INSERT INTO warnings (user_id, chat_id, warned_by, reason) VALUES ($1, $2, $3, $4)",
...
async def admin_list_warnings(message: Message, state: FSMContext):
    ...
    # Можно создать отдельное состояние, но для простоты используем FindUser
    await state.set_state(FindUser.query)  # переиспользуем существующее состояние
    # Мы перехватим результат в хендлере find_user_result? Нет, там другой вывод. Создадим временный хендлер.
    # Но чтобы не усложнять, просто выведем предупреждения после ввода.
    # Для этого нужно изменить логику. Лучше сделать отдельное состояние.
    # Однако для экономии времени можно временно сохранить user_id в данных и потом обработать.
...
# ==================== КОНЕЦ ЧАСТИ 6 ====================
# ==================== ЧАСТЬ 7: ПОЛЬЗОВАТЕЛЬСКИЕ ХЕНДЛЕРЫ – ФАРМЫ, БИРЖА, ОГРАБЛЕНИЯ, УНИВЕРСИТЕТ, МАГАЗИН, ПРОМОКОДЫ, РЕФЕРАЛКА, ЗАДАНИЯ, РОЗЫГРЫШИ ====================
...
async def log_cleaner():
    """Удаляет старые записи из логов каждые 3 часа (короткий срок хранения)."""
    while True:
        await asyncio.sleep(10800)  # 3 часа
        try:
            async with db_pool.acquire() as conn:
                # Удаляем старые записи из таблиц, которые можно считать логами
                cutoff = datetime.now() - timedelta(hours=6)  # храним 6 часов
                ...
                # Оставляем остальное как есть (пользователи, покупки и т.д.)
