# ==================== ЧАСТЬ 1: ИМПОРТЫ, НАСТРОЙКИ, БД, ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ (ИТОГОВАЯ ВЕРСИЯ) ====================
...
def betray_target_keyboard(participants: List[Dict], heist_id: int):
    """
    Создаёт клавиатуру для выбора цели в кидалове.
    participants: список словарей с ключами user_id, username (может быть None), first_name
    """
    kb = []
    for p in participants:
        user_id = p['user_id']
        # Формируем отображаемое имя: username, если есть, иначе first_name, иначе ID
        if p.get('username'):
            display_name = f"@{p['username']}"
        elif p.get('first_name'):
            display_name = p['first_name'][:15]  # обрежем длинное имя
        else:
...
async def get_level_reward_coins(level: int) -> float:
    """Возвращает количество монет за указанный уровень."""
    async with db_pool.acquire() as conn:
...
async def get_level_reward_rep(level: int) -> int:
    """Возвращает количество репутации за указанный уровень."""
    async with db_pool.acquire() as conn:
...
async def bonus_handler(message: Message):
    ...
    async with db_pool.acquire() as conn:
        ...
        bonus = random.randint(3, 12)  # уменьшенный бонус
...
async def show_top(message: Message, order_field: str, title: str):
    page = 1
    try:
...
async def dice_bet(message: Message, state: FSMContext):
    ...
    async with db_pool.acquire() as conn:
        ...
        if win:
            multiplier = 2.0  # можно оставить фиксированным или добавить в настройки
...
async def guess_number_callback(callback: CallbackQuery, state: FSMContext):
    ...
    # Получаем настройки с fallback на случай их отсутствия в части 1
    ...
    if rep_reward == 0:
        rep_reward = 1  # значение по умолчанию
...
async def slots_start(message: Message, state: FSMContext):
    if message.chat.type != 'private':
        return
    user_id = message.from_user.id
    min_level = await get_setting_int("min_level_casino")  # используем общий уровень казино
    level = await get_user_level(user_id)
    if level < min_level:
        await message.answer(f"❌ Для этой игры нужен {min_level} уровень. Твой уровень: {level}")
        return
...
# ----- УВЕДОМЛЕНИЕ АДМИНОВ О ПОКУПКЕ (новая функция) -----
...
# Все функции и переменные из частей 1-3 предполагаются доступными (при сборке)
...
async def heist_keyword_handler(message: Message):
    ...
    async with db_pool.acquire() as conn:
        ...
        # Проверяем кулдаун участия (глобальный, но с отдельным временем из настроек)
        cooldown_minutes = await get_setting_int("heist_cooldown_minutes")
        ok, remaining = await check_global_cooldown(user_id, 'heist_participate', cooldown_minutes * 60)
        if not ok:
        ...
        name = user_info['first_name'] if user_info else f"ID{user_id}"
        phrase = get_random_phrase(HEIST_TYPES[heist['event_type']].get('phrases_join', ["✅ {name} присоединился к налёту!"]),
                                   name=name, share=share, remaining=new_remaining)
        await auto_delete_reply(message, phrase)

        # Если есть BTC, можно добавить информацию
        if heist['btc_pot'] > 0:
            btc_share = float(heist['btc_pot']) / (await conn.fetchval("SELECT COUNT(*) FROM heist_participants WHERE heist_id=$1", heist['id']))
            await auto_delete_reply(message, f"₿ В банке есть BTC! Твоя базовая доля BTC: {btc_share:.4f} (будет распределена после распила)")
...
async def betray_command(message: Message, state: FSMContext):
    ...
    async with db_pool.acquire() as conn:
        ...
        if not heist:
            ...
            return
        # Получаем список других участников с их именами для клавиатуры
        participants = await conn.fetch("""
            SELECT hp.user_id, u.first_name, u.username
            FROM heist_participants hp
            JOIN users u ON hp.user_id = u.user_id
            WHERE hp.heist_id=$1 AND hp.user_id!=$2
        """, heist['id'], user_id)
        if not participants:
            await message.answer("❌ Нет других участников для кидалова.")
            return
    kb = betray_target_keyboard([dict(p) for p in participants], heist['id'])
...
async def betray_target_callback(callback: CallbackQuery, state: FSMContext):
    parts = callback.data.split("_")
...
async def betray_cancel_callback(callback: CallbackQuery, state: FSMContext):
    await state.finish()
    await callback.message.delete()
...
async def cmd_smuggle(message: Message):
    ...
    # Имя пользователя берём напрямую из message
...
async def cmd_jail(message: Message):
    ...
    # Проверка кулдауна (индивидуального для команды)
    cooldown_hours = await get_setting_int("jail_cooldown_hours")
    ok, remaining = await check_global_cooldown(user_id, 'jail', cooldown_hours * 3600)
    if not ok:
        await auto_delete_reply(message, f"⏳ В тюрьму можно попасть раз в {cooldown_hours} ч. Осталось {format_time_remaining(remaining)}.")
        return
    ...
    # Записываем начало срока
    await start_jail_sentence(user_id, message.chat.id, duration)

    # Отправляем сообщение о начале срока
    name = message.from_user.first_name
...
# Все функции и переменные из частей 1-4 предполагаются доступными (при сборке)
            ...
            async with db_pool.acquire() as conn:
                ...
                for run in runs:
                    ...
                    if rand <= success_chance:
                        ...
                        result_text = get_random_phrase(SMUGGLE_FAIL_PHRASES, name=name, username=username)  # можно использовать отдельные фразы для потери
                        status = 'failed'
                        penalty = 0
...
if __name__ == '__main__':
    ...
    loop.create_task(process_jail_sentences())
