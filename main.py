...
PERMISSIONS_LIST = [
    ...
    "edit_settings",
    "cleanup",
    "manage_admins",  # новое право для управления админами
]
    ...
    for attempt in range(1, retries + 1):
        try:
            db_pool = await asyncpg.create_pool(
                DATABASE_URL,
                min_size=10,  # увеличено для тысяч пользователей
                max_size=50,
                command_timeout=60,
                max_queries=50000,
...
async def init_db():
    async with db_pool.acquire() as conn:
        ...
        # Таблица типов бизнесов (ИСПРАВЛЕНО: правильные имена колонок)
        await conn.execute('''
        ...
        # Медиафайлы (расширенная таблица)
        await conn.execute('''
    ...
    await init_media_keys()  # инициализация предопределённых медиа-ключей
...
async def init_media_keys():
    """Инициализирует предопределённые ключи медиа, если их нет"""
    async with db_pool.acquire() as conn:
...
# Остальные функции налётов (finish_heist_joining, finish_heist_splitting, process_betray) остаются без изменений, только добавить в них отправку с медиа для итогов, если нужно.
...
# ==================== НОВЫЕ ФУНКЦИИ ДЛЯ СБРОСА СТАТИСТИКИ (ТОЛЬКО ДЛЯ АДМИНА) ====================
...
# Импорты из Частей 1 и 2 (предполагаются доступными в глобальной области)
...
async def shop_handler(message: Message):
    ...
    for row in rows:
        ...
        # ИСПРАВЛЕНО: используем buyproduct_ вместо buy_item_
...
# ИСПРАВЛЕНО: новый callback_data для покупки
...
# ==================== КИДАЛОВО В НАЛЁТЕ (ЛИЧНЫЕ СООБЩЕНИЯ) ====================
...
# Импорты из предыдущих частей (предполагаются доступными)
...
async def settings_category_handler(message: Message):
    ...
    for key, desc in params:
        ...
        kb_params.append((key, desc    if not await check_admin_permissions(message.from_user.id, "edit_settings"):
        await message.answer("❌ Недостаточно прав.")
        return
...
async def settings_back_callback(callback: Call))
...
async def edit_setting_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        ...
        f"Текущее❌ Недостаточно прав.", show_alert=True)
        return
    ...
    await callback.message.answer(
        ...
        reply_mark = await get_setting(key)
...
async def edit_setting_value(message: Message, state: FSMContext):
    ...
    значение: <code>{current_value}</code>\n\n"
        f"Введи новое значение:",
        reply_markup=back_keyboard()
    )
    await EditSettings.key.set()
...
async def edit_setting_value(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(f"✅ Настройка <bup=back_keyboard()
    )
    await EditSettings.key.set()
    await callback.answer()
...
async def cleanup_>{key}</b> обновлена!\nНовое значение: <code>{new_value}</code>")
    except Exception as e:
        logging.error(f"Error setting {key}: {e}")
        await message.answer("❌ Ошибка при сохранении настройки.")
...
async def cleanup_old_data(message: Message):
    ...
    await message.answer("    await state.finish()
    await settings_menu(message)
...
# ==================== ФОНОВАЯ ЗАДАЧА: СПАВН НАЛЁТОВ (В КАЖДЫЙ ЧАТ РАЗ В ЧАС) ====================
async def heist_spawner():
    """Каждый час создаёт налёты во всех подтверждённых чатах (если прошёл кулдаун)."""
    while True:
        try:
            # Ждём 1 час (60 минут)
            interval_minutes = 60
            ...
            for chat_id, chat_data in confirmed.items():
                try:
                    ...
                    async with db_pool.acquire() as conn:
                        ...
                        if last_heist:
                            try:
                                ...
                                # Не создаём, если с последнего налёта прошло меньше 60 минут
                    ...
                    # Небольшая задержка между созданием налётов в разных чатах
        ...
        try:
            ...
            async with db_pool.acquire() as conn:
                ...
                for run in runs:
                    ...
                    if chat_id:
                        # Отправляем с картинкой, если есть
                        media_key = 'smuggle_success' if status == 'completed' else 'smuggle_fail'
                        file_id = await get_media_file_id(media_key)
                        if file_id:
                            await bot.send_photo(chat_id, file_id, caption=result_text)
                        else:
            ...
            async with db_pool.acquire() as conn:
                ...
                for row in rows:
                    ...
                    if chat_id:
                        file_id = await get_media_file_id(media_key)
                        if file_id:
                            await bot.send_photo(chat_id, file_id, caption=phrase)
                        else:
