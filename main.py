...
DEFAULT_SETTINGS = {
    ...
    "golden_ticket_gift": "100",          # <-- НОВАЯ НАСТРОЙКА для золотого билета
...
def admin_main_keyboard(permissions: List[str]):
    ...
    if row4:
        builder.row(*row4)
    # FIX: Добавляем новый ряд с кнопками для быстрого создания задания и розыгрыша
    row5 = []
    if "manage_users" in permissions:
        row5.append(KeyboardButton(text="➕ Создать задание"))
    if "manage_giveaways" in permissions:
        row5.append(KeyboardButton(text="➕ Создать розыгрыш"))
    if row5:
        builder.row(*row5)
    ...
    try:
        ...
        async with db_pool.acquire() as conn:
            ...
            if stock != -1 and stock <= 0:
                await callback.message.edit_text("❌ Товара нет в наличии!")
                return
            balance = await get_user_balance(user_id)
            if balance < price:
                await callback.message.edit_text(f"❌ Не хватает баксов! Нужно {price:.2f}, у тебя {balance:.2f}")
                return
            async with conn.transaction():
        ...
        # FIX: редактируем сообщение вместо удаления
        await callback.message.edit_text(f"✅ Ты купил {name}! {phrase}")
        ...
        await callback.message.edit_text("❌ Ошибка при покупке. Попробуй позже.")
...
async def perform_theft(message: Message, robber_id: int, victim_id: int, cost: float = 0):
    # FIX: добавлена проверка на бан жертвы
        ...
        async with conn.transaction():
            ...
            if success and victim_balance > 0:
                ...
                if steal_amount > 0:
                    ...
                    # FIX: используем RETURNING для получения нового значения theft_success
                    new_success = await conn.fetchval("""
                        UPDATE users SET theft_attempts = theft_attempts + 1, theft_success = theft_success + 1
                        WHERE user_id = $1
                        RETURNING theft_success
                    """, robber_id)
...
async def purchase_done(callback: CallbackQuery):
    ...
    try:
        ...
        await callback.message.edit_text("✅ Покупка отмечена как выполненная")
    except Exception as e:
...
# (эти хендлеры находятся в части 4, здесь они не дублируются)
# ==================== КОНЕЦ ЧАСТИ 5.1 ====================
...
# ==================== НОВЫЕ ХЕНДЛЕРЫ ДЛЯ КНОПОК ГЛАВНОГО МЕНЮ АДМИНА ====================
@dp.message(F.text == "➕ Создать задание")
async def admin_create_task_from_main(message: Message, state: FSMContext):
    if not await check_admin_permissions(message.from_user.id, "manage_users"):
        await message.answer("❌ Недостаточно прав.")
        return
...
async def admin_create_giveaway_from_main(message: Message, state: FSMContext):
...
async def admin_remove_order_finish(message: Message, state: FSMContext):
    ...
    try:
        ...
        await state.clear()
        return
    # Используем функцию отмены заявки, которая сама всё сделает в транзакции
    success = await cancel_bitcoin_order(order_id, None)  # user_id=None для админского удаления
    if success:
...
async def admin_end_giveaway(callback: CallbackQuery):
    ...
    await callback.message.edit_text("✅ Розыгрыш завершён, победители уведомлены.")
...
async def admin_delete_giveaway(callback: CallbackQuery):
    ...
    await callback.message.edit_text("✅ Розыгрыш удалён.")
    await admin_active_giveaways(callback.message)
...
async def edit_setting_start(callback: CallbackQuery, state: FSMContext):
    # FIX: добавлен мгновенный ответ (если его не было, то добавляем)
    await callback.answer()
...
async def edit_setting_value(message: Message, state: FSMContext):
    ...
    await state.clear()
    # FIX: после сохранения возвращаемся в меню категории
    # Для этого нужно знать категорию, но мы её не сохраняли. Можно либо сохранять, либо просто показать сообщение и предложить вернуться в настройки.
    # Упростим: покажем сообщение и предложим вернуться в настройки.
    await settings_menu(message)
