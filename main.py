...
DEFAULT_SETTINGS = {
    ...
    # ----- КАЗИНО И ИГРЫ (упрощённые настройки) -----
    "dice_win_chance": "50",
    "dice_min_bet": "1",
    "dice_max_bet": "1000",
    "min_level_dice": "1",
    ...
    "roulette_win_chance": "47.3",  # (18/38) для красного/чёрного
    "roulette_min_bet": "1",
    ...
    # ----- НАЛЁТЫ -----
    "heist_min_interval_hours": "2",
    "heist_max_interval_hours": "5",
    "heist_join_minutes": "5",
    "heist_split_minutes": "2",
    "heist_min_pot": "50",
    "heist_max_pot": "200",
    "heist_btc_chance": "10",
    "heist_min_btc": "0.001",
...
# ==================== ФРАЗЫ ДЛЯ ТЮРЬМЫ ====================
JAIL_PHRASES = [
    "🏛 Ты подрался с авторитетом и заслужил уважение. +{auth} авторитета.",
    "🏛 В камере тебя уважают за прошлые заслуги. +{auth} авторитета.",
    "🏛 Ты навалял смотрящему. +{auth} авторитета.",
    "🏛 Тебя унизили и заставили мыть парашу. 0 авторитета.",
    "🏛 Ты проиграл в карты и остался должен. 0 авторитета.",
    "🏛 Ты подружился с ворами в законе. +{auth} авторитета.",
    "🏛 Тебя избили за неправильный взгляд. 0 авторитета.",
    "🏛 Ты помог старику и заработал уважение. +{auth} авторитета.",
    "🏛 Ты попытался украсть баланду, но попался. 0 авторитета.",
    "🏛 Ты стал главным в камере. +{auth} авторитета.",
    "🏛 Тебя перевели в штрафной изолятор. 0 авторитета.",
    "🏛 Ты научил новичка местным законам. +{auth} авторитета."
]
...
class ThrottlingMiddleware(BaseMiddleware):
    ...
    async def on_process_message(self, message: types.Message, data: dict):
        ...
        # очистка старых записей раз в 100 сообщений
    ...
    async with db_pool.acquire() as conn:
        ...
        await conn.execute('''
            CREATE TABLE IF NOT EXISTS business_types (
                ...
                base_income_week INTEGER NOT NULL,
                description TEXT,
...
async def get_random_user(exclude_id: int):
    async with db_pool.acquire() as conn:
        # Более эффективный способ, чем ORDER BY RANDOM()
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            if delta.total_seconds() < 0:
                await conn.execute("UPDATE user_businesses SET last_collection=$1 WHERE id=$2", now.strftime("%Y-%m-%d %H:%M:%S"), business_id)
                return False, "Ошибка даты. Дата сброшена, попробуйте снова."
            weeks_passed = delta.days // 7
            if weeks_passed <= 0:
                return False, "Доход ещё не накопился. Приходи через неделю после последнего сбора."
            max_weeks = 4
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            if weeks_passed > 0:
                ...
                weeks_to_collect = min(weeks_passed, 4)
                if weeks_to_collect > 0:
                    total_income = income_per_week * weeks_to_collect
                    await update_user_balance(user_id, float(total_income), conn=conn)
                    await conn.execute(
                        "UPDATE user_businesses SET last_collection=$1 WHERE id=$2",
                        datetime.now().strftime("%Y-%m-%d %H:%M:%S"), business_id
                    )
...
async def perform_cleanup(manual=False):
    ...
    async with db_pool.acquire() as conn:
        ...
        # Очистка глобальных кулдаунов старше 1 дня
...
async def match_orders(conn):
    while True:
        ...
        # Здесь можно добавить комиссию, но для простоты пока без неё
...
# ----- СОСТОЯНИЯ ДЛЯ КИДАЛОВА В НАЛЁТЕ -----
...
def casino_menu_keyboard():
    return ReplyKeyboardMarkup(keyboard=[
        ...
        [KeyboardButton(text="◀️ Назад")]  # 👥 Мультиплеер 21 удалён
...
def settings_categories_keyboard():
    return ReplyKeyboardMarkup(keyboard=[
        ...
        [KeyboardButton("⚙️ Тюрьма")],  # добавлена новая категория
        [KeyboardButton("◀️ Назад в админку")]
    ], resize_keyboard=True)
...
# ----- Клавиатура для выбора цели в кидалове -----
...
async def universal_back_handler(message: types.Message, state: FSMContext):
    ...
    elif current_state.startswith('RoomChat'):
        await state.finish()
        await message.answer("Комнаты больше не поддерживаются.", reply_markup=main_menu_keyboard(is_admin_user))
...
async def dice_bet(message: types.Message, state: FSMContext):
    ...
    min_bet = await get_setting_float("dice_min_bet")
    max_bet = await get_setting_float("dice_max_bet")
    ...
    win_chance = await get_setting_float("dice_win_chance")
    ...
    async with db_pool.acquire() as conn:
        ...
        if win:
            multiplier = 2.0  # фиксированный множитель для костей (можно вынести в настройки, но упростим)
...
async def guess_bet(message: types.Message, state: FSMContext):
    ...
    user_id = message.from_user.id
    balance = await get_user_balance(user_id)
    min_bet = await get_setting_float("guess_min_bet")
    max_bet = await get_setting_float("guess_max_bet")
    max_input = await get_setting_float("max_input_number")
    if amount < min_bet:
...
async def guess_number_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    win_chance = await get_setting_float("guess_win_chance")
    win = random.random() * 100 <= win_chance
    ...
    async with db_pool.acquire() as conn:
        ...
        if win:
            ...
            secret = random.randint(1, 5)  # просто для красоты
...
async def slots_bet(message: types.Message, state: FSMContext):
    ...
    if win:
        # симулируем выигрышную комбинацию
...
async def process_roulette_bet(message: types.Message, state: FSMContext, user_id: int):
    ...
    win_chance = await get_setting_float("roulette_win_chance")  # примерно 47.3% для красного/чёрного
    if bet_type == 'green':
        win_chance = 1 / 37 * 100  # ~2.7%
    elif bet_type == 'number':
        win_chance = 1 / 37 * 100  # ~2.7%
    # для красного/чёрного используем общую настройку
            ...
            async with conn.transaction():
                ...
                if success and victim_balance > 0:
                    ...
                    if steal_amount > 0:
                        ...
                        if new_success >= required_thefts:  # ИСПРАВЛЕНО: >= вместо ==
...
async def buy_business_choose(callback: types.CallbackQuery, state: FSMContext):
    # Этот хендлер должен быть зарегистрирован ДО магазинного (или с другим фильтром)
    # Мы его разместили выше в коде (порядок регистрации важен)
...
async def sell_bitcoin_start(message: types.Message, state: FSMContext):
    if message.chat.type != 'private':
        return
    user_id = message.from_user.id
    btc_balance = await get_user_bitcoin(user_id)
...
async def buy_bitcoin_start(message: types.Message, state: FSMContext):
    if message.chat.type != 'private':
        return
    min_amount = await get_setting_float("exchange_min_amount_btc")
    await message.answer(
...
async def heist_keyword_handler(message: types.Message):
    ...
    # Автоудаление сообщения пользователя, если есть права
    if await can_delete_message(message.chat.id, message):
        await message.delete()
...
async def betray_command(message: types.Message, state: FSMContext):
    ...
    async with db_pool.acquire() as conn:
        ...
        await HeistBetray.target_id.set()

@dp.callback_query_handler(lambda c: c.data.startswith("betray_target_"), state=HeistBetray.target_id)
...
@dp.callback_query_handler(lambda c: c.data == "betray_cancel", state=HeistBetray.target_id)
async def betray_cancel_callback(callback: types.CallbackQuery, state: FSMContext):
    await state.finish()
    await callback.message.delete()
    await callback.answer("❌ Кидалово отменено.")
...
async def cmd_jail(message: types.Message):
    """Игра «Тюрьма»: раз в час игрок может получить авторитет."""
    ...
    # Проверка кулдауна (индивидуального для этой команды)
    ok, remaining = await check_global_cooldown(user_id, 'jail')
    if not ok:
        await auto_delete_reply(message, f"⏳ В тюрьму можно попасть раз в час. Осталось {format_time_remaining(remaining)}.")
        return
    ...
    if success:
        ...
        phrase = random.choice(JAIL_PHRASES).format(auth=auth_gain)
    else:
        auth_gain = 0
        # Выбираем фразу с 0 авторитета (те, что не содержат +{auth})
        zero_phrases = [p for p in JAIL_PHRASES if "0 авторитета" in p]
        phrase = random.choice(zero_phrases) if zero_phrases else "Ты отсидел срок без последствий. 0 авторитета."
    
    # Устанавливаем кулдаун
    await set_global_cooldown(user_id, 'jail')
    
    # Отправляем результат в чат с упоминанием игрока
...
async def cmd_chat_top(message: types.Message):
    """Топ участников чата по богатству и репутации."""
    if not await check_chat(message):
        return
    user_id = message.from_user.id
    if await is_banned(user_id) and not await is_admin(user_id):
    ...
    # Получаем список участников чата (не всех пользователей бота, а только тех, кто есть в чате)
    # Для этого нужно получить список участников чата. В больших группах это может быть проблематично,
    # поэтому используем ограниченный топ по всем пользователям, но можно добавить фильтр по чату в будущем.
    # Пока сделаем общий топ, но с пометкой "все игроки".
...
async def chat_top_callback(callback: types.CallbackQuery):
    parts = callback.data.split("_")
    category = parts[2]  # balance или reputation
    page = int(parts[3])
...
async def cmd_chat_profile(message: types.Message):
...
async def cmd_chat_heist_status(message: types.Message):
    """Статус текущего налёта в чате."""
    if not await check_chat(message):
        return
    user_id = message.from_user.id
    if await is_banned(user_id) and not await is_admin(user_id):
    ...
    async with db_pool.acquire() as conn:
        ...
        count = participants[0]['count'] if participants else 0
        status_emoji = "🟡" if heist['status'] == 'joining' else "🔴"
        time_remaining = (heist['join_until'] if heist['status'] == 'joining' else heist['split_until']) - datetime.now()
        time_str = format_time_remaining(int(time_remaining.total_seconds())) if time_remaining.total_seconds() > 0 else "завершается"
        
        text = (
            f"{status_emoji} Налёт: {HEIST_TYPES[heist['event_type']]['name']}\n"
            f"💰 Банк: {heist['total_pot']} ₽"
        )
        if heist['btc_pot'] > 0:
            text += f" + {heist['btc_pot']} BTC"
        text += f"\n👥 Участников: {count}\n"
...
SETTINGS_CATEGORIES = {
    "⚙️ Казино": [
        ("dice_win_chance", "🎲 Шанс выигрыша в кости (%)"),
        ("dice_min_bet", "🎲 Мин. ставка кости"),
        ("dice_max_bet", "🎲 Макс. ставка кости"),
        ("min_level_dice", "🎲 Мин. уровень для костей"),
        ("guess_win_chance", "🔢 Шанс выигрыша в угадайку (%)"),
        ("guess_multiplier", "🔢 Множитель угадайки"),
        ("guess_min_bet", "🔢 Мин. ставка угадайка"),
        ("guess_max_bet", "🔢 Макс. ставка угадайка"),
        ("min_level_guess", "🔢 Мин. уровень для угадайки"),
        ("slots_win_probability", "🍒 Шанс выигрыша в слотах (%)"),
        ("slots_min_bet", "🍒 Мин. ставка слоты"),
        ("slots_max_bet", "🍒 Макс. ставка слоты"),
        ("min_level_slots", "🍒 Мин. уровень для слотов"),
        ("roulette_win_chance", "🎡 Шанс выигрыша в рулетке (%)"),
        ("roulette_min_bet", "🎡 Мин. ставка рулетка"),
        ("roulette_max_bet", "🎡 Макс. ставка рулетка"),
...
async def remove_balance_start(message: types.Message, state: FSMContext):
    if not await check_admin_permissions(message.from_user.id, "manage_users"):
        await message.answer("❌ Недостаточно прав.")
        return
    await message.answer("Введи ID или @username пользователя:", reply_markup=back_keyboard())
...
async def add_reputation_start(message: types.Message, state: FSMContext):
    if not await check_admin_permissions(message.from_user.id, "manage_users"):
        await message.answer("❌ Недостаточно прав.")
        return
    await message.answer("Введи ID или @username пользователя:", reply_markup=back_keyboard())
...
async def remove_reputation_start(message: types.Message, state: FSMContext):
    if not await check_admin_permissions(message.from_user.id, "manage_users"):
        await message.answer("❌ Недостаточно прав.")
        return
    await message.answer("Введи ID или @username пользователя:", reply_markup=back_keyboard())
...
async def add_exp_start(message: types.Message, state: FSMContext):
    if not await check_admin_permissions(message.from_user.id, "manage_users"):
        await message.answer("❌ Недостаточно прав.")
        return
    await message.answer("Введи ID или @username пользователя:", reply_markup=back_keyboard())
...
async def set_level_start(message: types.Message, state: FSMContext):
    if not await check_admin_permissions(message.from_user.id, "manage_users"):
        await message.answer("❌ Недостаточно прав.")
        return
    await message.answer("Введи ID или @username пользователя:", reply_markup=back_keyboard())
...
async def add_bitcoin_start(message: types.Message, state: FSMContext):
    if not await check_admin_permissions(message.from_user.id, "manage_users"):
        await message.answer("❌ Недостаточно прав.")
        return
    await message.answer("Введи ID или @username пользователя:", reply_markup=back_keyboard())
...
async def remove_bitcoin_start(message: types.Message, state: FSMContext):
    if not await check_admin_permissions(message.from_user.id, "manage_users"):
        await message.answer("❌ Недостаточно прав.")
        return
    await message.answer("Введи ID или @username пользователя:", reply_markup=back_keyboard())
...
async def add_authority_start(message: types.Message, state: FSMContext):
    if not await check_admin_permissions(message.from_user.id, "manage_users"):
        await message.answer("❌ Недостаточно прав.")
        return
    await message.answer("Введи ID или @username пользователя:", reply_markup=back_keyboard())
...
async def remove_authority_start(message: types.Message, state: FSMContext):
    if not await check_admin_permissions(message.from_user.id, "manage_users"):
        await message.answer("❌ Недостаточно прав.")
        return
    await message.answer("Введи ID или @username пользователя:", reply_markup=back_keyboard())
...
async def find_user_start(message: types.Message, state: FSMContext):
    if not await check_admin_permissions(message.from_user.id, "manage_users"):
        await message.answer("❌ Недостаточно прав.")
        return
    await message.answer("Введи ID или @username пользователя:", reply_markup=back_keyboard())
...
async def add_shop_item_start(message: types.Message, state: FSMContext):
    if not await check_admin_permissions(message.from_user.id, "manage_shop"):
        return
...
async def remove_shop_item_start(message: types.Message, state: FSMContext):
    if not await check_admin_permissions(message.from_user.id, "manage_shop"):
        return
    try:
        async with db_pool.acquire() as conn:
...
async def edit_shop_item_start(message: types.Message, state: FSMContext):
    if not await check_admin_permissions(message.from_user.id, "manage_shop"):
        return
...
async def edit_shop_item_id(message: types.Message, state: FSMContext):
    if message.text == "◀️ Назад":
        await state.finish()
        await admin_shop_menu(message)
        return
...
async def add_business_start(message: types.Message, state: FSMContext):
    if not await check_admin_permissions(message.from_user.id, "manage_businesses"):
        return
...
async def edit_business_start(message: types.Message, state: FSMContext):
    if not await check_admin_permissions(message.from_user.id, "manage_businesses"):
        return
...
async def toggle_business_start(message: types.Message, state: FSMContext):
    if not await check_admin_permissions(message.from_user.id, "manage_businesses"):
        return
...
async def admin_remove_order_start(message: types.Message, state: FSMContext):
    if not await check_admin_permissions(message.from_user.id, "manage_exchange"):
        return
...
async def remove_media_start(message: types.Message, state: FSMContext):
    if not await check_admin_permissions(message.from_user.id, "manage_media"):
        return
...
# ==================== КОНЕЦ ЧАСТИ 5 ====================
# ==================== ЧАСТЬ 6: ФОНОВЫЕ ЗАДАЧИ И ЗАПУСК БОТА ====================
    ...
    while True:
        try:
            ...
            async with db_pool.acquire() as conn:
                ...
                # Проверяем время последнего налёта в этом чате
            ...
            # Обновляем время последнего налёта в чате
            async with db_pool.acquire() as conn:
                await conn.execute(
...
async def on_startup(dp):
    # Удаляем вебхук (на всякий случай) и устанавливаем команды
    await bot.delete_webhook(drop_pending_updates=True)
