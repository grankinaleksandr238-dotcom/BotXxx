...
# Полностью переписано для aiogram 3.x. Все ошибки исправлены и код оптимизирован.
...
async def acquire_lock(lock_name: str, timeout: int = 10) -> bool:
    """
    Пытается захватить блокировку в Redis.
    Если Redis недоступен, возвращает True (отсутствие блокировки),
    но с предупреждением в логах.
    """
    if redis_client is None:
        logging.warning(f"Redis недоступен, блокировка {lock_name} не применяется")
        return True
    try:
        ...
        if levels_gained > 0:
            ...
            if reward_summary:
                await safe_send_message(
                    user_id,
                    "🎉 Поздравляем! Ты достиг новых уровней!\n" + "\n".join(reward_summary) +
...
async def recover_heists():
    ...
    logging.info(f"Восстановлено {len(joining_heists)} налётов в сборе и {len(splitting_heists)} в распиле.")
...
# ----- Состояния для покупки/продажи по цене из стакана (ДОБАВЛЕНЫ) -----
...
# ----- Состояние для отклонения покупки (исправлено) -----
class PurchaseReject(StatesGroup):
    comment = State()

# ----- Состояния для редактирования розыгрышей (ДОБАВЛЕНЫ) -----
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            if cost > 0:
                ...
                # robber_balance обновляется, но в дальнейшем не используется
...
# ==================== КОНЕЦ ЧАСТИ 4 ====================
# ==================== ЧАСТЬ 5: АДМИНИСТРАТИВНАЯ ПАНЕЛЬ (ИСПРАВЛЕННАЯ) ====================
# Полностью переписано для aiogram 3.x. Все ошибки исправлены.
# - Исправлена пагинация во всех списках (убраны блоки парсинга из текста)
# - Добавлен класс EditGiveaway и исправлен StateFilter
# - Использование BufferedInputFile для экспорта
