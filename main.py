...
from aiogram.filters import Command, CommandStart, StateFilter  # FIX: добавлены CommandStart, StateFilter
...
DEFAULT_SETTINGS = {
    ...
    # ----- АВТОУДАЛЕНИЕ -----
    "auto_delete_commands_seconds": "900",  # FIX: изменено с 30 на 900 (15 минут)
...
class GlobalCooldownMiddleware(BaseMiddleware):
    async def __call__(self, handler, event: Message, data: dict):
        if event.chat.type == 'private':
            return await handler(event, data)
        # FIX: добавляем белый список команд, на которые кулдаун не действует
        if event.text and event.text.startswith('/'):
            command = event.text.split()[0]  # берём команду без аргументов
...
async def auto_delete_reply(message: Message, text: str, delete_seconds: int = None, **kwargs):
    ...
    # FIX: удаляем только команду, а ответ оставляем? По новым требованиям команды удаляются сразу, а ответы остаются.
    # В данной функции ответ тоже удаляется через delete_after. Чтобы оставить ответ, нужно не вызывать delete_after.
    # Но в исходном коде эта функция используется для ответов, которые тоже должны удаляться (например, сообщения об ошибках).
    # Оставим как есть, но в местах, где ответ должен остаться, будем использовать отдельную отправку без автоудаления.
...
async def auto_delete_command(message: Message, text: str = None, **kwargs):
    ...
    if text:
        ...
        # FIX: здесь тоже удаляем ответ. Если нужно, чтобы ответ остался, используйте другой метод.
        asyncio.create_task(delete_after(sent, delete_seconds))

# ==================== КОНЕЦ ЧАСТИ 1a ====================
# ==================== ЧАСТЬ 1b: ПОДКЛЮЧЕНИЕ К БД, ИНИЦИАЛИЗАЦИЯ ТАБЛИЦ, РАБОТА С НАСТРОЙКАМИ И ДРУГИЕ ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ (ПРОДОЛЖЕНИЕ) ====================

# (продолжение после глобальных переменных и мидлварей)
...
# ==================== СОСТОЯНИЯ FSM (все из исходного кода) ====================
...
# Добавляем состояние для покупки товара
...
def main_menu_keyboard(is_admin: bool = False) -> ReplyKeyboardMarkup:
    ...
    builder.row(
        KeyboardButton(text="🎁 Розыгрыши"),          # пользовательская кнопка
        KeyboardButton(text="📊 Уровень")
    )
    builder.row(
        KeyboardButton(text="🏪 Фармилка"),            # изменено с "Мои бизнесы" на "Фармилка"
...
def admin_main_keyboard(permissions: List[str]) -> ReplyKeyboardMarkup:
    """Главное меню админ-панели с обновлёнными кнопками."""
    builder = ReplyKeyboardBuilder()
    row1 = []
...
# (Для угадайки и рулетки оставляем инлайн-кнопки для удобства)
...
# FIX: добавлена недостающая функция
...
async def universal_back_handler(message: Message, state: FSMContext):
    ...
    if current_state.startswith('DiceBet') or current_state.startswith('GuessBet') or \
       current_state.startswith('SlotsBet') or current_state.startswith('RouletteBet'):
        ...
        await state.clear()
        # Перенаправляем в админку розыгрышей (кнопка "+Розыгрыш" или список)
        # Но у нас нет отдельного меню, поэтому просто в админку
        ...
        await state.clear()
        # Аналогично, возвращаем в админку
        permissions = await get_admin_permissions(user_id)
        await message.answer("Панель администратора:", reply_markup=admin_main_keyboard(permissions))
...
async def dice_bet(message: Message, state: FSMContext):
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():  # FIX: добавлена транзакция
...
async def process_roulette_bet(message: Message, state: FSMContext, user_id: int):
    ...
    # Шанс выигрыша (для цвета используется настраиваемый, для числа и зелёного рассчитывается)
    if bet_type == 'number':
...
# ==================== КОНЕЦ ЧАСТИ 4 ====================
# ==================== ЧАСТЬ 5: ПОЛЬЗОВАТЕЛЬСКИЕ ХЕНДЛЕРЫ – УНИВЕРСИТЕТ, МАГАЗИН, ПРОМОКОДЫ, РЕФЕРАЛКА, ЗАДАНИЯ, РОЗЫГРЫШИ ====================
...
# FIX: полностью переработанный хендлер покупки с транзакцией и обработкой ошибок
...
async def tasks_user_menu(message: Message):
    ...
    for row in rows:
        ...
        text += f"🔹 ID {row['id']}: {row['name']}{progress}\n{row['description']}\nНаграда: {float(row['reward_coins']):.2f} MLB, {row['reward_reputation']} репутации\n\n"
    text += "Введи ID задания, которое хочешь выполнить (или /cancel)."
    await send_with_media(message.chat.id, text, media_key='tasks')
    await state.set_state(DeleteTask.task_id)  # используем состояние DeleteTask.task_id как ожидание ID задания

@dp.message(DeleteTask.task_id, F.text)
async def process_task_selection(message: Message, state: FSMContext):
    if message.text == "◀️ Назад":
        await state.clear()
        await main_menu_keyboard(await is_admin(message.from_user.id))
        await message.answer("Главное меню:", reply_markup=main_menu_keyboard(await is_admin(message.from_user.id)))
        return
    try:
        task_id = int(message.text.strip())
    except ValueError:
        await message.answer("❌ Введи число (ID задания).")
        return
    ...
    async with db_pool.acquire() as conn:
        task = await conn.fetchrow("SELECT * FROM tasks WHERE id=$1 AND active=TRUE", task_id)
        if not task:
            await message.answer("❌ Задание не найдено или неактивно.")
            await state.clear()
            return

        completed = await conn.fetchval("SELECT 1 FROM user_tasks WHERE user_id=$1 AND task_id=$2", user_id, task_id)
        if completed:
            await message.answer("✅ Ты уже выполнил это задание!")
            await state.clear()
            return
        ...
        if task['task_type'] == 'subscribe':
            ...
            if not subscribed:
                ...
                await message.answer("❌ Ты не подписан на этот канал! Подпишись и попробуй снова.", reply_markup=kb)
                await state.clear()
                return
            ...
            await message.answer("❌ Неподдерживаемый тип задания.")
            await state.clear()

# ==================== ПОЛЬЗОВАТЕЛЬСКИЕ РОЗЫГРЫШИ ====================
# FIX: добавлены хендлеры для пользовательских розыгрышей
...
async def user_completed_giveaways(message: Message):
    async with db_pool.acquire() as conn:
        rows = await conn.fetch("SELECT id, prize, end_date, winners_list FROM giveaways WHERE status='completed' ORDER BY end_date DESC LIMIT 10")
        if not rows:
            await message.answer("Нет завершённых розыгрышей.")
            return
        text = "🏁 Последние завершённые розыгрыши:\n\n"
        for row in rows:
...
# ==================== ФАРМИЛКА (ПОЛЬЗОВАТЕЛЬСКАЯ ЧАСТЬ) ====================
...
async def my_businesses(message: Message):
    ...
    if not businesses:
        kb = ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="🛒 Купить новую фармилку")], [KeyboardButton(text="◀️ Назад")]], resize_keyboard=True)
        await message.answer("У тебя пока нет фармилок. Купи первую!", reply_markup=kb)
        return
    await message.answer("Твои фармилки:", reply_markup=business_main_keyboard(businesses))

# ----- Выбор конкретной фармилки -----
@dp.message(F.text.regexp(r'^[🥤🏪🏬🍽️🏨🛢️] .+ \(ур\. \d+\)$'))
async def select_business(message: Message, state: FSMContext):
    # Парсим название и уровень из текста кнопки
    text = message.text
    emoji = text[0]
    name = text.split(' (ур.')[0][2:]  # убираем эмодзи и пробел
    level = int(text.split('ур. ')[1][:-1])
    user_id = message.from_user.id
    businesses = await get_user_businesses(user_id)
    biz = next((b for b in businesses if b['emoji'] == emoji and b['name'] == name and b['level'] == level), None)
    if not biz:
        await message.answer("❌ Фармилка не найдена.")
        return
    await state.update_data(business_id=biz['id'])
    await message.answer(f"Выбрана фармилка {biz['emoji']} {biz['name']} (ур. {biz['level']})", reply_markup=business_actions_keyboard())

# ----- Сбор дохода -----
@dp.message(F.text == "💰 Собрать доход")
async def collect_income(message: Message, state: FSMContext):
    data = await state.get_data()
    business_id = data.get('business_id')
    if not business_id:
        await message.answer("❌ Сначала выбери фармилку.")
        return
    user_id = message.from_user.id
    success, msg, amount = await collect_business_income(user_id, business_id)
    await message.answer(msg)
    if success:
        # можно добавить логику уведомлений
        pass
    await state.clear()  # или оставить состояние для дальнейших действий? Лучше оставить.
    # После сбора возвращаемся к списку фармилок
    await my_businesses(message)

# ----- Улучшение фармилки -----
@dp.message(F.text == "⬆️ Улучшить")
async def upgrade_business_start(message: Message, state: FSMContext):
    data = await state.get_data()
    business_id = data.get('business_id')
    if not business_id:
        await message.answer("❌ Сначала выбери фармилку.")
        return
    user_id = message.from_user.id
    # Проверим, можно ли улучшить
    async with db_pool.acquire() as conn:
        biz = await conn.fetchrow("SELECT level, max_level FROM user_businesses WHERE id=$1 AND user_id=$2", business_id, user_id)
        if not biz:
            await message.answer("❌ Фармилка не найдена.")
            return
        if biz['level'] >= biz['max_level']:
            await message.answer("❌ Фармилка уже максимального уровня.")
            return
    await state.update_data(business_id=business_id)
    await message.answer("Ты уверен, что хочешь улучшить эту фармилку? (да/нет)", reply_markup=back_keyboard())
...
async def upgrade_business_confirm(message: Message, state: FSMContext):
    if message.text.lower() == 'да':
        ...
        await message.answer("❌ Улучшение отменено.")
    await state.clear()
    await my_businesses(message)

# ----- Покупка новой фармилки -----
@dp.message(F.text == "🛒 Купить новую фармилку")
async def buy_business_start(message: Message, state: FSMContext):
    user_id = message.from_user.id
    # Проверим, не превышен ли лимит
    max_biz = await get_setting_int("business_max_businesses")
    businesses = await get_user_businesses(user_id)
    if len(businesses) >= max_biz:
        await message.answer(f"❌ У тебя уже максимальное количество фармилок ({max_biz}).")
        return
    types = await get_business_type_list(only_available=True)
    if not types:
        await message.answer("❌ Сейчас нет доступных для покупки фармилок.")
        return
    await message.answer("Выбери тип фармилки:", reply_markup=business_buy_keyboard(types))
    await state.set_state(BuyBusiness.business_type_id)

@dp.message(BuyBusiness.business_type_id, F.text)
async def buy_business_select(message: Message, state: FSMContext):
    # Парсим выбор из текста кнопки
    text = message.text
    # формат: "🥤 Ларёк – 50 BTC"
    parts = text.split(' – ')
    if len(parts) != 2:
    ...
    # Извлекаем имя без эмодзи
    emoji = name_part[0]
    name = name_part[2:].strip()
    types = await get_business_type_list(only_available=True)
    biz_type = next((bt for bt in types if bt['emoji'] == emoji and bt['name'] == name), None)
    if not biz_type:
        await message.answer("❌ Такой тип не найден.")
        return
    await state.update_data(business_type_id=biz_type['id'])
    # Покажем подтверждение
    await message.answer(
        f"Ты собираешься купить {biz_type['emoji']} {biz_type['name']} за {biz_type['base_price_btc']:.2f} BTC.\n"
        f"У тебя на счету: {await get_user_bitcoin(message.from_user.id):.4f} BTC.\n"
        "Подтверди покупку (да/нет):",
        reply_markup=back_keyboard()
    )
    await state.set_state(BuyBusiness.confirming)

@dp.message(BuyBusiness.confirming, F.text)
async def buy_business_confirm(message: Message, state: FSMContext):
    if message.text.lower() == 'да':
        data = await state.get_data()
        biz_type_id = data['business_type_id']
        user_id = message.from_user.id
        biz_type = await get_business_type(biz_type_id)
        if not biz_type:
            await message.answer("❌ Тип бизнеса не найден.")
            await state.clear()
            return
        ...
        if btc < price:
            await message.answer(f"❌ Недостаточно BTC. Нужно {price:.2f}, у тебя {btc:.4f}.")
            await state.clear()
            return
        async with db_pool.acquire() as conn:
            async with conn.transaction():
                ...
                if not success:
                    ...
                    return
                # Создаём бизнес
                await create_user_business(user_id, biz_type_id, biz_type.get('lifetime_hours', 720))
        await message.answer(f"✅ Ты приобрёл {biz_type['emoji']} {biz_type['name']}! Оно начнёт приносить доход через {await get_setting_int('business_collect_interval_minutes')} мин.")
    else:
...
async def buy_bitcoin_amount(message: Message, state: FSMContext):
    ...
    # Предварительная проверка: хватит ли MLB на покупку? (по минимальной цене)
    user_id = message.from_user.id
    balance = await get_user_balance(user_id)
...
async def sell_bitcoin_amount(message: Message, state: FSMContext):
    ...
    # Предварительная проверка: хватит ли BTC на продажу? (по минимальной цене)
    user_id = message.from_user.id
...
# ----- МОИ ЗАЯВКИ (с возможностью отмены) -----
...
async def my_orders(message: Message):
    ...
    for row in rows:
        ...
        text = (
            f"🆔 {row['id']} | {side}\n"
            f"{float(row['amount']):.4f} BTC @ {row['price']} MLB\n"
            f"Создано: {created}"
        )
        # Инлайн-кнопка для отмены
        kb = InlineKeyboardMarkup(
            inline_keyboard=[
                [InlineKeyboardButton(text="❌ Отменить", callback_data=f"cancel_order_{row['id']}")]
            ]
        )
        await message.answer(text, reply_markup=kb)

    # Добавим кнопку возврата в меню биржи (reply)
    kb = ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="◀️ Назад")]], resize_keyboard=True)
    await message.answer("Чтобы вернуться, нажми «Назад».", reply_markup=kb)
...
# ----- Обработчик кнопки "◀️ Назад" для возврата в меню биржи -----
# (Уже есть общий обработчик, но если нужно, можно добавить специфичный)
# В общем обработчике universal_back_handler уже предусмотрен возврат в меню биржи при состоянии SellBitcoin и т.д.
# Но для случая, когда мы просто в меню "Мои заявки" и нажимаем "Назад", нам нужно вернуться в главное меню биржи.
# Добавим отдельный обработчик для этого текста, но только когда не в состоянии.
@dp.message(F.text == "◀️ Назад", StateFilter(None))
async def back_to_exchange_from_orders(message: Message):
    # Проверим, что мы находимся в контексте биржи (например, последнее сообщение было из биржи)
    # Упрощённо: просто вернёмся в меню биржи.
    await bitcoin_exchange_menu(message)
...
async def heist_keyword_handler(message: Message):
    ...
    try:
        async with db_pool.acquire() as conn:
            async with conn.transaction():
                # FIX: блокируем строку налёта для атомарной проверки и вставки
                heist = await conn.fetchrow(
                    "SELECT * FROM heists WHERE chat_id=$1 AND status='joining' AND join_until > NOW() FOR UPDATE",
                    chat_id
                )
                if not heist:
                    return
...
async def cmd_smuggle(message: Message):
    ...
    # FIX: удаляем команду, отправляем сообщение без автоудаления (системное)
    await message.delete()
...
async def jail_article_message(message: Message, state: FSMContext):
    ...
    if chat_id:
        try:
            # Отправляем системное сообщение в чат без автоудаления
...
async def chat_gift(message: Message):
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            # FIX: оптимизированный выбор получателя
            try:
            ...
            recipient_info = await conn.fetchrow("SELECT first_name FROM users WHERE user_id=$1", recipient_id)
            recipient_name = recipient_info['first_name'] if recipient_info else f"ID{recipient_id}"

            # Проверяем глобальный лимит получателя
            recipient = await conn.fetchrow("SELECT last_gift_time, gift_count_today FROM users WHERE user_id=$1 FOR UPDATE", recipient_id)
            if not recipient:
                recipient = {'last_gift_time': None, 'gift_count_today': 0}
            if recipient['last_gift_time'] and recipient['last_gift_time'].date() == today_date:
                recipient_gift_count = recipient['gift_count_today']
            else:
            ...
            if recipient['last_gift_time']:
                last_gift = recipient['last_gift_time']
...
# ==================== ЧАСТЬ 9: АДМИНИСТРАТИВНАЯ ПАНЕЛЬ (УПРАВЛЕНИЕ БИЗНЕСАМИ, БИРЖЕЙ, МЕДИА, ЗАДАНИЯМИ, РОЗЫГРЫШАМИ) ====================
...
async def edit_giveaway_start(message: Message, state: FSMContext):
    if not await check_admin_permissions(message.from_user.id, "manage_giveaways"):
        return
    await message.answer("Введи ID розыгрыша для редактирования:", reply_markup=back_keyboard())
    await state.set_state(EditGiveaway.field)
    await state.update_data(giveaway_id=None)  # будем ждать ввод
...
@dp.message(EditGiveaway.field, F.text)
async def edit_giveaway_option(message: Message, state: FSMContext):
    if message.text == "◀️ Назад":
        await state.clear()
        await admin_panel(message)
        return
    ...
    if option == '1':
        ...
        await message.answer("❌ Неверный номер. Введи 1-6.")
        ...
        try:
            ...
            async with db_pool.acquire() as conn:
                ...
                for run in runs:
                    ...
                    # FIX: проверка на существование пользователя
