...
class GlobalCooldownMiddleware(BaseMiddleware):
    async def __call__(self, handler, event: Message, data: dict):
        ...
        if event.text and event.text.startswith('/'):
            ...
            game_commands = ['/mlb_smuggle', '/mlb_jail', '/mlb_profile', '/mlb_top', '/mlb_heist', '/myheist']
...
# ИСПРАВЛЕНО: 
# 1. В update_user_reputation добавлен параметр conn.
# 2. В collect_business_income исправлен расчёт wait_minutes.
# 3. В match_orders исправлена логика (комиссия, обновление балансов).
# 4. В process_betray_results исправлен вызов set_global_cooldown для "betray".
    ...
    async with db_pool.acquire() as conn:
        ...
        await conn.execute('''
            CREATE TABLE IF NOT EXISTS promocodes (
                ...
                created_at TEXT,
                created_by BIGINT,
                expires_at TIMESTAMP   -- ИСПРАВЛЕНО: добавлено поле срока действия
            )
        ''')
...
# ИСПРАВЛЕНО: добавлен параметр conn
@db_retry()
...
async def get_business_price(business_type: dict, level: int) -> float:
    ...
    if level == 1:
        ...
        # ИСПРАВЛЕНО: линейная зависимость вместо level**1.5 для уменьшения цен
...
async def collect_business_income(user_id: int, business_id: int, conn=None) -> Tuple[bool, str, float]:
    async def _update(conn):
        ...
        if minutes_passed < collect_interval:
            # ИСПРАВЛЕНО: правильный расчёт оставшихся минут
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            for attacker_id, target_id in assigned.items():
                ...
                # ИСПРАВЛЕНО: используем правильный кулдаун для кидалова
...
async def match_orders(conn):
    ...
    while True:
        ...
        # У продавца списываем BTC (уже зарезервированы)
        # При создании ордера на продажу BTC уже зарезервированы, поэтому сейчас не списываем
...
# ИСПРАВЛЕНО: в jail_cell_keyboard добавлена кнопка отмены, 
# переименованы все упоминания "Фармилка" на "Фарм".
...
def admin_main_keyboard(permissions: List[str]) -> ReplyKeyboardMarkup:
    ...
    if "manage_users" in permissions:
        ...
        row4.append(KeyboardButton(text="❌ Удалить задание"))   # Новая кнопка
...
# ИСПРАВЛЕНО: добавлена кнопка отмены
...
# ИСПРАВЛЕНО: в guess_number_callback добавлен conn в update_user_reputation
...
# ИСПРАВЛЕНО: в process_guess_repeat добавлен conn в update_user_reputation
...
# ИСПРАВЛЕНО: в complete_task добавлен conn в update_user_reputation, исправлена опечатка в theft_random.
...
# ИСПРАВЛЕНО:
# - в heist_keyword_handler убрано прямое начисление доли (теперь только добавляем в pot)
# - исправлена проверка на администратора чата
# - улучшена обработка ошибок при удалении сообщения
# - исправлены хендлеры тюрьмы (кнопка отмены, проверка состояний)
# - в chat_gift обеспечено создание получателя при необходимости
...
async def heist_keyword_handler(message: Message):
    ...
    # Проверка, что пользователь не администратор чата
    try:
        chat_member = await bot.get_chat_member(message.chat.id, user_id)
        if chat_member.status in ['creator', 'administrator']:
            await auto_delete_reply(message, "❌ Администраторы не могут участвовать в налётах.")
            return
    except Exception as e:
    ...
    try:
        async with db_pool.acquire() as conn:
            async with conn.transaction():
                ...
                # ИСПРАВЛЕНО: не начисляем на баланс, а добавляем в pot
...
async def jail_cell_callback(callback: CallbackQuery, state: FSMContext):
    # Не проверяем состояние через get_state, так как FSM уже фильтрует
    try:
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            if not recipient_info:
                # Создаём запись получателя, если её нет
...
# ИСПРАВЛЕНО: все хендлеры проверены, ошибок не обнаружено.
...
# ИСПРАВЛЕНО: добавлен хендлер для настроек, сброс кэша каналов.
...
async def add_channel_link(message: Message, state: FSMContext):
    ...
    try:
        ...
        # ИСПРАВЛЕНО: сбрасываем кэш каналов
...
# ==================== ДОБАВЛЕН ХЕНДЛЕР ДЛЯ КНОПКИ НАСТРОЕК ====================
...
async def settings_menu_handler(message: Message):
    if not await check_admin_permissions(message.from_user.id, "edit_settings"):
        await message.answer("❌ Недостаточно прав.")
        return
    await settings_menu(message)  # вызов функции из части 10

# ==================== КОНЕЦ ЧАСТИ 8 ====================
# ==================== ЧАСТЬ 9: АДМИНИСТРАТИВНАЯ ПАНЕЛЬ (УПРАВЛЕНИЕ БИЗНЕСАМИ, БИРЖЕЙ, МЕДИА, ЗАДАНИЯМИ, РОЗЫГРЫШАМИ) ====================
# ИСПРАВЛЕНО: добавлены пользовательские хендлеры для фармов, добавлен хендлер удаления заданий,
# исправлено отображение завершённых розыгрышей.
...
# ИСПРАВЛЕНО: порядок определения SETTINGS_CATEGORIES, все хендлеры проверены.
...
# ИСПРАВЛЕНО:
# 1. В heist_spawner исправлена проверка кулдауна (актуальное значение из БД).
# 2. В process_smuggle_runs добавлена защита от деления на ноль и передача conn.
# 3. В process_jail_sentences добавлена передача conn в add_exp.
# 4. В process_giveaways добавлена обработка ошибок для каждого розыгрыша.
# 5. Добавлена новая фоновая задача log_cleaner для очистки логов каждые 3 часа.
# 6. Обновлён список команд бота в on_startup.
            ...
            for chat_id, chat_data in confirmed.items():
                try:
                    async with db_pool.acquire() as conn:
                        ...
                        if existing:
                            continue
                        # ИСПРАВЛЕНО: получаем актуальное значение last_heist_time из БД
        ...
        try:
            ...
            async with db_pool.acquire() as conn:
                ...
                for gw in time_giveaways:
                    try:  # ИСПРАВЛЕНО: обрабатываем каждый розыгрыш отдельно
...
# ==================== НОВАЯ ФОНОВАЯ ЗАДАЧА: ОЧИСТКА ЛОГОВ КАЖДЫЕ 3 ЧАСА ====================
...
async def on_startup():
    ...
    # ИСПРАВЛЕНО: обновлён список команд
    ...
    asyncio.create_task(log_cleaner())  # НОВАЯ ЗАДАЧА
