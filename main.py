...
# ==================== НАДЁЖНЫЕ БЛОКИРОВКИ (REDIS + FALLBACK НА ТАБЛИЦУ locks) ====================
async def acquire_lock(lock_name: str, timeout: int = 10) -> bool:
    """
    Пытается захватить блокировку с именем lock_name на timeout секунд.
    Сначала пробует Redis (setnx + expire), затем таблицу locks в БД.
    ...
    if redis_client is not None:
        try:
            ...
            if result:
                ...
                return True
            # Проверяем, может блокировка истекла, но не удалена? setnx уже даст false, если ключ есть.
            # Значит, заблокировано.
            return False
        except Exception as e:
            logging.error(f"Redis acquire_lock error for {lock_name}: {e}")
            # Пробуем через БД

    # Fallback на таблицу locks в PostgreSQL
    if db_pool is None:
    ...
    try:
        async with db_pool.acquire() as conn:
            async with conn.transaction():
                ...
                await conn.execute("DELETE FROM locks WHERE locked_until < NOW()")
                # Пытаемся вставить новую блокировку
                # Если конфликт по имени, проверяем, не истекла ли она
                result = await conn.fetchrow("""
                    INSERT INTO locks (lock_name, locked_until, locked_by)
                    ...
                    RETURNING lock_name
                """, lock_name, timeout, f"bot_{os.getpid()}")
...
async def release_lock(lock_name: str):
    ...
    if redis_client is not None:
        try:
            ...
            # Не возвращаемся, т.к. Redis приоритет, но если была БД-блокировка, тоже удалим
        except Exception as e:
    ...
    try:
        async with db_pool.acquire() as conn:
            await conn.execute("DELETE FROM locks WHERE lock_name = $1", lock_name)
    except Exception as e:
...
DEFAULT_SETTINGS = {
    ...
    "task_penalty_coins": "10",
    "task_penalty_reputation": "1",
...
MEDIA_KEYS = [
    ...
    "admin_settings", "admin_tasks", "admin_fights",
...
JAIL_SUCCESS_PHRASES = [
    "🎉 {name} (@{username}) устроил бунт в тюрьме и захватил власть в камере {cell}! Авторитет +{auth}. (статья {article})",
...
class ThrottlingMiddleware(BaseMiddleware):
    def __init__(self, rate_limit=1.0):
        ...
        # Используем TTLCache для автоматической очистки старых записей
        self.user_last_time = TTLCache(maxsize=10000, ttl=3600)  # храним до 10000 пользователей, TTL 1 час
        self.last_warning = TTLCache(maxsize=10000, ttl=3600)
    ...
    async def __call__(self, handler, event: Message, data: dict):
        ...
        if last_time and now - last_time < self.rate_limit:
            if self.last_warning.get(user_id) is None or now - self.last_warning[user_id] > 60:
                try:
    ...
    async with db_pool.acquire() as conn:
        ...
        await conn.execute('''
            CREATE TABLE IF NOT EXISTS tasks (
                ...
                penalty_coins NUMERIC(12,2) DEFAULT 0,
        ...
        await conn.execute('''
            CREATE TABLE IF NOT EXISTS bets (
                ...
                amount NUMERIC(12,2) NOT NULL,
                odds FLOAT NOT NULL,  -- коэффициент на момент ставки (важно для справедливых выплат)
        ...
        await conn.execute("CREATE INDEX IF NOT EXISTS idx_bets_fighter_id ON bets(fighter_id)")
...
@db_retry()
async def set_global_cooldown(user_id: int, command: str, cooldown_seconds: int = None, conn=None):
    if cooldown_seconds is None:
        cooldown_seconds = await get_setting_int("global_cooldown_seconds")
    async def _set(conn):
        await conn.execute('''
...
async def upgrade_business(user_id: int, business_id: int, conn=None) -> Tuple[bool, str]:
    async def _upgrade(conn):
        ...
        return True, f"✅ Отмыть BTC {biz['emoji']} {biz['name']} улучшена до уровня {biz['level'] + 1}! Потрачено {cost:.2f} BTC."
    if conn:
...
async def spawn_heist(chat_id: int) -> Optional[int]:
    ...
    # Возможность появления BTC в налёте
    if random.randint(1, 100) <= await get_setting_int("heist_btc_chance"):
...
async def ask_betray_choice(heist_id: int, split_until: datetime):
    async with db_pool.acquire() as conn:
        ...
        # Получаем информацию о пользователях для отображения имён и юзернеймов
        user_info_cache = {}
        for attacker_id, target_id in pairs:
            for uid in (attacker_id, target_id):
                if uid not in user_info_cache:
                    user = await conn.fetchrow("SELECT first_name, username FROM users WHERE user_id=$1", uid)
                    user_info_cache[uid] = (user['first_name'] if user else f"ID{uid}", user['username'] if user else "нет юзернейма")
            ...
            target_name, target_username = user_info_cache[target_id]
        ...
        # Участники без пары получают уведомление, что их не будут кидать
...
async def create_betray_pairs(heist_id: int, conn) -> List[Tuple[int, int]]:
    ...
    pairs = []
    # Соединяем i-го с i+1-м, последнего с первым
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            for attacker in attackers:
                ...
                await set_global_cooldown(attacker_id, "betray", betray_cooldown, conn=conn)
                ...
                # Получаем информацию об именах для сообщений
                for uid in (attacker_id, target_id):
                    if uid not in user_info:
                        row = await conn.fetchrow("SELECT first_name, username FROM users WHERE user_id=$1", uid)
                        user_info[uid] = (row['first_name'] if row else f"ID{uid}", row['username'] if row else "нет юзернейма")

                attacker_name, attacker_username = user_info[attacker_id]
                target_name, target_username = user_info[target_id]
                amount_changed = abs(attacker_share - new_attacker_share)
                ...
                if success:
                    await safe_send_message(target_id, f"🔪 Вас кинул {attacker_name} (@{attacker_username}) в налёте! Вы потеряли {amount_changed:.2f} MLB.")
                else:
                    await safe_send_message(attacker_id, f"😅 Ваша попытка кинуть {target_name} (@{target_username}) провалилась! Вы потеряли {amount_changed:.2f} MLB.")
                    await safe_send_message(target_id, f"🛡 Вас пытался кинуть {attacker_name} (@{attacker_username}) в налёте, но неудачно! Вы получили {amount_changed:.2f} MLB.")

            # После обработки всех кидалов, собираем финальные доли
            ...
            # Распределение BTC, если они были
            if heist['btc_pot'] > 0 and final_participants:
                btc_per_participant = round(float(heist['btc_pot']) / len(final_participants), 4)
                for p in final_participants:
                    await update_user_bitcoin(p['user_id'], btc_per_participant, conn=conn)
                    await safe_send_message(p['user_id'], f"₿ Вы получили {btc_per_participant:.4f} BTC из налёта!")
            ...
            for idx, p in enumerate(final_participants[:3], 1):
                uid = p['user_id']
                if uid not in user_info:
                    row = await conn.fetchrow("SELECT first_name FROM users WHERE user_id=$1", uid)
                    name = row['first_name'] if row else f"ID{uid}"
                else:
                    name = user_info[uid][0]
                top_list.append(f"{idx}. {name} — {float(p['current_share']):.2f} MLB")
            top_str = "\n".join(top_list)

            # Выбираем красивую фразу для завершения
            config = HEIST_TYPES[heist['event_type']]
            text = get_random_phrase(config.get('phrases_result', ["🏁 Налёт завершён!\n🏆 Топ воров:\n{top}"]), top=top_str)
            await safe_send_chat(heist['chat_id'], text)

            # Начисляем финальные доли
            for p in final_participants:
...
async def match_orders(conn):
    ...
    while True:
        ...
        await asyncio.sleep(0)  # даём шанс другим корутинам
...
async def place_bet(user_id: int, fight_id: int, fighter_id: int, amount: float) -> Tuple[bool, str, float]:
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            # Сохраняем ставку с коэффициентом на момент ставки
            await conn.execute("""
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            for bet in bets:
                ...
                odds = float(bet['odds'])  # используем сохранённый коэффициент
...
class BroadcastToChats(StatesGroup):
    target = State()
    chat_id = State()
    media = State()
...
def admin_main_keyboard(permissions: List[str]) -> ReplyKeyboardMarkup:
    """Главное меню админ-панели с категориями."""
    builder = ReplyKeyboardBuilder()
    row1 = []
    ...
    if "manage_giveaways" in permissions:
        row1.append(KeyboardButton(text="🎁 Управление розыгрышами"))
    if row1:
        builder.row(*row1)
    ...
    row4 = []
    if "manage_users" in permissions:
        row4.append(KeyboardButton(text="📋 Управление заданиями"))
    ...
    if "manage_fights" in permissions:
        builder.row(KeyboardButton(text="🥊 Управление боями"))
...
def fighter_management_keyboard() -> ReplyKeyboardMarkup:
    ...
    return ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text="➖ Удалить бойца")],
            [KeyboardButton(text="◀️ Назад")]
        ],
        resize_keyboard=True
    )
...
def fight_management_keyboard() -> ReplyKeyboardMarkup:
    ...
    return ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text="➖ Удалить бой")],
            [KeyboardButton(text="◀️ Назад")]
        ],
        resize_keyboard=True
    )
...
def confirm_chat_inline(chat_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Подтвердить", callback_data=f"conf_chat_{chat_id}"),
         InlineKeyboardButton(text="❌ Отклонить", callback_data=f"rej_chat_{chat_id}")]
    ])
...
def betray_choice_keyboard(heist_id: int, target_id: int, target_name: str, target_username: str) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"🔪 Кинуть {target_name} (@{target_username})", callback_data=f"by_{heist_id}_{target_id}")],
        [InlineKeyboardButton(text="❌ Отказаться", callback_data=f"bn_{heist_id}")]
    ])
...
def task_detail_keyboard(task_id: int, button_link: str = None) -> InlineKeyboardMarkup:
    ...
    kb.append([InlineKeyboardButton(text="✅ Проверить подписку", callback_data=f"chk_task_{task_id}")])
    kb.append([InlineKeyboardButton(text="◀️ Назад", callback_data="tasks_back")])
    return InlineKeyboardMarkup(inline_keyboard=kb)

def giveaway_condition_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⏰ По времени", callback_data="gw_cond_time")],
        [InlineKeyboardButton(text="👥 По количеству участников", callback_data="gw_cond_parts")]
    ])

def promo_type_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="💰 MLB", callback_data="promo_type_coins"),
         InlineKeyboardButton(text="₿ Биткоины", callback_data="promo_type_btc")]
    ])

def reset_stats_confirm_keyboard(user_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Подтвердить сброс", callback_data=f"rst_conf_{user_id}")],
        [InlineKeyboardButton(text="❌ Отмена", callback_data="rst_cancel")]
    ])

def purchase_action_keyboard(purchase_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Выполнено", callback_data=f"pur_done_{purchase_id}"),
         InlineKeyboardButton(text="❌ Отказ", callback_data=f"pur_rej_{purchase_id}")]
    ])
...
def settings_param_keyboard(params: List[Tuple[str, str]], category: str) -> InlineKeyboardMarkup:
    ...
    builder.button(text="◀️ Назад", callback_data=f"set_back_{category}")
    builder.adjust(1)
...
def completed_giveaway_detail_keyboard(giveaway_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="👀 Посмотреть победителей", callback_data=f"view_comp_{giveaway_id}")]
    ])
...
def fighter_selection_keyboard(fighters: List[dict], page: int, total_pages: int, callback_prefix: str) -> InlineKeyboardMarkup:
    ...
    callback_prefix: что передавать в callback (например, 'f1' для выбора первого бойца)
    """
    builder = InlineKeyboardBuilder()
    for f in fighters:
    ...
    nav_buttons = []
    if page > 1:
        nav_buttons.append(InlineKeyboardButton(text="⬅️", callback_data=f"{callback_prefix}_p{page-1}"))
    if page < total_pages:
        nav_buttons.append(InlineKeyboardButton(text="➡️", callback_data=f"{callback_prefix}_p{page+1}"))
    if nav_buttons:
...
def fight_confirmation_keyboard(fight_id: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Подтвердить", callback_data=f"bet_conf_{fight_id}")],
...
def fight_detail_keyboard(fight_id: int, fighter1_id: int, fighter2_id: int, name1: str, name2: str, odds1: float, odds2: float) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(
            ...
            callback_data=f"bf_{fight_id}_{fighter1_id}",
            style="primary"
        ),
         InlineKeyboardButton(
            text=f"{name2} ({odds2:.2f})", 
            callback_data=f"bf_{fight_id}_{fighter2_id}",
            style="primary"
        )],
        [InlineKeyboardButton(
...
def confirm_delete_keyboard(item_type: str, item_id: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Да, удалить", callback_data=f"del_yes_{item_type}_{item_id}")],
...
async def cmd_start(message: Message):
                    ...
                    async with db_pool.acquire() as conn:
                        ...
                        if referrer_exists and not await is_banned(referrer_id):
                            ...
                            if result == "INSERT 0 1":
                                ...
                                await safe_send_message(referrer_id, 
                                    f"🔗 Новый пользователь {message.from_user.first_name} зарегистрировался по вашей ссылке! Награда будет выдана после того, как он совершит {await get_setting('referral_required_thefts')} успешных ограблений.")
            except:
                pass
    ...
    # Отправляем приветственное сообщение с медиа
    welcome_text = (
        "🌟 <b>Добро пожаловать в Malboro GAME!</b>\n\n"
        "Здесь ты найдёшь:\n"
        "🎰 Казино и розыгрыши\n"
        "🏪 Фармилки и биткоин-биржа\n"
        "💰 Кражи и налёты\n"
        "📋 Задания и рефералы\n\n"
...
async def cmd_help_private(message: Message):
    ...
    text = (
        ...
        "👤 <b>Профиль</b> – статистика и характеристики\n"
        "🎁 <b>Бонус</b> – ежедневный бонус\n"
        "🛒 <b>Магазин</b> – покупка подарков\n"
        "🎰 <b>Казино</b> – азартные игры (кости, угадайка, слоты, рулетка)\n"
        "🎟 <b>Промокод</b> – активация промокодов\n"
        "🏆 <b>Топ игроков</b> – рейтинг по MLB, репутации, биткоинам и т.д.\n"
        "💰 <b>Мои покупки</b> – история заказов\n"
        "🔫 <b>Ограбить</b> – укради MLB у другого\n"
        "📋 <b>Задания</b> – выполняй и получай награды\n"
        "🔗 <b>Рефералка</b> – приглашай друзей\n"
        "📊 <b>Уровень</b> – твой прогресс\n"
        "🎁 <b>Розыгрыши</b> – активные и завершённые\n"
        "🏪 <b>Отмыть BTC</b> – управление фармилками (покупка за BTC)\n"
        "💼 <b>Биткоин-биржа</b> – продавай и покупай BTC за MLB\n"
        "🥊 <b>Подпольные бои</b> – ставки на бойцов\n"
        "🎓 <b>Университет</b> – прокачка навыков за авторитет\n"
        "⚙️ <b>Админка</b> – для администраторов"
    )
...
# ==================== КАЗИНО (только меню, игры в части 4) ====================
...
async def tasks_user_menu(message: Message):
    ...
    for row in rows:
        ...
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="✅ Выполнить", callback_data=f"chk_task_{row['id']}")]
        ])
        await message.answer(text, reply_markup=kb)
    await message.answer("Выбери задание из списка выше.", reply_markup=back_keyboard())

@dp.callback_query(F.data.startswith("chk_task_"))
    ...
    async with db_pool.acquire() as conn:
        ...
        for row in rows:
            ...
            if already:
                ...
                kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="✅ Участвовать", callback_data=f"gw_join_{row['id']}")]])
                await message.answer(text, reply_markup=kb)
...
@dp.callback_query(F.data.startswith("view_comp_"))
...
async def my_orders(message: Message):
    ...
    for row in rows:
        ...
        kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="❌ Отменить", callback_data=f"can_ord_{row['id']}")]])
        await message.answer(text, reply_markup=kb)
...
@dp.callback_query(F.data.startswith("can_ord_"))
...
async def order_book(message: Message):
    ...
    text = "📊 <b>Стакан заявок:</b>\n\n"
    text += "<b>💰 Покупка (Bid):</b>\n"
...
async def perform_theft(message: Message, attacker_id: int, victim_id: int, state: FSMContext):
    ...
    attacker_name = await get_user_name(attacker_id)
    ...
    if rand <= success_chance:
        ...
        # Уведомляем жертву
        await safe_send_message(victim_id, f"🔫 Вас ограбил {attacker_name}! Вы потеряли {amount} MLB.")
        if level_up_msg:
        ...
        # Уведомляем жертву
        await safe_send_message(victim_id, f"🛡 Вы защитились от ограбления {attacker_name}! Он потерял {penalty} MLB.")
    else:
        async with db_pool.acquire() as conn:
            await conn.execute("UPDATE users SET theft_failed = theft_failed + 1 WHERE user_id=$1", attacker_id)
        ...
        # Уведомлять жертву не нужно, она не пострадала
    await state.clear()
...
async def underground_fights_menu(message: Message):
    ...
    for fight in fights:
        ...
        kb.button(text=f"🥊 Бой #{fight['id']}", callback_data=f"fv_{fight['id']}")
    kb.adjust(1)
...
async def fight_view_callback(callback: CallbackQuery):
    fight_id = int(callback.data.split("_")[1])
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"{f1['name']} ({odds1:.2f})", callback_data=f"bf_{fight_id}_{fight['fighter1_id']}"),
         InlineKeyboardButton(text=f"{f2['name']} ({odds2:.2f})", callback_data=f"bf_{fight_id}_{fight['fighter2_id']}")],
...
@dp.callback_query(F.data.startswith("bf_"))
async def bet_fighter_info_callback(callback: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Сделать ставку", callback_data=f"bc_{fight_id}_{fighter_id}")],
        [InlineKeyboardButton(text="◀️ Назад к бою", callback_data=f"fv_{fight_id}")]
    ])
...
async def bet_amount_handler(message: Message, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Да", callback_data=f"bet_ok_{fight_id}")],
        [InlineKeyboardButton(text="❌ Нет", callback_data="bet_cancel")]
    ])
    await message.answer(text, reply_markup=kb)
    await state.set_state(PlaceBet.confirm)

@dp.callback_query(PlaceBet.confirm, F.data.startswith("bet_ok_"))
...
# ==================== КОНЕЦ ЧАСТИ 3 ====================
# ==================== ЧАСТЬ 4: ПОЛЬЗОВАТЕЛЬСКИЕ ХЕНДЛЕРЫ – ИГРЫ КАЗИНО И ПОВТОР СТАВОК ====================
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            if win:
                ...
                phrase = f"🎲 <b>{dice1} + {dice2} = {total}</b> — Победа! +{profit:.2f} MLB!"
            else:
                exp = await get_setting_int("exp_per_dice_lose")
                phrase = f"🎲 <b>{dice1} + {dice2} = {total}</b> — Проигрыш. -{amount:.2f} MLB."
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            if win:
                ...
                phrase = f"🔢 <b>Ты угадал! Было {guess}.</b> Выигрыш: +{profit:.2f} MLB и +{rep_reward} репутации!"
            else:
                exp = await get_setting_int("exp_per_guess_lose")
                secret = random.randint(1, 5)
                phrase = f"🔢 <b>Не угадал. Было {secret}.</b> -{amount:.2f} MLB."
            level_up_msg = await add_exp(user_id, exp, conn=conn)
...
async def slots_bet(message: Message, state: FSMContext):
    ...
    if win and random.random() < 0.1:
        # иногда выпадает джекпот
        result = [random.choice(symbols) for _ in range(3)]
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            if win:
                ...
                phrase = f"🍒 <b>{result_str}</b> — Ура! Выигрыш x{multiplier:.1f}! +{profit:.2f} MLB!"
            else:
                exp = await get_setting_int("exp_per_slots_lose")
                phrase = f"🍒 <b>{result_str}</b> — Не повезло. -{amount:.2f} MLB."
            level_up_msg = await add_exp(user_id, exp, conn=conn)
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            if win:
                ...
                phrase = f"🎡 <b>Выпало {number} {color}!</b> Ты выиграл {profit:.2f} MLB!"
            else:
                exp = await get_setting_int("exp_per_roulette_lose")
                phrase = f"🎡 <b>Выпало {number} {color}.</b> Твоя ставка не сыграла. -{amount:.2f} MLB."
            level_up_msg = await add_exp(user_id, exp, conn=conn)
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            if win:
                ...
                await update_user_reputation(user_id, rep_reward, conn=conn)
                exp = await get_setting_int("exp_per_guess_win")
                phrase = f"🔢 <b>Ты угадал! Было {secret}.</b> Выигрыш: +{profit:.2f} MLB и +{rep_reward} репутации!"
            else:
                exp = await get_setting_int("exp_per_guess_lose")
                phrase = f"🔢 <b>Не угадал. Было {secret}.</b> -{amount:.2f} MLB."
            level_up_msg = await add_exp(user_id, exp, conn=conn)
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            if win:
                ...
                await update_user_balance(user_id, profit, conn=conn, allow_negative=False)
                exp = await get_setting_int("exp_per_roulette_win")
                phrase = f"🎡 <b>Выпало {spin} {color}!</b> Ты выиграл {profit:.2f} MLB!"
            else:
                exp = await get_setting_int("exp_per_roulette_lose")
                phrase = f"🎡 <b>Выпало {spin} {color}.</b> Твоя ставка не сыграла. -{amount:.2f} MLB."
            level_up_msg = await add_exp(user_id, exp, conn=conn)
...
async def university_menu(message: Message):
    ...
    betray_bonus = skills['skill_betray'] * await get_setting_int("skill_betray_bonus_per_level")
    text = (
        f"🎓 <b>Криминальный университет</b>\n\n"
...
# ==================== ПРОМОКОД (хендлеры уже в части 3) ====================
# (перенесено из части 3 для целостности, но они там уже есть)
# @dp.message(F.text == "🎟 Промокод") и promo_activate уже в части 3, оставляем только в части 3.
# ==================== КОНЕЦ ЧАСТИ 5 ====================
...
async def activate_chat_command(message: Message):
    ...
    for admin_id in admins:
        await safe_send_message(
            admin_id,
            f"🔔 <b>Запрос на активацию чата!</b>\n"
...
@dp.callback_query(F.data.startswith("hj_"))
async def heist_join_callback(callback: CallbackQuery):
    heist_id = int(callback.data.split("_")[1])
    user_id = callback.from_user.id
    chat_id = callback.message.chat.id

    logging.info(f"🔥 heist_join_callback START: heist_id={heist_id}, user={user_id}, chat={chat_id}")
    ...
    try:
        ...
        async with db_pool.acquire() as conn:
            async with conn.transaction():
                ...
                # Устанавливаем кулдаун участия с передачей соединения
                await set_global_cooldown(user_id, "heist_participate", participant_cooldown, conn=conn)
...
@dp.callback_query(F.data.startswith("bn_"))
...
async def cmd_chat_heist_status(message: Message):
    ...
    async with db_pool.acquire() as conn:
        ...
        text = (
            f"{status_emoji} <b>Налёт: {HEIST_TYPES[heist['event_type']]['name']}</b>\n"
            f"👥 Участников: {count}\n"
...
async def cmd_jail(message: Message, state: FSMContext):
    ...
    # Устанавливаем состояние для выбора камеры (статья будет случайной)
...
async def jail_cell_callback(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        # Генерируем случайную статью (1-300)
        ...
        if random.random() < 0.01:
            ...
            if chat_id:
                await safe_send_chat(
                    chat_id,
                    f"🎫 <b>ЗОЛОТОЙ БИЛЕТ!</b>\n"
                    f"{callback.from_user.first_name} нашёл золотой билет и получает {gift_amount:.2f} MLB!"
                )
        ...
        await callback.message.answer("✅ Ты отправился в тюрьму! Ожидай результатов.")
        await state.clear()
        
    except Exception as e:
        logging.exception(f"Ошибка в jail_cell_callback: {e}")
...
async def cmd_my_heist(message: Message):
    ...
    async with db_pool.acquire() as conn:
        ...
        if not row:
            ...
            return
        text = (
            f"🔫 <b>Твой текущий налёт ({row['event_type']}):</b>\n"
...
# ==================== КОМАНДА /mlb_fights (СПИСОК АКТИВНЫХ БОЁВ В ЧАТЕ) ====================
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            await safe_send_chat(message.chat.id,
                f"🎁 <b>Администратор {message.from_user.first_name} активировал подгон!</b>\n"
...
async def add_balance_amount(message: Message, state: FSMContext):
    ...
    try:
        async with db_pool.acquire() as conn:
            async with conn.transaction():
                success, new_balance, _ = await update_user_balance(uid, amount, conn=conn, allow_negative=False)
                if not success:
...
async def add_reputation_amount(message: Message, state: FSMContext):
    ...
    try:
        async with db_pool.acquire() as conn:
            async with conn.transaction():
                await update_user_reputation(uid, amount, conn=conn)
...
async def add_bitcoin_amount(message: Message, state: FSMContext):
    ...
    try:
        async with db_pool.acquire() as conn:
            async with conn.transaction():
                success, new_balance = await update_user_bitcoin(uid, amount, conn=conn)
                if not success:
...
async def add_authority_amount(message: Message, state: FSMContext):
    ...
    try:
        async with db_pool.acquire() as conn:
            async with conn.transaction():
                await update_user_authority(uid, amount, conn=conn)
...
async def find_user_result(message: Message, state: FSMContext):
    ...
    last_active_str = last_active.strftime("%Y-%m-%d %H:%M") if last_active else "неизвестно"
    text = (
        f"👤 <b>Пользователь: {name} (ID: {uid})</b>\n"
...
async def reset_stats_admin_user(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"🔑 <b>Сгенерирован ключ для сброса статистики пользователя {uid}:</b>\n"
        f"<code>{key}</code>\n\n"
...
@dp.callback_query(F.data.startswith("rst_conf_"))
...
@dp.callback_query(F.data == "rst_cancel")
...
@dp.callback_query(F.data.startswith("pur_done_"))
...
@dp.callback_query(F.data.startswith("pur_rej_"))
...
async def create_promo_reward_type(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    reward_type = callback.data.split("_")[2]  # coins или btc
...
async def list_promos(message: Message, page: int = 1):
    ...
    try:
        ...
        if not rows:
            ...
            return
        text = f"🎫 <b>Промокоды (страница {page}):</b>\n"
        for row in rows:
...
@dp.callback_query(F.data.startswith("conf_chat_"))
...
@dp.callback_query(F.data.startswith("rej_chat_"))
...
# ==================== КОНЕЦ ЧАСТИ 8 ====================
# ==================== ЧАСТЬ 9: АДМИНИСТРАТИВНАЯ ПАНЕЛЬ (УПРАВЛЕНИЕ БИЗНЕСАМИ, БИРЖЕЙ, МЕДИА, ЗАДАНИЯМИ, РОЗЫГРЫШАМИ, ПОДПОЛЬНЫМИ БОЯМИ) + ПОЛЬЗОВАТЕЛЬСКИЕ "ОТМЫТЬ BTC" ====================
...
async def admin_list_businesses(message: Message, page: int = 1):
    ...
    if page > 1 or page < total_pages:
        builder = InlineKeyboardBuilder()
        if page > 1:
            builder.button(text="⬅️", callback_data=f"abiz_p{page-1}")
        if page < total_pages:
            builder.button(text="➡️", callback_data=f"abiz_p{page+1}")
...
@dp.callback_query(F.data.startswith("abiz_p"))
...
async def add_business_price(message: Message, state: FSMContext):
    ...
    try:
        ...
        return
    await state.update_data(price=price)
    await message.answer("Введи базовый доход в MLB/час:")
...
async def add_business_description(message: Message, state: FSMContext):
    if message.text == "◀️ Назад":
        ...
        return
    await state.update_data(description=message.text)
    await message.answer("Введи максимальный уровень прокачки:")
...
async def add_business_lifetime(message: Message, state: FSMContext):
    ...
    await message.answer("Введи ключ картинки (например, 'business_kiosk'):")
...
async def add_business_image_key(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer("✅ Бизнес добавлен!", reply_markup=admin_business_keyboard())
    except asyncpg.UniqueViolationError:
...
async def edit_business_start(message: Message, state: FSMContext):
    ...
    if not await check_admin_permissions(message.from_user.id, "manage_businesses"):
        return
    await message.answer("Введи ID бизнеса:", reply_markup=back_keyboard())
...
async def edit_business_id(message: Message, state: FSMContext):
    ...
    await message.answer("Что изменить? (name/emoji/price/income/description/max_level/available/image_key/lifetime_hours)")
...
async def edit_business_field(message: Message, state: FSMContext):
    ...
    await state.update_data(field=field)
    if field == 'available':
        await message.answer("Введи True/False:")
    elif field == 'price':
        await message.answer("Введи новую цену в BTC:")
    elif field == 'income':
        await message.answer("Введи новый доход в MLB/час:")
    elif field == 'max_level':
        await message.answer("Введи новый максимальный уровень:")
...
async def admin_exchange_menu(message: Message):
    ...
    if not await check_admin_permissions(message.from_user.id, "manage_exchange"):
        await message.answer("❌ Недостаточно прав.")
        return
    await send_with_media(message.chat.id, "Управление биржей:", media_key='admin_exchange', reply_markup=admin_exchange_keyboard())
...
async def admin_list_orders(message: Message, page: int = 1):
    ...
    text = f"📋 Активные заявки (стр. {page}):\n\n"
    for o in rows:
        d = dict(o)
        text += (
    ...
    if page > 1 or page < total_pages:
        builder = InlineKeyboardBuilder()
        if page > 1:
            builder.button(text="⬅️", callback_data=f"aord_p{page-1}")
        if page < total_pages:
            builder.button(text="➡️", callback_data=f"aord_p{page+1}")
        await message.answer(text, reply_markup=builder.as_markup())
    else:
...
@dp.callback_query(F.data.startswith("aord_p"))
...
async def admin_remove_order_start(message: Message, state: FSMContext):
    ...
    if not await check_admin_permissions(message.from_user.id, "manage_exchange"):
        return
    await message.answer("Введи ID заявки:", reply_markup=back_keyboard())
...
async def admin_remove_order_finish(message: Message, state: FSMContext):
    ...
    if success:
        ...
        await message.answer(f"❌ Не удалось отменить заявку {order_id}.")
    await state.clear()
...
async def admin_media_menu(message: Message):
    ...
    if not await check_admin_permissions(message.from_user.id, "manage_media"):
        await message.answer("❌ Недостаточно прав.")
        return
    await send_with_media(message.chat.id, "Управление медиа:", media_key='admin_media', reply_markup=admin_media_keyboard())
...
async def add_media_start(message: Message, state: FSMContext):
    ...
    if not await check_admin_permissions(message.from_user.id, "manage_media"):
        return
    await message.answer("Введи ключ (например, 'profile'):", reply_markup=back_keyboard())
...
async def add_media_key(message: Message, state: FSMContext):
    ...
    await message.answer("Отправь фото/документ/видео:")
...
async def remove_media_start(message: Message, state: FSMContext):
    ...
    if not await check_admin_permissions(message.from_user.id, "manage_media"):
        return
    await message.answer("Введи ключ медиа:", reply_markup=back_keyboard())
...
async def remove_media_finish(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(f"✅ Медиа с ключом '{key}' удалено.")
    except Exception as e:
...
async def list_media(message: Message):
    ...
    if not rows:
        await message.answer("Нет медиа.")
        return
...
async def create_task_type(message: Message, state: FSMContext):
    ...
    await message.answer("Введи ID канала или @username:")
...
async def create_task_target(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(f"❌ Не удалось проверить канал: {e}")
        await state.clear()
        return
...
async def create_task_reward_coins(message: Message, state: FSMContext):
    ...
    await message.answer("Введи награду в репутации (целое):")
...
async def create_task_reward_rep(message: Message, state: FSMContext):
    ...
    await message.answer("Введи макс. количество выполнений (0 - без лимита):")
...
async def create_task_penalty_coins(message: Message, state: FSMContext):
    ...
    await message.answer("Введи штраф в репутации за отписку (целое):")
...
async def create_task_button_link(message: Message, state: FSMContext):
    ...
    await message.answer("Отправь медиа (или 'нет'):")
...
async def list_tasks(message: Message, page: int = 1):
    ...
    try:
        ...
        if page > 1 or page < total_pages:
            builder = InlineKeyboardBuilder()
            if page > 1:
                builder.button(text="⬅️", callback_data=f"tsk_p{page-1}")
            if page < total_pages:
                builder.button(text="➡️", callback_data=f"tsk_p{page+1}")
            await message.answer(text, reply_markup=builder.as_markup())
        else:
...
@dp.callback_query(F.data.startswith("tsk_p"))
...
async def delete_task_start(message: Message, state: FSMContext):
    ...
    if not await check_admin_permissions(message.from_user.id, "manage_users"):
        return
    await message.answer("Введи ID задания:", reply_markup=back_keyboard())
...
def admin_giveaway_keyboard() -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text="📋 Активные розыгрыши")],
            [KeyboardButton(text="🏁 Завершённые розыгрыши")],
            [KeyboardButton(text="🎲 Завершить вручную")],
            [KeyboardButton(text="◀️ Назад в админку")]
        ],
        resize_keyboard=True
    )
...
@dp.callback_query(CreateGiveaway.condition_type, F.data.startswith("gw_cond_"))
async def create_giveaway_condition(callback: CallbackQuery, state: FSMContext):
    cond = callback.data.split("_")[2]  # time или parts
    await state.update_data(condition_type=cond)
    if cond == "time":
        await callback.message.edit_text("Введи дату и время в формате ГГГГ-ММ-ДД ЧЧ:ММ:")
...
async def create_giveaway_end_date(message: Message, state: FSMContext):
    ...
    await message.answer("Введи количество победителей (по умолчанию 1):", reply_markup=back_keyboard())
...
async def list_active_giveaways(message: Message, page: int = 1):
    ...
    if not rows:
        await message.answer("Нет активных розыгрышей.")
        return
    text = f"📋 Активные розыгрыши (стр. {page}):\n\n"
    for r in rows:
    ...
    if page > 1 or page < total_pages:
        builder = InlineKeyboardBuilder()
        if page > 1:
            builder.button(text="⬅️", callback_data=f"agw_p{page-1}")
        if page < total_pages:
            builder.button(text="➡️", callback_data=f"agw_p{page+1}")
        await message.answer(text, reply_markup=builder.as_markup())
    else:
...
@dp.callback_query(F.data.startswith("agw_p"))
...
async def list_completed_giveaways(message: Message, page: int = 1):
    ...
    if not rows:
        await message.answer("Нет завершённых розыгрышей.")
        return
    text = f"🏁 Завершённые розыгрыши (стр. {page}):\n\n"
    for r in rows:
    ...
    if page > 1 or page < total_pages:
        builder = InlineKeyboardBuilder()
        if page > 1:
            builder.button(text="⬅️", callback_data=f"cgw_p{page-1}")
        if page < total_pages:
            builder.button(text="➡️", callback_data=f"cgw_p{page+1}")
        await message.answer(text, reply_markup=builder.as_markup())
    else:
...
@dp.callback_query(F.data.startswith("cgw_p"))
...
@dp.message(F.text == "🎲 Завершить вручную")
...
async def create_fighter_name(message: Message, state: FSMContext):
    if message.text == "◀️ Назад":
        ...
        return
    await state.update_data(name=message.text)
    await message.answer("Введи описание бойца:")
...
async def create_fighter_description(message: Message, state: FSMContext):
    if message.text == "◀️ Назад":
        ...
        return
    await state.update_data(description=message.text)
    await message.answer("Введи силу (1-10):")
...
async def create_fighter_strength(message: Message, state: FSMContext):
    ...
    await message.answer("Введи ловкость (1-10):")
...
async def create_fighter_agility(message: Message, state: FSMContext):
    ...
    await message.answer("Введи выносливость (1-10):")
...
async def list_fighters(message: Message, page: int = 1):
    ...
    if page > 1 or page < total_pages:
        builder = InlineKeyboardBuilder()
        if page > 1:
            builder.button(text="⬅️", callback_data=f"fghtr_p{page-1}")
        if page < total_pages:
            builder.button(text="➡️", callback_data=f"fghtr_p{page+1}")
        await message.answer(text, reply_markup=builder.as_markup())
    else:
...
@dp.callback_query(F.data.startswith("fghtr_p"))
...
async def edit_fighter_start(message: Message, state: FSMContext):
    ...
    if not await check_admin_permissions(message.from_user.id, "manage_fights"):
        return
    await message.answer("Введи ID бойца:", reply_markup=back_keyboard())
...
async def edit_fighter_id(message: Message, state: FSMContext):
    ...
    await message.answer("Что изменить? (name/description/strength/agility/stamina/photo)")
...
async def edit_fighter_field(message: Message, state: FSMContext):
    ...
    if field == 'photo':
        ...
        await message.answer(f"Введи новое значение для {field} (1-10):")
    else:
        await message.answer(f"Введи новое значение для {field}:")
...
async def delete_fighter_id(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"⚠️ Удалить бойца {fighter['name']} (ID {fid})?",
        reply_markup=confirm_delete_keyboard('fighter', fid)
    )
    await state.set_state(DeleteFighter.confirm)

@dp.callback_query(DeleteFighter.confirm, F.data.startswith("del_yes_fighter_"))
async def delete_fighter_confirm(callback: CallbackQuery, state: FSMContext):
    ...
    if success:
        await callback.message.edit_text(f"✅ Боец удалён.")
    else:
        await callback.message.edit_text("❌ Не удалось удалить (возможно, участвует в активных боях).")
    await state.clear()
...
async def create_fight_start(message: Message, state: FSMContext):
    ...
    if not await check_admin_permissions(message.from_user.id, "manage_fights"):
        return
    fighters, total = await get_fighters_list(only_ids=True, page=1, items_per_page=100)
    if not fighters:
        await message.answer("❌ Сначала создайте бойцов.")
        return
    await state.update_data(fighters=fighters)
    kb = fighter_selection_keyboard(fighters, page=1, total_pages=1, callback_prefix="f1")
    await message.answer("Выбери первого бойца:", reply_markup=kb)
    await state.set_state(CreateFight.fighter1)

@dp.callback_query(CreateFight.fighter1, F.data.startswith("f1_"))
async def create_fight_fighter1(callback: CallbackQuery, state: FSMContext):
    if not await check_admin_permissions(callback.from_user.id, "manage_fights"):
        await callback.answer("❌ Нет прав", show_alert=True)
        return
    fighter_id = int(callback.data.split("_")[1])
    data = await state.get_data()
    ...
    if not other_fighters:
        ...
        await state.clear()
        return
    kb = fighter_selection_keyboard(other_fighters, page=1, total_pages=1, callback_prefix="f2")
    await callback.message.edit_text("Выбери второго бойца:", reply_markup=kb)
    await state.set_state(CreateFight.fighter2)

@dp.callback_query(CreateFight.fighter2, F.data.startswith("f2_"))
...
async def create_fight_confirm(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await callback.message.edit_text(f"✅ Бой создан! ID: {fight_id}.")
    except Exception as e:
...
async def list_fights(message: Message, page: int = 1):
    ...
    if page > 1 or page < total_pages:
        builder = InlineKeyboardBuilder()
        if page > 1:
            builder.button(text="⬅️", callback_data=f"flist_p{page-1}")
        if page < total_pages:
            builder.button(text="➡️", callback_data=f"flist_p{page+1}")
        await message.answer(text, reply_markup=builder.as_markup())
    else:
...
@dp.callback_query(F.data.startswith("flist_p"))
...
async def delete_fight_start(message: Message, state: FSMContext):
    ...
    if not await check_admin_permissions(message.from_user.id, "manage_fights"):
        return
    await message.answer("Введи ID боя:", reply_markup=back_keyboard())
...
async def delete_fight_id(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"⚠️ Удалить бой ID {fid}?",
        reply_markup=confirm_delete_keyboard('fight', fid)
    )
    await state.set_state(DeleteFight.confirm)

@dp.callback_query(DeleteFight.confirm, F.data.startswith("del_yes_fight_"))
async def delete_fight_confirm(callback: CallbackQuery, state: FSMContext):
    ...
    if success:
        await callback.message.edit_text(f"✅ Бой удалён.")
    else:
        await callback.message.edit_text("❌ Не удалось удалить.")
    await state.clear()

# ==================== ПОЛЬЗОВАТЕЛЬСКИЕ ХЕНДЛЕРЫ ДЛЯ "ОТМЫТЬ BTC" (БЫВШАЯ ФАРМИЛКА) ====================

@dp.message(F.text == "🏪 Отмыть BTC")
async def my_businesses_handler(message: Message):
    ...
    if not businesses:
        ...
        if biz_types:
            text = "🏪 У вас пока нет бизнесов. Выберите для покупки:\n\n"
            for bt in biz_types:
            ...
            await message.answer("🏪 Бизнесы временно недоступны.")
        return
    
    await message.answer("🏪 Ваши бизнесы:", reply_markup=business_main_keyboard(businesses))
...
async def buy_business_start(message: Message, state: FSMContext):
    ...
    biz_types = await get_business_types_list(only_available=True)
    if not biz_types:
        await message.answer("❌ Нет доступных бизнесов.")
        return
...
async def buy_business_confirm(message: Message, state: FSMContext):
    ...
    # (Предварительная проверка не обязательна, но можно оставить для быстроты)
    # if max_biz > 0:
    #     current_biz = await get_user_businesses(user_id)
    #     if len(current_biz) >= max_biz:
    #         await message.answer(f"❌ Вы не можете купить больше {max_biz} бизнесов.")
    #         return
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            # Блокируем пользователя для проверки лимита
            if max_biz > 0:
                # Блокировка строки пользователя
                await conn.fetchval("SELECT 1 FROM users WHERE user_id=$1 FOR UPDATE", user_id)
                current_biz = await conn.fetchval("SELECT COUNT(*) FROM user_businesses WHERE user_id=$1", user_id)
                if current_biz >= max_biz:
                    await message.answer(f"❌ Вы не можете купить больше {max_biz} бизнесов.")
                    return
...
async def collect_business_income_handler(message: Message, state: FSMContext):
    ...
    if not business_name:
        await message.answer("❌ Сначала выберите бизнес.")
        return
...
async def list_admins(message: Message, page: int = 1):
    ...
    if page > 1 or page < total_pages:
        builder = InlineKeyboardBuilder()
        if page > 1:
            builder.button(text="⬅️", callback_data=f"adm_p{page-1}")
        if page < total_pages:
            builder.button(text="➡️", callback_data=f"adm_p{page+1}")
        await message.answer(text, reply_markup=builder.as_markup())
    else:
...
@dp.callback_query(F.data.startswith("adm_p"))
...
@dp.message(F.text == "📢 Рассылка в чаты")
...
@dp.callback_query(F.data.startswith("set_back_"))
...
async def check_task_expirations():
    ...
    while True:
        try:
            await asyncio.sleep(await get_setting_int("task_subscribe_check_interval"))
            now = datetime.now()
            async with db_pool.acquire() as conn:
        ...
        try:
            ...
            try:
                async with db_pool.acquire() as conn:
                    ...
                    logging.info("Статистика чатов обновлена (active_users_today сброшена)")
            finally:
...
async def on_startup():
    ...
    asyncio.create_task(update_chat_stats())
