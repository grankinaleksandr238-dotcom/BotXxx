...
# Импортируем необходимые функции и классы из Частей 1 и 2
# (предполагается, что всё уже определено в глобальной области)
...
async def universal_back_handler(message: Message, state: FSMContext):
    ...
    elif current_state.startswith('ResetStats'):
        await state.finish()
        await admin_users_menu(message)
...
async def guess_number_callback(callback: CallbackQuery, state: FSMContext):
    ...
    multiplier = 2.0  # можно добавить в настройки
    rep_reward = 1
...
# ----- Повтор ставки (исправлен для рулетки) -----
...
async def buy_callback(callback: CallbackQuery):
    ...
    try:
        ...
        if len(parts) != 3 or parts[0] != "buy" or parts[1] != "item":
...
async def tasks_user_menu(message: Message):
    ...
    for row in rows:
        ...
        if row['media_file_id'] and row['media_type'] == 'photo':
            # Для каждого задания можно отправить картинку отдельно, но здесь просто ссылка
            pass
...
# ==================== КОНЕЦ ЧАСТИ 3 ====================
# ==================== ЧАСТЬ 4: ГРУППОВЫЕ ХЕНДЛЕРЫ (ИСПРАВЛЕННАЯ) ====================
...
async def cmd_smuggle(message: Message):
    ...
    if file_id:
        ...
        # Удаляем ответ бота через настройки
        delete_seconds = int(await get_setting("auto_delete_commands_seconds"))
        asyncio.create_task(delete_after(sent, delete_seconds))
    else:
...
async def chat_gift(message: Message):
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            chat_info = await conn.fetchrow("SELECT * FROM confirmed_chats WHERE chat_id=$1", message.chat.id)
            if not chat_info:
                return
...
# Предполагается, что все функции из Части 1 и клавиатуры/состояния из Части 2 уже импортированы глобально
# (при сборке они будут доступны)
...
# ----- Сброс статистики по ключу -----
@dp.message_handler(lambda message: message.text == "🔄 Сброс статистики")
async def reset_stats_start(message: Message, state: FSMContext):
    if not await check_admin_permissions(message.from_user.id, "manage_users"):
        await message.answer("❌ Недостаточно прав.")
        return
    await message.answer("Введи ID или @username пользователя, чью статистику нужно сбросить:", reply_markup=back_keyboard())
    await ResetStats.key.set()

@dp.message_handler(state=ResetStats.key)
async def reset_stats_user_input(message: Message, state: FSMContext):
    if message.text == "◀️ Назад":
        await state.finish()
        await admin_users_menu(message)
        return
    ...
    await message.answer(
        ...
        f"<code>{key}</code>\n\n"
        f"Пользователь должен ввести этот ключ в личных сообщениях с ботом, используя команду /resetstats.\n"
        f"Ключ действителен 10 минут.\n\n"
        f"После подтверждения пользователем статистика будет сброшена.",
        reply_markup=back_keyboard()
    )
    await state.finish()
...
async def create_promo_reward(message: Message, state: FSMContext):
    ...
    await message.answer("Выбери тип награды:", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def create_promo_reward_type(callback: CallbackQuery, state: FSMContext):
    reward_type = callback.data.split("_")[2]  # coins или bitcoin
...
async def admin_tasks_menu(message: Message):
    if not await check_admin_permissions(message.from_user.id, "manage_users"):  # можно выделить отдельное право
        await message.answer("❌ Недостаточно прав.")
        return
...
async def heist_spawner():
    ...
    while True:
        try:
            # Ждём фиксированный интервал 70 минут (или из настроек)
...
async def on_startup(dp):
    ...
    await bot.set_my_commands([
        ...
        # Команда для сброса статистики пользователем (будет добавлена)
        # types.BotCommand("resetstats", "🔄 Сбросить статистику по ключу"),
    ])
